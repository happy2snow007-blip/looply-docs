...
| 终端 | 页面框架 | 内容顺序 |
|---|---|---|
| PC Web | 使用 PC Web 公共 Header 与 Footer | 页面标题 → 联系表单 → 客服联系信息 |
| Mobile Web / App | 游客态和登录态的个人中心均展示 Contact Us 入口；页面使用 Mobile 公共页面框架，顶部提供返回操作 | 页面标题 → 联系表单 → 客服联系信息 |
...
| 状态 | 触发条件 | 页面表现 | 可用操作 |
|---|---|---|---|
| 默认态 | 页面正常打开 | 游客三个字段为空；登录用户自动带入 Full Name 和 Email，Message 为空；同时展示完整联系信息 | 填写、提交、点击邮箱或电话 |
| 字段错误态 | 输入未通过校验 | 对应字段显示错误提示，保留其他已填内容 | 修改后重新提交 |
| 提交中 | 字段校验通过，正在提交 | Send 显示 Loading，全部字段保留并暂不可再次提交 | 等待提交结果 |
...
| 提交中 | 按本 PRD 规则补齐 | 按本 PRD 规则补齐 |
| 提交成功 | 按本 PRD 规则补齐 | 按本 PRD 规则补齐 |
| 提交失败 | 按本 PRD 规则补齐 | 按本 PRD 规则补齐 |
...
| 输入源 | 覆盖情况 | 本 PRD 处理 |
|---|---|---|
| Looply 现有 PC 页面 | 参考页已覆盖默认页面、字段及联系信息 | 沿用页面结构和字段，并补齐交互状态；当前版本不接入验证码服务 |
...
| Looply 法律文件链接 | 按当前市场展示已批准生效版本 | 依赖市场配置 | 依赖市场配置 | 多市场版本映射属于后续阶段；本期不新增香港市场链接 |
