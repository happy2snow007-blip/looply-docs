...
**后台原型参考**：`looply-shop页导航栏配置-后台原型-v0.2-antd.html`
...
| `levels` | 枚举 | 是 | `2`（两级）/ `3`（三级）；创建时选择，保存后不可修改 |
...
| 操作 | 规则 |
|---|---|
| 新增 | 名称和 levels 必填；market_id/channel_id/terminal 自动继承全局维度，不可手动修改 |
| 编辑 | 可修改名称、enabled；levels、market_id/channel_id/terminal 不可修改 |
| 排序 | 拖拽排序（拖拽手柄在行首），决定前台左侧导航展示顺序 |
| 启停 | 切换 enabled；停用后前台不展示该 Navigation，其下 Menu Items 不受影响，重新启用后恢复显示 |
| 删除 | **仅当 enabled=false（停用状态）时可操作删除**；删除前弹窗确认"将同时删除该导航下所有二级和三级导航项，不可恢复"；点击确认后，该 Navigation 及其全部 Menu Items 一并删除 |
...
#### 2.6.3 Menu Items（二级/三级导航）管理

**功能描述**：运营按 Navigation 的 levels 管理内容。两级模式直接维护 L2 图文卡片；三级模式先维护 L2 纯文字标题，再在每个 L2 下维护 L3 图文卡片。
...
| `level` | 枚举 | 是 | `2` 或 `3`；由所属 Navigation 的 levels 和 parentId 共同校验 |
| `parentId` | 关联 | 条件必填 | L2 Item 固定为 `null`；L3 Item 必填，且必须关联同一 Navigation 下的 L2 Item |
| `name` | 文本 | 是 | L2 或叶子卡片的展示名称；最大字符数以设计规范为准，输入框实时显示剩余字符数 |
| `image` | 图片 | 条件必填 | 两级模式的 L2 与三级模式的 L3 必填，1:1 比例；三级模式 L2 不填写图片 |
| `targetType` | 枚举 | 条件必填 | 叶子卡片必填：`collection`（关联合集）/ `external`（自定义链接）；三级模式 L2 不填写跳转目标 |
| `collectionId` | 关联 | 条件必填 | targetType=collection 时必填；保存 `collection_id`，由 Collection 模块返回展示标题与 `url_slug` |
| `targetUrl` | 字符串 | 条件必填 | 叶子卡片必填；collection 时由 `url_slug` 自动填充 `/collections/{url_slug}` 并只读，external 时由运营填写 |
| `enabled` | 布尔 | 是 | true（启用）/ false（停用）；默认 true |
| `order` | 整数 | 自动 | 两级模式按 Navigation 排列；三级模式 L2 按 Navigation 排列、L3 按 parentId 排列 |
...
| 操作 | 规则 |
|---|---|
| 新增 | 两级模式新增 L2 图文卡片；三级模式新增 L2 标题或在指定 L2 下新增 L3 图文卡片。叶子卡片的图片、targetType 与跳转信息完整后方可提交 |
| 编辑 | Navigation、level 与 parentId 不可修改；L2 可修改名称、enabled；叶子卡片可修改名称、图片、targetType、collectionId、targetUrl、enabled |
| 排序 | 拖拽排序（拖拽手柄在行首）；仅在相同层级和相同父级内有效 |
| 启停 | 切换 enabled；停用的 L2 及其 L3 不在前台展示，停用的叶子卡片不展示 |
| 删除 | **仅当 enabled=false（停用状态）时可操作删除**；删除 L2 时连同其全部 L3 一并删除，删除叶子卡片不影响同级其他项 |

**去重规则**：
- **L2 Item name**：同一 Navigation 下不可重复；字母大小写不同视为同一名称。
- **L3 Item name**：同一 parentId 下不可重复；字母大小写不同视为同一名称。
...
| 层级 | L2 / L3 |
| 名称 | Menu Item 展示名称 |
| 父级 | L3 展示所属 L2 标题；L2 显示“—” |
| 关联 Collection | 叶子卡片且 targetType=collection 时展示合集名称；其他显示“—” |
| 跳转链接 | 叶子卡片的 targetUrl；L2 显示“—” |
| 状态 | 启用/停用开关 |
| 操作 | 编辑按钮（常驻）；删除按钮（仅 enabled=false 时展示） |

**Collection 关联说明**：选择 Collection 后，后台展示 Collection 返回的当前语言标题和落地页链接（`/collections/{url_slug}`）；链接可点击在新标签页打开，供运营确认关联是否正确。字段对齐 Collection PRD v0.22：`collection_id`（唯一标识）、本地化展示标题、`url_slug`（URL 路径段）。

**逆向链路（Collection 被删除或 slug 变更）**：Collection 删除前必须阻止存在 enabled 叶子卡片引用的删除操作；运营先停用或改绑全部引用后才可删除。Collection 的 `url_slug` 变更后，关联叶子卡片的 targetUrl 自动同步更新。
