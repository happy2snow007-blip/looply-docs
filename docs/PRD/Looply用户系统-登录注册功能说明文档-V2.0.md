...
2. 用户输入邮箱地址
3. 用户设置密码
4. 用户勾选「I agree to the Terms of Service and Privacy Policy」
5. 用户点击「Create account」提交注册
6. 系统采集设备指纹 + IP
7. 系统校验注册限频（同一 IP / 设备指纹）
8. 系统校验邮箱是否已注册
9. 系统执行风控检测（参见 六、5.1 风控检测规则）
10. 风控通过，发送 6 位验证码到用户邮箱
11. 跳转至验证码校验页面（VerifyCode）
12. 用户输入验证码，点击「Verify code」
13. 验证通过，创建账号，签发 Token，记录设备指纹并关联 Session
14. 自动登录并跳转至首页
...
- 「Create account」主按钮（紫色）
- 分隔线「or」
- 「Continue with Google」按钮
- 「Continue with Apple」按钮
- 「Continue with Facebook」按钮
- 协议勾选：复选框 +「I agree to the Terms of Service and Privacy Policy」
...
**注册页（邮箱注册）：**
- 表单底部展示复选框 + 协议文案：「I agree to the Terms of Service and Privacy Policy」
...
- 顶部导航栏：左侧 Looply Logo + 右侧「Back」按钮，底部分隔线
- 内容区域：协议正文（长文本，支持滚动）
- 最后更新日期
...
### 2.7 OAuth 补充信息页（OAuthSupplementary）
...
- 用户点击「Skip for now」→ 跳过补充信息，直接跳转首页
...
| IP 跨国 | 直接拦截 | 不提供验证通道，展示拦截页面 |
| 暴力破解触发 | 图形验证码（CAPTCHA） | 该 IP 下所有请求需通过 CAPTCHA，持续 30 分钟 |

#### 高风险拦截处理

- **拦截页面提示**：「Unusual activity detected. Your request has been blocked for security reasons.」
- **申诉方式**：页面提供客服邮箱链接（support@looply.com），用户发邮件申诉
- **前期不做自助解封流程**，由客服人工审核处理
- **拦截记录写入风控日志**，客服后台可查询
...
| 风控拦截（跨国/高风险） | Unusual activity detected. Your request has been blocked for security reasons. | 独立拦截页面，含客服邮箱链接 |
