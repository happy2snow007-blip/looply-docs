...
- **说明**：首页为登录/注册的统一入口，页面顶部提供「Sign In」和「Sign Up」按钮。首页设计稿见独立文件：PC 端 looply-home-PC.pen / APP 端 looply-home-H5.pen（位于「海外业务首页/UI/」目录）

### 2.2 国家/地区选择器（Country Selector）
...
- PC 端：设计稿 CountrySelector（模态弹窗，居中显示）
- APP 端：设计稿 A-Onboarding-CountryConfirm（全屏引导页，替代弹窗方式，见下方说明）

#### APP 端国家确认引导页（Onboarding-CountryConfirm）

APP 端不使用模态弹窗，改为全屏引导页形式，在注册完成后首次进入时展示。

**页面元素：**

- 全屏紫色渐变背景（品牌色 #6432FC → #4A1ED4）
- 品牌 Logo + 标语「Verified luxury, secondhand first」
- 半透明遮罩 + 确认卡片（白色圆角卡片，居中偏下）
  - 国旗 emoji（圆形浅紫背景 64×64px）
  - 标题「Welcome to Looply」
  - 说明「We detected you're in {国家名}」
  - 「Continue as {国家名}」主按钮
  - 提示「Country/region can't be changed later」（灰色小字）

**交互说明：**

- 系统根据 IP 自动检测用户所在国家，预填到卡片中
- 点击「Continue」→ 确认国家选择，进入首页
- 不可关闭遮罩，必须确认后才能继续
- 确认后国家信息写入 user_profile.country_code，不可更改（业务成熟后再评估是否支持更换）
...
### 2.4 OAuth 快捷注册（Google / Apple / Facebook）
...
| 状态 | 说明 | 设计稿（PC） | 设计稿（APP） |
|------|------|-------------|-------------|
| 默认状态（可重发） | 底部显示「Didn't receive the code? **Resend**」，Resend 为紫色可点击链接 | VerifyCode | A-VerifyCode |
| 倒计时状态 | 底部显示「Didn't receive the code? Resend in 60s」，倒计时文案为灰色不可点击，60 秒后自动恢复为可重发状态 | VerifyCode-Countdown | A-VerifyCode-Countdown |
...
| 错误状态 | 输入框红色边框 + 字段下方 inline 错误提示（仅使用 Input/Error 组件，不使用顶部 Banner） | Login-ErrorState | A-Login-ErrorState |
| 加载状态 | 按钮显示 loading 动画，按钮文案变为「Signing in...」+ spinner | Login-LoadingState（设计稿待补） | A-Login-LoadingState |

#### UI 关联

- PC 端：设计稿 Login / Login-PasswordVisible / Login-ErrorState / Login-LoadingState（设计稿待补）
- APP 端：设计稿 A-Login / A-Login-PasswordVisible / A-Login-ErrorState / A-Login-LoadingState
...
已通过第三方注册的用户，使用对应账号一键登录。未注册用户点击后自动走注册流程（参见 2.4）。
...
5. 系统判断 provider_uid 是否已绑定
   ...
   - 未绑定：走 OAuth 注册流程（参见 2.4 步骤 9 起）
...
> **备注**：首页设计稿见独立文件：PC 端 looply-home-PC.pen / APP 端 looply-home-H5.pen（位于「海外业务首页/UI/」目录）。
...
- 验证码输入、重发、错误处理等逻辑与标准 VerifyCode 一致（参见 2.9）
- 验证通过后，正常完成登录流程
...
| 登录-错误状态 | Login-ErrorState | A-Login-ErrorState |
| 登录-加载状态 | Login-LoadingState（待补） | A-Login-LoadingState |
...
| 国家确认 | CountrySelector（弹窗） | A-Onboarding-CountryConfirm（全屏引导页） |
| 账号注销 | AccountDeletion | A-AccountDeletion |
