...
```
首页
├─→ 登录页（Login，默认展示验证码方式 Login/Code）
│    ├─→ 验证码弹窗（VerifyCodeModal，覆盖在登录页之上，不跳页）
│    │    ├─→ 首页（已有账号登录成功 / 新建账号后登录成功）
│    │    └─→ 登录页（关闭弹窗 / 改邮箱 / 返回登录）
│    ├─→ 登录页-密码方式（Login/Password，切换后）
│    ├─→ 注册页（Register）←→ 登录页
...
8. 系统判断 provider_uid 是否已绑定 Looply 账号
   - 已绑定（老用户）：直接签发 Token，跳转首页（参见 三、3.4）
   - 未绑定：继续下方注册流程
9. 系统判断是否获取到邮箱
...
- **登录页主按钮同时承担建号动作**（邮箱验证码登录时若该邮箱无账号则自动建号，参见 3.2），因此协议提示文案必须覆盖注册含义。源语种（英文）文案定为：

  > `By continuing, you agree to our Terms and Privacy Policy`

  该文案由产品定义，设计稿中的旧文案（`By signing in, ...`）已作废，以本节为准。

#### 合规要求

- 注册页和登录页均采用 Clickwrap 方式（点击操作按钮即视为同意），符合美国市场主流合规实践
- 邮箱验证码登录触发自动建号时，同样按 Clickwrap 记录协议同意（写入 ConsentRecord，参见 3.2）
- 记录用户同意的时间戳和协议版本号
...
| 场景 | 触发时机 | 验证通过后 | 承载页面 |
|------|----------|------------|----------|
| 邮箱注册 | 用户提交注册信息后 | 创建账号，自动登录跳转首页 | VerifyCode（独立页面） |
| 邮箱验证码登录 | 用户在登录页提交邮箱后 | 已有账号则登录；无账号则新建账号后登录（参见 3.2） | VerifyCodeModal（弹窗） |
| 忘记密码 | 用户提交邮箱后 | 跳转至重置密码页面 | VerifyCode（独立页面） |

> 三个场景共用同一套验证码规则（下方「验证码规则」「重发机制」「异常处理」），但**承载形态不同**：注册与忘记密码使用独立页面 VerifyCode（元素与状态变体见本节下方）；邮箱验证码登录使用**弹窗** VerifyCodeModal，覆盖在登录页之上、不跳页。该弹窗为 checkout 模块已实现的共享组件，登录场景直接复用，元素构成与状态变体见 3.2。
...
- **登录方式**：邮箱验证码登录（**登录页默认方式**，参见 3.2）、邮箱密码登录（参见 3.3）、OAuth 快捷登录（参见 3.4）
- **方式切换**：登录页在邮箱验证码与邮箱密码两种方式间提供切换入口，OAuth 三方登录按钮在两种方式下均展示。每次进入登录页均默认展示验证码方式，**不记忆用户上次的选择**
- **页面标题**：登录页仅展示主标题，**不展示副标题**（原副标题文案已移除）

### 3.2 邮箱验证码登录

#### 功能描述

用户输入邮箱、接收 6 位验证码并填写，无需密码即可进入站内。**本方式为登录页的默认方式。**

验证码校验通过后：该邮箱在系统中已有账号则登录该账号；系统中没有该邮箱则创建账号后登录。**两种情况在用户端表现完全一致**（见下方「账号枚举防护」）。

> **背景**：老用户的账号数据不迁移至新系统，首次访问时系统中查不到其账号。若登录仅支持邮箱密码，这部分用户必须先完整走一遍注册才能进入，因此将验证码登录设为默认方式，并让它同时承担建号职责。
...
- 登录页：左侧品牌展示区（660px 宽），右侧表单区垂直居中，与注册页、密码登录方式共用同一布局
- 验证码环节：**弹窗覆盖在登录页之上，不跳转页面**。弹窗为 checkout 模块已实现的共享组件（VerifyCodeModal），登录场景直接复用，不新建页面
...
| 标题 | 仅主标题，**不展示副标题** |
| 邮箱输入框 | 带字段标签 |
| 登录方式切换行 | 左侧为提示文案，右侧为切换至密码登录的链接（点击后切到 3.3） |
| 主按钮 | 提交邮箱、触发发送验证码并弹出验证码弹窗 |
| 分隔线 + OAuth 三方按钮 | Google / Facebook / Apple，行为与密码登录一致（参见 3.4） |
| 底部注册引导链接 | 跳转注册页 |
| 底部协议提示 | Clickwrap 方式，点击主按钮即视为同意；文案见 2.7 |

具体样式与其余文案见设计稿。

> **与密码登录方式（3.3）的元素差异**：本方式下**不展示** Remember me 复选框，也**不展示** Forgot password 链接——两者均只在密码方式下出现。

#### 弹窗元素（PC 端 · VerifyCodeModal）

**本弹窗为 checkout 模块已实现的共享组件，登录场景直接复用，不做改动。**
...
| 邮件图标 | — |
| 标题 | — |
| 副标题 + 修改邮箱入口 | 副标题回显本次收件邮箱，末尾带一个可点的修改入口 |
| 6 位验证码输入框 | 每位独立输入框，弹出时第 1 位自动聚焦 |
| 主按钮 | 提交验证码 |
| 重发行 | 位于主按钮下方。冷却中展示带倒计时的不可点文案，冷却结束变为可点的重发链接 |
| 返回登录链接 | 位于弹窗最底部 |

**两条展示口径：**

1. **重发行与验证码错误提示不同时出现**——验证码校验失败时，主按钮下方的重发行被错误提示文案替换。
2. **关闭按钮、修改邮箱入口、返回登录链接三者在登录场景下落点相同**：关闭弹窗、回到登录页，登录页邮箱输入框保留原值；其中修改邮箱入口额外将焦点置于邮箱输入框。（三者的差异来自 checkout 场景。）

> 弹窗取自 checkout 页面，帧中的收货地址、商品清单、金额、支付按钮均为 checkout 场景的背景内容，**不属于登录场景**，实现时只取弹窗本身。

#### 操作流程

**主流程：**

1. 用户进入登录页，页面默认展示邮箱验证码方式
2. 用户输入邮箱，点击主按钮
3. 系统采集设备指纹 + IP
4. 系统执行风控检测（参见 六、6.2 风控检测规则）
5. 系统校验发信限频（同一邮箱，参见 6.6）
6. 通过后向该邮箱发送 6 位验证码，**在当前页弹出验证码弹窗**（不跳页），副标题回显该邮箱
7. 用户输入验证码，点击弹窗主按钮
8. 系统校验验证码（规则参见 2.8）
9. 验证码正确后，按序判断：
   - 该邮箱已有账号，且账号处于**锁定**状态（密码连续输错触发，参见 6.4）→ 拒绝登录，提示锁定文案
   - 该邮箱已有账号，且账号处于**冻结**状态 → 拒绝登录，提示冻结文案
   - 该邮箱已有账号，状态正常 → 进入步骤 11
   - 该邮箱**无账号** → 进入步骤 10
10. 校验注册限频（同一设备指纹，参见 6.5）；通过则创建账号（初始状态见下方「新建账号的初始状态」），并写入 ConsentRecord
11. 签发 Token（设置 Cookie），记录设备指纹并关联 Session
12. 登录成功，跳转至首页（或登录前访问的页面）

**分支流程：**

- 邮箱格式错误 → 实时提示「Please enter a valid email address」
- 风控拦截 → 拦截或要求额外验证
- 发信限频触发（60 秒内重复发送 / 同一邮箱每小时超过 5 封）→ 提示「Too many requests. Please try again later.」
- 验证码错误 / 过期 / 连续 5 次错误 → 参见 2.8 异常处理
- 注册限频触发 → 提示「Too many attempts. Please try again later.」
- 账号被锁定 → 提示「Account locked. Too many failed attempts. Please try again in 30 minutes.」
- 账号被冻结 → 提示「Your account has been suspended. Please contact support@looply.com for assistance.」

#### 账号枚举防护（硬性）

本流程中，「邮箱已注册」与「邮箱未注册」在用户端必须完全无法区分：

| 环节 | 要求 |
|------|------|
| 发送验证码 | 不论该邮箱是否已有账号，一律照常发送；提示文案、页面表现一致 |
| 注册限频判断 | 放在**验证码校验通过之后**（步骤 10），不得前移到发码之前 |
| 锁定 / 冻结判断 | 同样放在验证码校验通过之后（步骤 9），不得前移到发码之前 |
| 提示文案 | 两种情况共用同一套文案，不得出现「该邮箱尚未注册」一类的区分性提示 |

**说明**：注册限频若前移到发码前，只有「邮箱不存在」的请求会被拦截；锁定判断若前移，只有「已锁定账号」的请求会被拦截。两者都会让不同类型的邮箱在发码环节表现出差异，等同于对外暴露某个邮箱是否在 Looply 注册过、是否处于锁定态。代价是用户在极端情况下会多完成一次验证码校验才被拦下，本期接受该代价。

现有邮箱密码登录（3.3）采用同一口径——不论邮箱是否存在，均提示「Incorrect email or password」。本流程与之保持一致。

#### 新建账号的初始状态

| 项目 | 取值 |
|------|------|
| 邮箱 | 用户本次已完成验证的邮箱，标记为**已验证** |
| 密码 | **无**。用户如需密码，自行在账号设置中设置；本期不弹引导、不强制 |
| 协议同意记录 | 写入 ConsentRecord，记录同意时间 + 当时生效的 ToS 版本号和 Privacy Policy 版本号（与邮箱注册一致，参见 2.2 步骤 12） |
| 注册来源 | 记录为验证码登录建号，用于后续区分账号来源 |

#### 校验规则

| 字段 | 规则 | 校验时机 |
|------|------|----------|
| 邮箱 | 符合邮箱格式（遵循 RFC 5321 标准，最大长度 254 字符） | 失焦时前端校验 + 提交时后端校验 |
| 验证码 | 6 位纯数字，且与该邮箱当前有效验证码一致 | 填满 6 位后主按钮可点击；提交时后端校验 |

#### 交互说明

- 登录页每次进入均默认展示本方式，**不记忆用户上次选择的登录方式**
- 点击切换链接切至密码登录（3.3），页面已填写的邮箱保留
- 验证码输入框的交互（自动聚焦、输入后自动跳下一位、支持粘贴 6 位自动填充、退格回退上一位）与 2.8 一致
- 关闭弹窗后回到登录页，且仍为验证码方式；已输入的验证码不保留，重新弹出时输入框为空
...
| 状态 | 说明 | 设计稿（PC） | 设计稿（APP） |
|------|------|-------------|-------------|
| 登录页-验证码方式（默认） | 空表单 | Login / Code | 待补 |
| 弹窗-未输入（冷却中） | 验证码框为空、第 1 位聚焦；重发行展示带倒计时的不可点文案 | 验证-未输入 | 待补 |
| 弹窗-已输入（可重发） | 验证码框已填入；重发行为可点链接 | 验证-已输入 | 待补 |
| 弹窗-验证码错误 | 输入框全部错误态；**重发行被错误提示替换** | 验证-错误 | 待补 |
| 登录页-邮箱错误态 | 输入框错误态 + 字段下方 inline 错误提示 | 待补 | 待补 |
| 登录页-加载态 | 主按钮 loading | 待补 | 待补 |

#### UI 关联

- PC 端：登录页 `Login / Code`（nodeId `3120:1992`）；验证码弹窗复用 checkout 已实现组件，状态帧 `36:1072`（未输入）/ `36:1718`（已输入）/ `36:2370`（错误）
- APP 端：待补
...
用户通过已注册的邮箱和密码登录 Looply 平台。**本方式不是登录页默认方式**，用户需从验证码方式（3.2）切换进入。

#### 前置条件

- 用户未登录状态
- 用户已拥有 Looply 账号，且该账号已设置密码

> 通过验证码登录自动新建的账号默认没有密码（参见 3.2）。这类用户在自行设置密码前无法使用本方式登录。
...
- 标题（仅主标题，**不展示副标题**）
- 邮箱输入框
- 密码输入框（带显示/隐藏切换），Forgot password 链接位于密码字段标签行右侧
- Remember me 复选框 + 切换至验证码登录的链接（同一行，两端对齐；点击后切回 3.2）
- 登录主按钮
- 分隔线 + OAuth 三方登录按钮（Google / Apple / Facebook）
- 底部协议提示
- 底部切换至注册页链接
...
### 3.4 OAuth 快捷登录（Google / Apple / Facebook）
...
| 锁定到期 | 自动解锁，错误计数重置 |
| 登录成功 | 错误计数重置 |
| 锁定期间使用**邮箱验证码登录** | 同样拒绝，提示同一句锁定文案。判断放在验证码校验通过之后（参见 3.2 步骤 9） |

> 锁定期间必须同时拦住验证码登录，否则可用密码试错将账号锁定后改走验证码通道进入，锁定失去意义。
> 判断时机放在验证码校验通过之后，是为了不让「已锁定账号」与「不存在的邮箱」在发码环节表现出差异（参见 3.2 账号枚举防护）。
...
| 建号路径 | 是否计入 | 判断时机 |
|----------|----------|----------|
| 邮箱注册（2.2） | 是 | 提交注册信息时（参见 2.2 步骤 6） |
| OAuth 快捷注册（2.3） | 是 | 授权返回后、建号之前 |
| **邮箱验证码登录自动建号（3.2）** | **是** | **验证码校验通过、建号之前**（参见 3.2 步骤 10），不得前移到发码之前 |
...
限制验证码邮件的发送频率，防止邮件轰炸。适用于注册验证码、**登录验证码**和忘记密码验证码三个场景。
...
| 验证码建号 | 邮箱验证码登录时该邮箱无账号，系统自动创建账号 | 登录验证码校验通过、系统判定该邮箱无账号后（参见 3.2 步骤 10） |
...
> **已有账号通过验证码登录**属于常规登录行为，按现有登录记录处理，不写入审计日志。审计日志只记录「验证码建号」这一次性事件。
...
| 登录 | **邮箱验证码登录（默认方式，含自动建号）**、邮箱密码登录、OAuth 快捷登录、忘记密码/重置密码、登录态保持、回访用户登录 |
| 退出 | 主动退出、账号注销（运营代操作） |
| 密码管理 | 密码修改（登录态） |
...
> Figma 文件 Looply-1.1，fileKey `sAOLkw0gIl1bwQRYCDqKuW`。带 nodeId 的为 2026-08-22 邮箱验证码登录本期确认帧；其余为历史帧，沿用原帧名。

| 页面 | PC 端设计稿 | APP 端设计稿 |
|------|------------|------------|
| 登录-验证码方式（默认） | Login / Code（`3120:1992`） | 待补 |
| 验证码弹窗-未输入（冷却中） | 验证-未输入（`36:1072`） | 待补 |
| 验证码弹窗-已输入（可重发） | 验证-已输入（`36:1718`） | 待补 |
| 验证码弹窗-验证码错误 | 验证-错误（`36:2370`） | 待补 |
| 登录-密码方式 | Login / Password（nodeId 待确认，见下方注） | 待补 |
| 登录 | Login | A-Login |
...
> **待确认（2026-08-22）**：密码方式帧的 nodeId 存在两个说法——需求说明标注为 `3121:1921`（经查该 ID 在文件中不存在），实际检索到的同名帧为 `Login / Password` = `3121:805`。以设计侧确认结果为准。
>
> **已作废、不得作为实现依据的帧**：`3121:655`（页内验证码框方案）、`3122:664`（页内倒计时态）、`3120:2569`（独立验证码页）、`3120:2290`（无切换行的旧版 Login / Code）。
...
| V3.1 | 2026-08-22 | **新增**：邮箱验证码登录（3.2）——登录页默认方式；输邮箱→弹出验证码弹窗→填码进站；验证码校验通过后，邮箱已有账号则登录、无账号则自动建号后登录，两种情况用户端表现完全一致。含账号枚举防护硬性要求（发码不区分邮箱是否存在；注册限频与锁定/冻结判断均后置到验证码校验之后）、新建账号初始状态（无密码、邮箱已验证、写入 ConsentRecord、记录注册来源）、弹窗两条展示口径（重发行与错误提示互斥；关闭/改邮箱/返回登录在登录场景下落点相同）。验证码环节采用**弹窗**形态（VerifyCodeModal，覆盖在登录页之上、不跳页），复用 checkout 模块已实现的共享组件，不新建页面；注册与忘记密码仍使用独立页面 VerifyCode。**修改**：登录模块概述（3.1）补充三种登录方式、切换规则（每次进入均默认验证码方式、不记忆上次选择）与「登录页不展示副标题」；邮箱密码登录（3.3）标注为非默认方式、页面元素改为「标题不含副标题」并新增切换至验证码登录的入口、Forgot password 位置改至密码标签行右侧、前置条件补充「账号已设置密码」；协议承接（2.7）明确登录页主按钮同时承担建号动作，源语种协议文案定为 `By continuing, you agree to our Terms and Privacy Policy`（设计稿旧文案 `By signing in, ...` 作废），并补充自动建号同样按 Clickwrap 记录同意；验证码校验（2.8）使用场景由 2 个扩为 3 个并新增承载页面列与形态说明；登录限错（6.4）新增「锁定期间验证码登录同样拒绝」；注册限频（6.5）新增适用范围表，验证码自动建号计入；发信限频（6.6）适用场景由 2 个扩为 3 个；审计日志（6.8）新增「验证码建号」操作类型（7→8 种）；全局页面流转（1.5）新增验证码登录与弹窗路径；MVP 范围（8.1）登录行更新；设计稿索引（9.1）新增 5 项、补充 fileKey、密码方式 nodeId 待确认注与作废帧清单。**章节重新编号**：原 3.2~3.7 顺延为 3.3~3.8 | 产品部 |
