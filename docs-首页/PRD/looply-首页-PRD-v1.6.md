...
> 版本：v1.6 | 日期：2026-07-08 | 端：移动端（主）+ PC 端
> 状态：🔄 迭代中
...
三个维度各自独立，可自由组合（如法国用户 + 英文界面 + 欧元展示是合法状态）：
- **国家 → market**：控制内容投放边界，决定哪些商品/资源位可见
- **语言**：控制文字呈现，走翻译模块统一管理
- **货币**：控制价格数字的符号和汇率换算，不影响内容逻辑

#### 国家（country → market）影响范围

| 页面 | 模块 / 元素 | 约束 |
|------|------------|------|
| 首页 | Banner | 按 `market_id` 拉取资源位，无配置时隐藏 |
| 首页 | Collections | 同上，按 `market_id` 过滤 |
| 首页 | Feed 商品池 | 只推当前 market 下 active 的 listing |
| 首页 | Announcement Bar | 多市场后文案需按 market 配置化，MVP 硬编码无差异 |
| 首页 | Trust Bar | **不受影响**，全市场统一 |
| 首页 | Globe 面板 Country 列表 | 只展示 `market.status = 'running'` 下的 country |
| 商详页（PDP） | 商品可购买状态 | listing 必须属于当前 market 渠道，否则不可购买 |
| 商详页（PDP） | 运费 / 税费说明 | 不同国家税率/运费规则不同（US 单市场暂不涉及） |
| Collections 落地页 | 商品列表 | 过滤当前 market 商品 |
| 搜索结果页 | 商品列表 | 同上 |
| "我的" | 国家/语言/货币设置 | 用户选择的 `country_code` 决定 market，进而影响全站内容 |
| 导航栏（App/PC） | Menu Items 内容 | 按 `market_id` 过滤，不同市场可配不同导航结构 |
| CMS 后台 | 资源位配置 | Banner/Collection 按 market_id 投放，运营可按市场差异化配置 |

#### 语言影响范围

| 页面 | 模块 / 元素 | 约束 |
|------|------------|------|
| 全站 | 所有固定文案（按钮、标签、Section 标题等） | 走 i18n；当前 MVP 全硬编码 en |
| 首页 | 商品卡标题 | `listing.listing_title` 走翻译模块，查不到 fallback en |
| 首页 | Globe 面板 Country 名称 / Language 名称 | 按当前语言展示本地化名称 |
| 首页 | Banner / Collections 文案（title / subtitle / cta_text） | CMS 字段多语言版本按语言返回；MVP 单语言无差异 |
| 首页 | Search 占位词 / Hot Trends | 搜推团队按语言返回词库 |
| 商详页（PDP） | 商品标题、描述、属性选项值、品牌名、系列名、类目名 | 翻译模块统一管理，商品系统只存原始值（商品系统 PRD v1.7 §1.7） |
| Favourites / 浏览历史 | 商品卡标题 | 同首页商品卡，走翻译模块 |
| Collections 落地页 | 合集标题、描述 | CMS 字段多语言，按语言返回 |
| "我的" | 页面所有 UI 文案 | 走 i18n |
| Footer（PC） | 法律条款、隐私政策文字 | 需翻译；EU 市场还有语言合规要求 |
| 搜索结果页 | 搜索建议词、分类标签 | 按语言返回 |
| 导航栏 | Menu Item 名称 | 目前运营直接填英文，多语言阶段需翻译模块介入 |

#### 货币影响范围

| 页面 | 模块 / 元素 | 约束 |
|------|------------|------|
| 首页 | 商品卡 listing_price | 原始货币按用户货币设置换算 + 尾差规则处理后展示 |
| 首页 | 商品卡 market_price（划线价） | 同上 |
| 首页 | 商品卡 Save 标签 | 差值在换算后货币下展示 |
| 商详页（PDP） | 价格展示 | 同首页商品卡；尾差规则按货币级 > Market 级 > 系统默认三层生效（商品系统 PRD v1.7 §尾差规则） |
| 商详页（PDP） | 结算价 / 支付 | 用户以换算后货币支付，非定价货币 |
| Favourites | 商品卡价格 | 同首页商品卡 |
| Favourites | 降价对比（price_at_save） | `price_at_save` 应以原始货币存储并在原始货币内比对，避免汇率波动误触发降价通知 |
| Collections 落地页 | 商品卡价格 | 同首页 |
| 搜索结果页 | 商品卡价格 | 同首页 |
| "我的" | 货币设置 | 用户选择展示货币，全站价格随之换算；货币不影响 market 归属 |
| 订单 / 支付（待设计） | 支付金额、汇率说明 | 以换算后货币结算，需展示汇率来源和时间戳 |

> ⚠️ **翻译模块已设计，对接规范见 §14.2a**：商品系统、CMS 均不自己维护多语言字段，统一由翻译模块管理。首页涉及的翻译对接字段清单已整理，见 §14.2a。
...
### §14.2a 翻译模块对接清单（首页 + 导航栏 + CMS 配置）

> V1.0 上线（美国英语单市场）全部内容英文硬编码，**无需实际翻译**。本节为预对接需求：将字段清单和 resource_type 命名提前给翻译侧注册，等多语言市场启动时按规范写入，不用临时补设计。

#### 需要提给翻译侧的（动态配置内容，有 CMS 管理）

| 内容 | resource_type | field_name | 说明 |
|------|--------------|------------|------|
| Banner 标题 | `cms_home_banner` | `title` | 运营在 CMS 填写，每条配置独立 |
...
| 首页 SEO Meta Title | `cms_home_seo` | `meta_title` | 首页级别，唯一一条 |
| 首页 SEO Meta Description | `cms_home_seo` | `meta_desc` | 同上 |
| 导航 L1/L2/L3 标签名 | `cms_nav_item` | `name` | 运营在导航栏 CMS 填写 |

> 翻译模块 PRD §3.2 将「Banner / 运营配置」标注为「模块未设计」，以上 resource_type 命名需与翻译侧确认后在 `translatable_field_config` 注册。

#### 不需要提给翻译侧的（静态文案或不翻译内容）

| 内容 | 类型 | 说明 |
|------|------|------|
| Trust Bar 4 条文案（Authenticated / Free Shipping / Easy Returns / Top Brands） | 静态文案 | 硬编码，随版本发布，走前端语言包即可，无需翻译模块介入 |
| Feed Tab 标签（For You / New Arrivals） | 静态文案 | 同上 |
| Search 占位文案 | 静态文案 | 同上 |
| 首页 Section 标题（Curated Collections / Explore Finds 等） | 静态文案 | 同上 |
| 品牌名（Chanel、Louis Vuitton 等） | 不翻译 | 翻译模块已明确：brand_name 永远英文 |
| 商品标题（listing_title） | 不归首页 | 商品模块负责对接翻译服务，首页直接消费翻译结果 |
| 价格、货币格式 | 不翻译 | 前端按 locale 格式化，非翻译内容 |
| OG 分享图 | 不翻译 | 图片不区分语言，各语言共用 |
...
| 编号 | 章节 | 变更内容 | 原因 |
|------|------|---------|------|
| C42 | §2.5 语言影响范围 | 底部 ⚠️ 提示从「翻译模块尚未设计」改为「已设计，对接规范见 §14.2a」 | 翻译模块 PRD v3.1 已完成 |
| C43 | §14.2a（新增） | 新增首页 + 导航栏 + CMS 配置的翻译模块对接清单，分两表：需提给翻译侧的动态配置字段（含 resource_type / field_name）；不需要提给翻译侧的静态文案和不翻译内容 | 明确翻译对接边界，供翻译侧预注册 resource_type |
...
| C41 | §2.5（新增） | 新增全产品国家/语言/货币影响范围总表，覆盖首页、商详、Favourites、Collections 落地页、搜索、"我的"、导航栏、CMS 后台；标注翻译模块为未设计空缺；Favourites price_at_save 应在原始货币比对的特殊说明 | 三个维度横跨全站，集中说明便于研发和设计提前预留，避免各页面 PRD 重复描述 |
