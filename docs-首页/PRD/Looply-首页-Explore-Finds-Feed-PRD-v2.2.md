...
| 商品曝光 | 不同 Market 独立统计 |
| 点击数据 | 不同 Market 独立统计 |
| 收藏数据 | 行为必须记录来源 market_id + channel_id |
| 购买数据 | 按 market_id + channel_id 记录来源 |
| 热门统计 | 按 market_id + channel_id 独立统计 |
| 用户兴趣分 | interest_score_24h 按 user_key + market_id + channel_id 计算 |
| 推荐训练 | 后续默认按 Market 隔离，不跨 Market 共享训练数据 |
...
base_pool
+ listed_at 不为空
+ listed_at >= 当前时间 - b_freshness_days（默认 30 天）
order by listed_at desc, listing_price desc, listing_id asc
```

排序说明：
- 第一排序键：listed_at desc（最新上架优先）
- 第二排序键：listing_price desc（同时间上架时，高价商品优先，提升 Feed 商品价值感）
- 第三排序键：listing_id asc（稳定的兜底排序键，避免分页时结果跳动）
...
在配额选择得到各通路候选商品之后、打散（§12.14）之前，使用加权轮询（weighted round-robin）分配最终展示位置。
...
**打散（§12.14）在混排后执行**，可能调整少量相邻位置以满足品牌/系列/类目多样性约束，不改变通路间整体比例。
