# Looply 首页 Explore Finds Feed PRD v2.4
...
本节为基于 v2.3 的增量修改；本节未涉及的规则保持不变。

### 25.1 恢复 Sale Tab

1. 1.1 版本未实现并移除的 `Deals` Tab 在下一期恢复，用户端名称统一使用 `Sale`。
2. `Sale` 对应原 `Deals` 能力，不新增第五个 Tab。
3. Feed Tab 顺序为：`For You`、`New Arrivals`、`Best Sellers`、`Sale`。
4. `Sale` 展示营销或价格服务返回的有效促销、降价商品；Feed 不自行生成折扣状态。
5. 商品不再满足促销或降价条件后，不进入 `Sale` 商品池。
6. `Sale` 复用其他 Feed Tab 的商品卡、分页加载、空态、失败重试和埋点规则，Tab 标识使用稳定值 `sale`。
