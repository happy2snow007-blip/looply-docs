# Looply 首页 Explore Finds Feed PRD v1.1
...
↓
判断 user_key 是否存在
→ 不存在：直接设 user_state = S0，跳过 A1/A2 通路
→ 存在：继续判断 user_state
↓
...
↓
构建 base_pool（仅含 listing_status = active 的在售商品）
↓
读取当前 Session 已展示 listing 列表，作为所有通路的召回排除集
↓
A1：当前 Session 点击 + 收藏基准，相似召回 Top 200
    （召回空间限定在 base_pool，排除已展示集）
↓
A2：24h 点击 + 收藏基准，剔除当前 Session 行为商品后，相似召回 Top 300
    （召回空间限定在 base_pool，排除已展示集）
↓
B：New Arrivals 商品池召回 Top 500
    （仅含 listing_status = active，排除已展示集）
↓
...
↓
执行过滤：24h 曝光频控、间隔去重
（不可售与已展示已在召回阶段排除；此处保留兜底，防召回后状态变化）
↓
...
1. A1/A2/B/C 跨通路去重；
2. 24h 曝光频控；
3. 间隔 X 个商品内不重复推荐同一 listing；
4. 同品牌、同系列、同类目、同价格带打散；
5. 不可售、无图、价格无效商品（已在召回阶段过滤，此处为安全兜底）；
6. 某通路基准不足或候选不足。
...
1. **user_key 判断前移**：在判断 user_state 之前，增加"user_key 是否存在"的显式分支。user_key 不存在时直接设 user_state = S0 并跳过 A1/A2 通路，避免进入后续兴趣判断逻辑。原流程仅凭 S0 配额隐式处理此场景，逻辑不清晰。

2. **在售过滤前移至召回阶段**：base_pool 构建时限定 `listing_status = active`；B/C 通路召回明确标注"仅含在售商品"；A1/A2 相似召回空间同步限定在 base_pool 内。末尾保留不可售兜底过滤，防止召回到混排期间状态变化漏出。原流程将不可售过滤统一放在合并后执行，导致无效候选占用召回配额。

3. **已展示集前移至召回阶段**：请求进入时读取当前 Session 已展示 listing 列表，作为排除集传入所有通路的召回查询。原流程在合并后统一过滤，B/C 通路会召回大量已展示商品浪费候选配额。

4. **召回损耗说明同步更新**：已展示过滤和不可售过滤移至召回阶段，损耗列表中调整为安全兜底注释。
