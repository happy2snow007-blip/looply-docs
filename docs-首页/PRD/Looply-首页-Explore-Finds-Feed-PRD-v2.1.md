...
| 项目 | 规则 |
|---|---|
| 市场 Market | Feed 数据必须基于当前 Market 获取 |
| Market 来源优先级 | 用户主动选择 Market > 用户账户默认 Market > 浏览器 / IP 推断 Market > 系统默认 Market |
| Channel 来源 | 当前 Market 对应默认 Channel |
| 商品池 | 不同 Market 使用不同 Channel 下的 listing 池 |
| 语言 | 优先使用当前 Market 默认语言；登录用户可覆盖语言设置 |
| 翻译策略 | 商品系统存储原始值，展示文案通过翻译模块获取；无翻译时 fallback 默认语言 |
| 货币 | 按当前 Market 对应 currency_code 展示 |
| 时区 | Feed 中涉及时间排序、统计周期时按当前 Market 时区计算 |
| 推荐配置 | 推荐配置支持按 Market / Channel 配置，未配置时使用系统默认值 |
...
│  └─ 使用账户默认 Market
├─ 未登录
│  └─ 根据 IP / 浏览器语言推断 Market
└─ 无法识别
   └─ 使用系统默认 Market（MVP 为 US）
```

### 5.2 不同 Market Feed 差异

| 维度 | 规则 |
|---|---|
| 商品池 | 每个 Market 使用自己对应 Channel 下的可售 listing |
| 行为统计 | 点击、收藏、购买、曝光等行为按 Market + Channel 分开统计 |
| 推荐结果 | For You / New Arrivals / Best Sellers 均只返回当前 Market + Channel 下的商品 |
| 配置覆盖 | 同一配置项可支持 Market / Channel 级覆盖 |
| 默认配置 | 未配置 Market / Channel 专属值时，使用系统默认配置 |
...
| 商品曝光 | 不同 Market 独立统计 |
| 点击数据 | 不同 Market 独立统计 |
| 收藏数据 | 用户维度可共享，但行为必须记录来源 Market |
| 购买数据 | 按 Market + Channel 记录来源 |
| 热门统计 | 按 Market + Channel 独立统计 |
| 用户状态 | interest_score_24h 按 user_key + Market + Channel 计算 |
| 推荐配置 | 支持系统默认值，也支持 Market / Channel 覆盖 |
| 推荐训练 | 后续默认按 Market 隔离 |
...
| 加载状态 | Skeleton / Loading More | 首屏和分页加载分别处理 |
...
| purchase_complete | 购买完成 | 是 | 用于用户状态分、Best Sellers 购买热度 |
...
| Best Sellers | 热卖 / 热门 | 可延后或使用 C 通路逻辑 | 使用 C 通路热门逻辑，不直接按 listing 销量排序 |
| Deals | 折扣 | 可隐藏 | 依赖促销价、降价记录、营销活动 |
...
|---|---|
| 已登录且有行为数据 | For You |
| 已登录但无行为数据 | For You，使用 B/C 冷启动 |
| 未登录 | For You，使用 anonymous_user_id 级行为 + B/C 冷启动 |
| 推荐服务异常 | New Arrivals 或基础商品池兜底 |
...
系统从 recommend_listing_feature_snapshot 读取基准商品特征：
...
B 通路在 New Arrivals Tab 中按 listed_at 倒序展示。
...
↓
读取 / 计算 recommend_user_interest_24h
↓
...
打散规则在配额选择后执行，用于避免 Feed 连续出现过于相似的商品。

| 维度 | 默认规则 | 说明 | 是否支持配置 |
|---|---|---|---:|
| 品牌 | 同 brand_id 连续最多 2 个 | 避免同品牌刷屏 | 是 |
| 系列 | 同 series_id 连续最多 1 个 | 系列为空不参与限制 | 是 |
| 后台类目 | 同 backend_category_id 连续最多 3 个 | 避免单类目刷屏 | 是 |
| 价格带 | 同 price_band 连续最多 4 个 | 保持价格层次多样性 | 是 |

打散失败处理：

1. 从当前通路后续候选中寻找满足规则的商品补位；
2. 当前通路无法补位时，从其他通路候选中按配额释放规则补位；
3. 全部无法补位时，进入无限流兜底逻辑；
4. 不因打散规则返回不可售或已展示商品。
...
1. 继续向更早 listed_at 的商品分页；
2. 过滤已展示商品；
3. 若仍不足，使用基础商品池按 listed_at desc + listing_id asc 补足；
...
| purchase_complete | 购买完成 | 订单完成支付 | user_id、anonymous_user_id、listing_id、product_id、spu_id、order_id | 用户状态分、Best Sellers 购买热度 |
...
### 20.1 recommend_behavior_event_log
...
### 20.2 recommend_listing_feature_snapshot
...
### 20.4 recommend_user_interest_24h
...
### 20.7 recommend_exposure_control
...
| For You Tab | 是 |
| New Arrivals Tab | 是 |
| Best Sellers Tab | 可延后 / 可使用 C 通路逻辑 |
| Deals Tab | 可隐藏 |
| 商品卡基础展示 | 是 |
| 收藏入口 | 是 |
| Feed 曝光 / 点击 / 收藏埋点 | 是 |
