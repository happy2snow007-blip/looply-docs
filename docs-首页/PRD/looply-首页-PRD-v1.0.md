...
> 版本：v1.0 | 日期：2026-06-17 | 端：移动端（主）+ PC 端
> 状态：🔄 迭代中
...
```
首页
├── 搜索展开卡 ──→ 搜索结果页（点击搜索词 / 提交搜索）
├── Banner CTA ──→ 活动落地页 / 品类页（landing_page 字段决定）
├── Collections 卡片 ──→ 合集落地页
├── Feed 商品卡 ──→ 商品详情页
├── Header 收藏图标 ──→ Favorites 页
├── Header 购物车图标 ──→ 购物车页
...
**MVP 限制**：MVP 阶段 UI 界面固定为英文，语言识别结果不影响页面文案（无 i18n）。语言选项仅用于：在 Market & Language 面板中回显用户当前语言、以及确定 Market 候选列表的展示语言。
...
**面板形态**：
- 移动端：从屏幕底部向上滑出（bottom sheet），背景遮罩
...
└──────────────────────────────────────────────┘
  ...
  18px       h:36px 圆角18px 灰色背景   24×24px 各
  Playfair   占位词每3s淡入淡出轮播     间距8px
  Italic
  点击→首页   点击→搜索展开态          ♡→Favorites
                                       🛒→购物车(徽标)
                                       🌐→Market面板
```
...
| Favourite 图标（♡） | 24×24px，outline 心形 | 点击跳转 Favorites 页；已登录时如有未读收藏动态可加小红点 |
| Cart 图标（🛒） | 24×24px，购物袋形 | 点击跳转购物车；Cart 内有商品时右上角显示数量徽标（最大 99+） |
...
```
┌────────────────────────────────────────────────────────┐  h: 40px
│           Free Shipping on Orders Over $500 | Auth Guaranteed     │  ← Announcement Bar (#6432FC 紫)
├────────────────────────────────────────────────────────┤  h: 72px
│ LOOPLY Handbags Shoes Jewelry Watches Brands  [🔍 Search...] ♡ 👤 🛒 🌐│  ← Navbar (白底)
└────────────────────────────────────────────────────────┘
  ↑navLeft (Logo + 6个NavLinks)    ↑searchBar(320px) ↑navRight(4图标)
                                   胶囊形，点击展开  hover: 用户→下拉
                                   搜索卡片
```
...
- **高度**：40px
- **背景色**：`#6432FC`（品牌紫）
- **内容**：文字公告，MVP 一期前端硬编码文案（文案由运营提供，暂用：`Free Shipping on Orders Over $500 | Authenticity Guaranteed`）
- **文字**：14px，白色，居中
- **可关闭**：MVP 不做关闭按钮，始终展示
...
Logo + NavLinks (320px flex)  Search（320px center）          Heart + User + Cart + Globe
```

**navLeft（左栏）**：
- Logo：Playfair Display Italic，20px，`LOOPLY`，点击返回首页
- NavLinks（Logo 右侧横排）：`Handbags | Shoes | Jewelry | Watches | Accessories | Brands`；字号 14px，Regular；hover 下划线；点击跳转对应品类页

**searchBar（中栏）**：
- 宽度：320px，胶囊形
- 样式与移动端 Search Pill 相同（背景灰，圆角，搜索图标 + 占位词）
- 点击进入搜索展开态（下拉卡片，不跳页）
- 占位词轮播逻辑与移动端一致

**navRight（右栏，从左到右）**：
- 收藏（outline heart）：跳转 Favorites 页
- 用户（outline person）：已登录 → 个人中心菜单（hover 下拉）；未登录 → 点击跳转登录页
- 购物车（outline shopping bag）：跳转购物车；有商品时显示数量徽标
- Globe（Market & Language）：点击展开下拉浮层（见 §2.3）
...
展开卡片从 Search Pill 正下方向下滑出（移动端 bottom sheet 浮于内容上方；PC 端为下拉卡片，宽度与搜索框对齐）。
...
Banner 是首页最顶部的视觉焦点，内容由 CMS 系统控制（见 CMS PRD v2.5.1）。首页只负责展示，不感知运营规则。
...
| `landing_page` | URL | 点击跳转地址，由 CMS 后台自动生成 |
...
- **尺寸**：full-width，高度 600px
- 其余规则同移动端，CTA 按钮更大（padding 更宽）
...
| 图片加载失败 | 显示品牌色填充背景（`#6432FC`），隐藏文字 |
| 视频加载失败 | 自动降级展示 `asset_image` |
...
Explore Finds
┌────────────────────────────────────────┐
│  [■ For You ■]  [New Arrivals]  [Best Sellers]   │  ← 胶囊式 Tab
│   激活：深色背景   未激活：透明+边框   未激活   │
└────────────────────────────────────────┘
...
New Arrivals ────► 最近30天新上架 (B通路)
Best Sellers ────► 热卖榜 (C通路 hot_score)
```
...
| New Arrivals | `new_arrivals` | ✅ 展示 | 最近 30 天新上架商品（B 通路） |
| Best Sellers | `best_sellers` | ✅ 展示 | 热卖榜（C 通路 hot_score） |
| Deals | `deals` | ❌ 隐藏 | 营销系统未上线，MVP 不展示 |

**Tab 顺序**：固定为 For You → New Arrivals → Best Sellers，后续版本配置化。
...
- 首屏加载：约 20 个商品（每列 10 行）
- 滚动至距底部约 200px 时：触发加载下一页（20 个）
- 加载中：底部显示 loading spinner
- 所有商品加载完毕：底部显示"You've seen it all"文案（继续滚动时循环加载新内容，见 Feed PRD）
- 加载失败：底部显示 Retry 按钮
...
├───────────────────────────────────────────────────────────┤
│  Explore Finds                                             │
│  [■ For You ■] [New Arrivals] [Best Sellers]               │
│  ┌──────┐┌──────┐┌──────┐┌──────┐  (4列网格)               │
│  │      ││      ││      ││      │                          │
│  └──────┘└──────┘└──────┘└──────┘                         │
│  ... （无限滚动）                                            │
├───────────────────────────────────────────────────────────┤
│  [ Footer - 深色背景 ]                                      │
│  About | Support | Sell | Social          © 2026 Looply   │
└───────────────────────────────────────────────────────────┘
```
...
| 列 | 内容 |
|----|------|
| 关于 | About Looply / How It Works / Authenticity |
| 支持 | Help Center / Contact Us / Shipping & Returns |
| 出售 | Sell With Us（C1 未开放时跳转 waitlist 或隐藏） |
| 关注我们 | Instagram / Facebook / Pinterest 图标 |
...
| Banner（home_banner） | 图片加载失败 | 显示品牌色背景块 |
...
- Search 展开卡（Recent Searches + Hot Trends 硬编码）
- home_banner + home_collection CMS 资源位
- Trust Bar 硬编码（待运营文案）
- Feed 三 Tab（For You / New Arrivals / Best Sellers），Deals 隐藏
- 商品卡片（无促销价）
...
*文档维护：Looply 产品团队 | 首页 PRD v1.0 | 2026-06-17*
