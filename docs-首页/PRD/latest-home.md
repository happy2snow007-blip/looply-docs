...
> 版本：v1.1 | 日期：2026-06-17 | 端：移动端（主）+ PC 端
> 状态：🔄 迭代中
...
- **高度**：40px
- **背景色**：`#6432FC`（品牌紫）
- **内容**：文字公告，MVP 一期前端硬编码文案（文案由运营提供，此处文案示例，正式文案由运营提供）
- **文字**：14px，白色，居中
- **可关闭**：MVP 不做关闭按钮，始终展示
...
Logo + NavLinks (320px flex)  Search（320px center）          Heart + Cart + Globe + User
```
...
- 所有商品加载完毕：底部显示"You've seen it all"文案；后续滚动行为按 Feed PRD 推荐流规则执行（循环加载 / 扩大召回池）
- 加载失败：底部显示 Retry 按钮
...
> ℹ️ MVP 阶段 Footer 各链接仅展示文字，暂无跳转目标（相关页面待设计）。
...
*文档维护：Looply 产品团队 | 首页 PRD v1.1 | 2026-06-17*
