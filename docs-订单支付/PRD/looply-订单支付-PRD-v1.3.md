...
| 弃单 | 用户进入结算页但未完成支付的会话 |
| 结算进度 | 买家在一个结算会话里最远走到的环节（进入结算页 → 已填邮箱 → 已填收货地址 → 已选配送方式 → 已填支付信息 → 已提交支付），只进不退，用于定位流失环节。**系统自动带出的内容不推进进度**，只由买家的实际动作推进 |
...
- 🔴 **系统自动带出的内容不推进进度**：已登录买家进页面时邮箱与地址已自动带出，此时进度仍是 `entered`，直到买家产生实际动作（修改地址 / 选配送 / 填支付信息 / 提交支付）才往前走。**若自动带出也算「已填」，每个已登录买家一进页面进度就是 `address_filled`，哪怕他连看都没看就关掉了——「流失环节」这个指标对老客群体会整体失真**
- **可跳跃**：进度按买家的实际动作推进，中间值不必逐个经过
...
> **为什么要记来源**：已登录买家进结算页时，邮箱和收货地址**大多是系统自动带出来的**，他本人一个字都没填。不区分来源，后台看到的是「邮箱有了、地址也全了」，会误判成「买家填完了才走的」。**后台必须能区分这些字段是系统带的还是买家自己填的**，具体判定与存储方式由技术方案定。
