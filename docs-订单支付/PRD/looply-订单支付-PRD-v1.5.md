# looply 订单与支付模块 PRD v1.5

## 一、概述

### 1.1 背景与目标

looply 是一个面向多个国家／地区运营的二手电商平台，采用 B2C 模式（三方商家 + 自营商家入驻）。当前已运行美国站，本版本记录中国香港站的订单支付闭环。本 PRD 覆盖买家从结算到支付、以及后台订单与支付管理的完整交易闭环。

经系统代码确认，订单与支付单的 Market／币种、下单、支付、回调、查单、退款、对账及历史交易处理均为现有可用能力。V1.5 的唯一开发增量是：**支持按 Market（站点／地区）差异化配置支付方式**。完整流程继续在本 PRD 中归档，但不作为本次改造范围。
...
- 覆盖交易核心节点的买家邮件通知
- 复用现有 Market、交易国家／地区、币种及原路退款能力；美国站使用 USD，中国香港站使用 HKD
- 新增 Market 与支付方式的收单路由，让不同 Market 使用不同的支付方式组合
...
| 结算实时换汇 | 本期按站点使用固定交易币种，不在订单支付链路换汇 |
| 混合币种订单 | 同一结算与订单只允许一个币种 |
| 香港新支付渠道／本地钱包 | 本期只验证并开放现有支付方式，不新增支付方式枚举 |
| 跨站迁移购物车 | 切站后重新创建结算，不迁移原结算会话 |
...
7. **香港站交易**：香港站商品以 HKD 进入结算 → 根据 HK Market 收单路由展示支付方式 → 后续支付、退款与对账沿用现有流程
...
| 弃单 | 用户进入结算页但未完成支付的会话 |
| 结算进度 | 买家在一个结算会话里最远走到的环节（进入结算页 → 已填邮箱 → 已填收货地址 → 已选配送方式 → 已填支付信息 → 已提交支付），只进不退，用于定位流失环节。**系统自动带出的内容算「已填」**，与买家手填一视同仁 |
| 优惠码尝试记录 | 买家在结算页每提交一次优惠码校验产生的一条记录，含码值、来源、通过与否、未通过原因。同一会话可有多条 |
| 地址填写状态 | 结算会话中收货地址 / 账单地址的填写完整度（未填写 / 部分填写 / 已填完整） |
| 交易站点 | 买家创建结算时已确认的 Market 与国家／地区；下单后不可改变 |
| 交易币种 | 本次结算、订单、支付与退款共同使用的 ISO 4217 币种 |
| 支付能力 | 某 Market 在指定币种、支付通道和终端下实际可使用的支付方式 |
| 交易归属冻结 | 结算创建后保留站点与币种，后续不因切站、IP、地址簿或用户资料变化而改变 |

### 1.7 多语言 / 多国家策略

本模块为混合模块：C 端结算页、订单成功页和交易邮件接入多语言；运营后台本期保持中文。静态 UI 文案沿用前端语言包，运营可在多语言后台查看或修改；订单号、金额、物流单号、邮箱和时间等结构化数据不翻译。动态商品内容由商品模块接入，订单模块按用户语言读取，不重复建卡。

| Market | 国家／地区 | 首期交易币种 | 语言与内容 | 交易要求 |
|---|---|---|---|---|
| 美国站 | US | USD | 沿用英语、西班牙语和英文源文本规则 | 保持 V1.4 行为 |
| 中国香港站 | HK | HKD | 使用平台已支持的语言包和翻译服务；缺少译文时按统一 Fallback 返回英文源文本 | 商品、优惠、运费、税费、支付与退款全部使用 HKD |

结算会话从当前 Market 获取国家／地区、渠道和默认币种。语言切换只改变展示内容，不改变交易站点与币种。邮件按用户语言偏好渲染，但其中的订单链接、金额和币种读取原订单交易归属。

邮件模板继续使用 `notif-email` 资源卡，已有字段清单和建卡信息沿用多语言模块登记。本版本不新增订单支付自有动态翻译字段；香港站文案补充进入现有 `checkout`、`orderSuccess` 语言包和 `notif-email` 邮件卡，不拆新卡。Fallback 统一为：有有效译文返回译文 → 有过期译文返回旧译文 → 无记录返回英文源文本。
...
- **只进不退**：买家回头修改前面的信息不会让进度回落
- **自动带出算「已填」**（2026-08-31 产品拍板）：已登录买家进页面时邮箱与地址自动带出，进度直接按带出的内容推进，与买家手填一视同仁
- **可跳跃**：中间值不必逐个经过——已登录买家进页面即可从 `entered` 跳到 `address_filled`

> ⚠️ **看这个指标要配合时间线**：自动带出算已填，意味着已登录买家一进页面进度通常就是 `address_filled`，哪怕他什么都没动。**「进度 = 已填收货地址」不等于「买家填过地址」**——要判断买家有没有真的动手，看时间线里是「自动带入」还是「填写 / 修改」。
...
### 2.5 多站交易归属与金额快照（现有能力）

以下规则经系统代码确认已经支持，作为完整业务规则归档，本期不改造。

| 规则 | 说明 |
|---|---|
| 结算归属 | 创建结算会话时记录 `market_id、country_code、channel_id、currency_code`，来源是当前已确认的购物站点和商品报价，不从 IP、用户注册国家或收货地址推断 |
| 订单继承 | 父订单从结算会话继承四项；支付成功拆出的子订单继续继承。父订单创建后站点与币种不可修改 |
| 支付继承 | 支付单记录原父订单的 Market、国家／地区和币种；技术侧支付接入配置解析实际商户账号与凭证，支付单记录实际账号标识；支付请求金额与币种必须等于支付单 |
| 退款继承 | 退款单从原支付单读取通道、实际商户账号标识和币种，不读取用户当前站点，不自动换汇 |
| 单币种 | 同一结算中的商品、优惠、运费、税费、应付与支付必须同币种；不一致时阻断下单并要求重新结算 |
| 切站影响 | 切站只影响之后新建的结算会话；既有结算、订单、回调、退款和对账继续按原交易归属处理 |
| 历史数据 | 香港站开放前的正常生产交易按“当时仅有美国站”标记为 US／USD；不改金额，不触发支付、退款或通知。异常记录单独待核 |

美国站固定使用 USD，中国香港站固定使用 HKD。本版本不支持实时换汇、混合币种订单或跨站迁移购物车。

#### 支付方式主数据与收单路由

收单配置归订单支付模块，包含全局“支付方式”主数据和按 Market 维护的“收单路由”。支付方式只维护名称、唯一编码、支付通道和状态；收单路由引用支付方式，只维护 Market、状态和展示顺序。币种属于 Market 与交易本身，支付通道属于支付方式主数据，均不在路由展示或维护。商户账号、Client ID／Client Secret、API Key、Webhook 密钥和证书等关键参数由技术侧支付接入配置维护，不进入产品后台。

现有支付方式主数据共四条：PayPal → PayPal 通道；Credit Card、Apple Pay、Klarna → Airwallex 通道。美国 Market 建立并启用四条路由；中国香港 Market 建立并启用 PayPal、Credit Card、Apple Pay 三条路由，不建立 Klarna 路由。同一 Market 与支付方式只能存在一条路由。

配置变化只影响新支付。停用支付方式主数据时，关联路由同步停用；重新启用主数据后，路由仍需单独启用。创建支付单时，技术侧根据支付方式、Market、币种和环境解析实际账号与凭证，并把账号标识写入支付单。历史支付的回调、查单和退款继续读取支付单中记录的原支付方式、通道、实际账号标识和币种。

#### 系统代码确认（2026-09-24）

订单与支付单的 Market／币种数据、交易继承、切站后的历史交易处理，以及相关查询流程均已支持。V1.5 不新增字段、不做历史数据迁移，也不改造交易列表、详情、筛选或导出；本期只在现有支付能力之上增加 Market 收单路由。
...
结算页是买家完成购买的核心页面。用户从商品详情页（Buy Now）或购物车（Checkout）进入，在单页内完成收货信息填写、配送方式选择、支付方式选择，点击下单按钮发起支付。页面创建结算会话时同时冻结 Market、国家／地区、渠道和币种；页面内所有金额按该交易币种展示。
...
- 用户可以是已登录状态或未登录状态
- 当前 Market 已开放新结算，且商品有该 Market 的可售 listing 与同币种报价
- 中国香港站使用 HKD；美国站使用 USD
...
> 本章节仅概述用户侧支付需求。美国站基线见 `looply-支付渠道对接集成说明文档-v1.0.md`，香港站增量验证见 `looply-支付渠道-香港站增量对接要求-v1.0.md`。

#### 前置条件

- 用户已在结算页完成所有必填信息（地址、支付方式）
- 系统已创建结算会话，记录商品和价格快照
- 结算会话已有 Market、国家／地区、渠道和币种，且商品、优惠、运费与税费币种一致
- 所选支付方式已通过当前站点、币种、技术侧支付接入配置和终端能力校验

#### 操作流程

**主流程：**

1. 用户点击带交易币种金额的 Buy Now 按钮（美国显示 USD，香港显示 HKD）
...
5. 向支付通道发起支付；请求金额和币种直接读取支付单，不在支付层换汇
6. 创建支付单前再次校验支付方式资格；不可用时拒绝提交并返回当前可用方式
...
运营人员在后台管理所有订单，包括查看订单列表、查看订单详情、执行发货和取消操作。多站上线后，后台所有订单、支付和退款记录必须按交易 Market 与币种识别，不能依赖金额符号推断。
...
- 右上角「导出」按钮 → 打开导出配置弹窗，见 4.1.3

**筛选/搜索字段（两行布局）：**

6 个只会单独使用的**精确匹配 ID 类**维度并成**一个统一搜索框**（运营不需要先选类型）；需要与日期、状态互相组合的**筛选类**保持独立控件。

| 字段 | 控件类型 | 说明 |
|------|---------|------|
| 统一搜索框 | 文本输入 | **精确匹配**，一个框同时查 6 个维度：订单号 / 父订单号 / 渠道商品ID / HBLitemID / 红布林订单号 / 物流单号。placeholder：`订单号 / 商品ID / 红布林单号 / 物流单号` |
| 买家邮箱 | 文本输入 | 模糊搜索 |
| 收件人 / 地址 | 文本输入 | 模糊搜索。同一关键词同时在收件人姓名与收货地址各字段（地址行 1/2、城市、州/省、邮编）中匹配，任一命中即返回 |
| 卖家ID | 文本输入 | 精确匹配 |
| 子单类型 | 下拉选择 | 子单（默认）/ 父单 / 全部 |
| 取消原因 | 下拉选择 | 全部 / 支付超时 / 买家取消 / 运营取消 / 缺货 |
| 创建时间 | 日期范围选择 | 起止日期 |
| Market | 下拉选择 | 全部 / 美国 / 中国香港；筛选订单创建时冻结的交易站点 |

**关于 HBLitemID**：它是**实物商品上的一个商品属性值**，不是红布林商品库主键 `hbl_info_id`。两者都是纯数字、极易混淆，按 HBLitemID 搜索时不得回退到 `hbl_info_id`。

**为什么这 6 项能并成一个框**：它们都是「手上有一个号，找它属于哪张单」的场景，一次只会用一个；而买家邮箱、收件人/地址、卖家ID、日期、状态是筛选，天然要互相组合，必须独立。若全部平铺会有 11 个控件、占三行，把列表可视区压掉。

> 🔴 **实现约束（开发必读）：不要按格式判断输入的是哪种号，六个维度全查、命中即返回。**
> 这几个维度的取值形态是**重叠**的：订单号是 20 位纯数字，而 USPS 物流单号是 20~22 位纯数字；HBLitemID 是 9~10 位数字，而 DHL 物流单号是 10 位数字。先猜类型再查一个，必然出现「明明有这张单却搜不到」。
> 接受的代价：极小概率同一串数字在两个维度上各命中一张单，列表返回 2 行，运营看订单号即可分清，不作为缺陷处理。六个维度都是精确匹配、都能走索引，开销低于同页的「收件人 / 地址」模糊搜索。
...
| 列名 | 说明 |
|------|------|
| 订单号 | **点击在新标签页打开订单详情**，等宽字体灰色背景药丸样式 |
| 买家 | 下单用户邮箱 |
| 商家 | 所属商家名称 |
| 商品数 | 订单包含的商品件数 |
| 订单金额 | 订单总金额（含运费、税费） |
...
| 付款时间 | 支付成功时间 |
| 操作 | 查看详情（**新标签页打开**）；已付款状态下额外显示"发货"（主按钮）和"发起退款"（危险按钮） |

**「新标签页打开」的四条行为要求**（订单号列与「查看详情」按钮都要满足；实现方式由技术方案定，但用脚本弹窗的做法满足不了）：

| 行为 | 为什么运营需要 |
|------|---------------|
| 直接点击 → 新标签页 | 运营扫列表、逐单发货，当前窗口打开会丢失筛选与滚动位置 |
| Cmd/Ctrl + 点击 → 新标签页 | 批量处理时连点多单，一次开一批 |
| 鼠标中键点击 → 新标签页 | 同上，比按修饰键更快 |
| 右键 →「在新标签页打开」「复制链接地址」 | 把某一单发给同事跟进 |

订单详情页已具备独立地址（形如 `.../transactions/orders/{订单号}`），本项不涉及地址改造。其他跳转（发货、发起退款）保持当前窗口内的弹窗。
...
|------|------|
| 商品图片 | 缩略图（快照） |
| 商品名称 | **两行展示**：第一行品牌（全大写、次级文本样式），第二行商品名称。均取下单冻结快照。**不拼成一个字符串**——拼接后运营复制商品名去搜索会带上品牌搜不到，且与导出里品牌、商品名称分两列的口径不一致。历史订单无品牌时不渲染品牌行、不占位 |
| 成色等级 | 快照 |
| 单价 | 成交单价（快照） |
| 数量 | 二手默认 1 |
| 优惠分摊 | 该商品承担的优惠金额 |
...
**一键复制（本版新增）**：区块标题右侧加「复制地址」按钮，点击把多行文本复制到剪贴板并给成功提示。运营在线下发货，此前需逐字段手抄进快递系统。

| 规则 | 说明 |
|------|------|
| 格式 | 按美国地址书写规范：姓名 / 电话 / 地址行1 / 地址行2 / `城市, 州 邮编` / 国家 |
| 空字段 | 地址行 2 为空时整行不输出，不产生空行 |
| 国家／地区 | 按订单收货国家／地区输出完整展示名，不写死 `United States`；同时保留订单快照中的 ISO 码供系统识别 |
| 留痕 | **不记操作日志**——复制是高频只读动作，记录会淹没订单操作日志。导出才记 |

**账单地址区域**：与收货地址相同的两列布局 + 同样的复制按钮（当收货地址与账单地址不同时展示）。
...
|------|------|
| 优惠类型 | 如"满减" |
| **优惠码** | **本版新增**。展示码值（大写等宽字体，如 `SAVE30`）；自动折扣无码，显示 `—`。一单多码时按码分行展示 |
| 优惠名称 | 如"满 $200 减 $30（跨店）" |
| 优惠金额 | 红色负数显示 |
| 分摊明细 | 各商品分摊金额说明 |

营销 v2.4 已定：**一单可以有多个码同时生效**（前提是作用于不同商品行），单个商品行至多生效一条折扣。C 端不受影响——营销已定码值不透传 C 端，本项只改运营后台。
...
线上已有一版导出（9 列：类型 / 订单号 / 父订单号 / 买家邮箱 / 订单状态 / 币种 / 订单金额 / 商品数 / 下单时间）。本版扩充为 **40 列 = 保留现有 9 列 + 新增 31 列**。

**完整字段清单、退款取数规则、优惠码取值与通用规则，见迭代 PRD V1.4 §三**（`迭代需求/looply-订单支付-迭代PRD-V1.4-20260908.md`）。此处只沉淀不随迭代变化的口径：

| 口径 | 内容 |
|------|------|
| **导出粒度** | **一行一商品**。订单域里运营关心的动作几乎全发生在商品行——发货按件（支持分批）、退款按件（`order_item.refund_status` 是唯一事实源）、红布林建单按件（一单一件）、找货按 HBLitemID 按件 |
| **金额输出** | 商品级金额（优惠分摊、实付小计、本行退款金额）每行都是本行真值；**订单级金额（订单优惠合计、订单金额）只在该订单第 1 行输出，其余行留空**，这样两类都能直接求和 |
| **定位列** | 另给「订单内序号」与「订单商品总数」两列 |
| **范围一致** | 导出结果必须与列表当前筛选条件下看到的数据完全一致。弹窗的「N 单」按订单计数，实际行数为这批订单的商品行数之和，弹窗需写明 |
| **时区** | 一律按**美东时间（US/Eastern）**输出并在表头注明，含夏令时、按美东当地自然日切分 |
| **枚举与金额格式** | 枚举一律导出**中文展示值**（不导出 `paid`/`cancelled` 这类英文枚举名）；金额必须带币种，按对应币种标准小数位输出 |
| **空值** | 一律留空，不填「无」「未知」「-」 |
| **PII 与留痕** | 导出含完整收货地址与联系电话。每次导出记一条操作日志：操作人、时间、筛选条件、导出订单数与行数。权限跟随订单管理页现有权限 |

**导出配置弹窗**（形态与交互复用本模块对账管理的导出弹窗，见 4.6）：

| 配置项 | 控件 | 取值 |
|------|------|------|
| 导出范围 | 单选（必填） | 当前筛选结果（N 单，默认）/ 全部数据（N 单） |
| 订单状态 | 下拉 | 全部状态 / 待付款 / 已付款 / 已发货 / 交易完成 / 已取消，默认跟随列表当前状态 Tab |
| 创建时间 | 日期范围 | 默认跟随列表当前日期筛选 |
| 文件格式 | 单选（必填） | Excel (.xlsx，默认) / CSV (.csv) |

顶部提示："导出当前筛选条件下的订单数据，**按商品逐行展开**，包含买家收货地址、支付与退款信息，用于线下发货与对账"；范围选项下方提示："**按订单计数；一单多件会拆成多行，实际行数多于订单数**"。点击导出 → 弹窗 → 确认 → 切换为成功状态展示文件名与「完成」按钮。
...
| 页面 | 设计稿 |
|------|--------|
| 订单列表 | 后台原型: looply-订单管理后台原型-v25.html — OrderListPage |
| 订单详情 | 后台原型: looply-订单管理后台原型-v25.html — OrderDetailPage |
...
> **来源只体现在事件时间线里，不落会话字段、不进导出**（2026-08-31 产品拍板）：已登录买家的邮箱与收货地址大多是系统自动带出的，时间线用「自动带入」与「填写 / 修改」两类事件区分（见 §4.2.2）。**不新增「来源」字段**——结算进度已按「自动带出算已填」统一口径，导出的地址填写情况同样不区分来源。
...
| 记录项 | 说明 |
|------|------|
| 优惠码码值 | 买家实际提交的字符串，统一转成大写后存储。**不校验、不判定码的真伪，买家提交什么就记什么** |
...
| 场景 | 怎么记 |
|------|------|
| **链接带入的码自动应用成功** | 记 1 条「已应用」，码来源 = 链接带入。买家没有输入动作，但这是一次真实校验 |
| **链接带入的码校验不通过，买家侧完全静默**（营销 §2.3.4：不提示、不回填、页面与「没有这个码」一致）| **照记 1 条「未通过」**。买家看不到 ≠ 没发生——运营排查「投放链接带的码为什么没人用成」查的正是这批 |
| **链接带入的码未达门槛，买家侧只出中性引导**（非报错）| 同样记 1 条「未通过」，原因 = 未达使用门槛 |
...
| 买家身份明确前后得到不同校验结果（营销 T1/T2 两时点） | 各记一条。T1 通过、T2 因限次被拒的，会是「已应用」+「未通过（该买家已用过）」两条；原「已应用」那条同时补记被移除时间 |

> **T2 撤销与买家主动移除共用「被移除时间」一个字段，不新增字段区分**（2026-08-31 产品拍板）。需要分辨时，看该码后面**有没有紧跟一条「未通过」记录**：有 → 系统按 T2 判定撤销；没有 → 买家自己点 ✕ 移除。

**会话级冗余**（供导出与后续统计直接取值，避免每次聚合明细）：当前生效优惠码、当前优惠减免金额、优惠码使用情况（枚举见 §2.2.1）。
...
> **v1.3 对本页的唯一改动就是这个导出按钮**——筛选项与列表列均保持原样。优惠码、地址填写情况、结算进度都是排查用的细节，落点在详情页（§4.2.2）与导出文件（§4.2.3）。
...
> **列表沿用原有 7 列，本次不增列。** 优惠码、地址填写情况、流失环节都是排查用的细节，放进列表会把表格撑到需要横向滚动，而运营在列表这一层要的是「哪些单该跟」，不是「这单为什么没成」——前者靠上方筛选定位，后者进详情看。细节的落点是详情页（§4.2.2）与导出（§4.2.3）。
...
| 单价 | 快照价格 |
| 数量 | — |
| 币种 | 交易币种（美国 USD／中国香港 HKD） |

**事件时间线（优惠码尝试记录并入本区块，不单独成模块）：**

> 优惠码的每一次尝试都在时间线里出一条事件，**不另起一张表**。买家在结算页的动作本来就是一条时间序列——填邮箱、填地址、试码、选配送、付款失败、离开——把试码单独抽成表格，运营要在两个地方来回对时间才能还原发生了什么。
> **数据照记不变**：§4.2.0 的尝试记录该存的字段一个不少，本条只规定后台怎么展示；导出仍按 §4.2.3 输出全部优惠码字段。
...
| 事件 | 事件详情格式 | 颜色 | 触发 |
|------|---------|------|------|
| **自动带入联系方式** | `{邮箱} · 取自登录账户` | 普通-灰色 | 已登录买家进入结算页 |
| **自动带入收货地址** | `{地址摘要} · 取自账户默认地址` | 普通-灰色 | 已登录买家进入结算页 |
| 填写邮箱 | 买家填写的邮箱 | 普通-灰色 | 买家手动填写 |
| **开始填写收货地址** | `已填 {已填字段数}/{必填字段数} 项` | 普通-灰色 | 收货地址首次有任一字段有值 |
| **收货地址填写完成** | `{地址摘要}` | 普通-灰色 | 收货地址首次达到「已填完整」 |
...
> 🔴 **自动带出与买家填写必须是两条不同的事件**：已登录买家的信息大多是系统带的，混记会让运营把「系统填的」看成「买家填的」，误判流失位置。**一眼要能看出这单里哪些是买家自己动的手。**
> 🔴 **地址拆「开始填」与「填完了」两条事件**：只记一条答不了「买家是填了一半走的还是填完了才走的」。只有「开始填写」没有「填写完成」= 卡在填地址这一步。
> **自动带出的地址不出这两条事件**（出「自动带入收货地址」即可）。因此：**进度到了 `address_filled` 但时间线没有「收货地址填写完成」，说明地址是系统带的、买家没碰过。**
> **自动填写的码与手输的码一视同仁**：链接带入的码即使在买家侧静默失败，后台时间线照样出「优惠码校验失败」事件——两侧口径不同是刻意的。

**码来源**取「链接带入」/「买家手输」。三类优惠码事件由 §4.2.0 的尝试记录逐条派生：**一条尝试记录 = 一条时间线事件**；被移除的那条额外派生一条「移除优惠码」事件。
...
| 8 | 商品小计 | 会话内商品金额合计，**不含运费、税费、优惠**。与「预估总金额」配合可拆出金额构成 |
| 9 | 预估总金额 | — |
| 10 | 币种 | ISO 4217 |
| 11 | 运费 | 未选配送方式时为空 |
| 12 | 配送方式 | 未选时为空 |
| 13 | 支付方式 | 买家在 Payment 区块选定的方式（Credit Card / PayPal / Klarna / Apple Pay），**未走到该步骤则为空**。与「结算进度 = 已填支付信息」对应 |
| 14 | 优惠码使用情况 | 未填写 / 填写未通过 / 已应用 |
| 15 | 当前生效优惠码 | 无则为空 |
| 16 | 优惠减免金额 | 无则为 0 |
| 17 | 优惠码尝试次数 | 该会话的尝试记录总条数 |
| 18 | 最后一次尝试的优惠码 | 无则为空 |
| 19 | 最后一次未通过原因 | 最后一次尝试为「未通过」时填；否则为空 |
| 20 | 全部尝试过的优惠码 | 该会话所有尝试的码值去重后用分号连接 |
| 21 | 收货地址填写情况 | 未填写 / 部分填写 / 已填完整 |
| 22 | 收件人 | — |
| 23 | 收件人电话 | — |
| 24 | 收货地址行 1 | — |
| 25 | 收货地址行 2 | — |
...
| 28 | 收货邮编 | — |
| 29 | 收货国家 | — |
| 30 | 账单地址填写情况 | 未填写 / 部分填写 / 已填完整 / 同收货地址 |
| 31 | 账单联系人 | 「同收货地址」时为空 |
| 32 | 账单联系电话 | 同上 |
| 33 | 账单地址行 1 | 同上 |
| 34 | 账单地址行 2 | 同上 |
...
| 37 | 账单邮编 | 同上 |
| 38 | 账单国家 | 同上 |
| 39 | 恢复邮件发送次数 | 累计发送次数，未发送为 0 |
| 40 | 最后一次恢复邮件发送时间 | 未发送为空 |
| 41 | 创建时间 | 进入结算页的时间 |
| 42 | 最后活动时间 | 会话最后一次被更新的时间 |
| 43 | 转化订单号 | 状态为「已转化」时填写对应的买家订单号；否则为空 |
...
| 页面 | 设计稿 |
|------|--------|
| 弃单列表 | 后台原型: looply-订单管理后台原型-v25.html — AbandonedCartPage |
| 弃单详情 | 后台原型: looply-订单管理后台原型-v25.html — AbandonedDetailPage |
| 导出弹窗 | 后台原型: looply-订单管理后台原型-v25.html — AbandonedCartPage 导出弹窗（形态复用 ReconciliationPage）|
...
| 页面 | 设计稿 |
|------|--------|
| 支付单列表 | 后台原型: looply-订单管理后台原型-v25.html — PaymentOrderListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 退款单列表 | 后台原型: looply-订单管理后台原型-v25.html — RefundOrderListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 支付流水列表 | 后台原型: looply-订单管理后台原型-v25.html — PaymentTransactionListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 对账管理列表 | 后台原型: looply-订单管理后台原型-v25.html — ReconciliationPage |
...
经代码确认，订单、支付单、支付流水、退款单和对账的 Market、币种及查询处理均复用现有能力，本期不改造交易列表、详情、筛选或导出。

展示继续遵循现有规则：Market 作为交易维度；订单金额、优惠、支付金额、手续费和退款金额等金额字段直接携带币种，不增加独立币种列；USD 与 HKD 不直接合计为一个金额。

### 4.9 收单配置

**功能类型**：页面型

**V1.5 唯一新增能力**：现有系统已经具备四种支付方式及完整支付处理流程，但不能按 Market 配置不同的可用方式。本节新增 `Market × 支付方式` 收单路由，其他支付流程不变。

支付中心新增“收单配置”菜单，放在支付中心二级菜单最后一项；下设带独立图标的“收单路由”和“支付方式”两个三级菜单，明确缩进、默认展开并分别进入独立页面。

**支付方式页面**展示并维护系统现有全局主数据：支付方式名称、唯一编码、支付通道、状态、更新时间。本期不新增支付方式类型。页面不配置商户账号、Client Secret、API Key、Webhook 密钥或证书。支持新增、编辑、启用和停用；编辑时编码不可修改。停用时二次确认，并同步停用所有关联路由。

PayPal、Credit Card、Apple Pay 和 Klarna 的技术接入账号、凭证保存边界及 Apple Pay 附加条件见 `PRD/looply-支付渠道技术配置参考-v1.0.md`。

**收单路由页面**维护 Market 与支付方式的关系：Market、支付方式、状态、展示顺序和更新时间。页面不展示示例提示条，路由不展示币种、支付通道或商户账号。新增时选择 Market 和已启用且已绑定支付通道的支付方式；同一 Market 与支付方式重复时不允许保存。编辑时 Market 和支付方式不可修改。启用、停用均需二次确认。

| Market | 支付方式 | 通道 | 是否配置路由 |
|---|---|---|---|
| 美国 | PayPal | PayPal | 是 |
| 美国 | Credit Card | Airwallex | 是 |
| 美国 | Apple Pay | Airwallex | 是 |
| 美国 | Klarna | Airwallex | 是 |
| 中国香港 | PayPal | PayPal | 是 |
| 中国香港 | Credit Card | Airwallex | 是 |
| 中国香港 | Apple Pay | Airwallex | 是 |
| 中国香港 | Klarna | Airwallex | 否，不建立路由 |

只有“支付方式主数据已启用”且“对应 Market 收单路由已启用”的方式才可用于新支付。历史订单的回调、查单和退款不受配置停用影响。
...
| 地址联想服务 | 结算页地址自动填充 | 需按交易国家／地区提供地址联想与校验结果；美国沿用现有能力，香港上线前完成验证 |
| 物流模块 | 物流追踪信息 | 需提供物流查件链接和状态更新（发货后） |
| 税费计算服务 | 返回当前交易站点的税费结果 | 税费计算由对应市场规则负责；订单只接收与交易币种一致的结果并冻结 |
| 营销活动模块 | 结算页优惠码的校验与减免计算 | 需提供优惠码的判定结果、失败原因与同币种减免金额；香港优惠返回 HKD，美国优惠不得用于香港结算。订单侧只记录结论、不重新判定 |
| Market 模块 | 交易站点、国家／地区、渠道、默认币种与开关 | 创建结算时提供唯一有效归属；关闭新结算不影响历史订单履约、查单与退款 |
| 支付渠道与财务 | 香港 HKD 支付、退款、商户账号和对账能力 | 至少一种支付方式完成真实 HKD 收款、回调、查单、部分／全额退款与对账验证后才开放香港新结算 |
...
| 站点或币种缺失／串用 | 香港订单可能按 USD 或美国账号收款，后续无法正确退款和对账 | 缺失即阻断下单；创建订单、支付、回调和退款各环节校验原交易归属 |
| 支付方式公开资料与商户实际能力不一致 | 结算页展示后无法收款或退款 | 以实际商户账号和联调结果为准；未完成全链路验证的方法不在香港展示 |
| 切站覆盖历史交易 | 回调、退款或邮件错误使用用户当前站点 | 所有后续动作只读取原结算、订单和支付单快照 |
...
- 后台退款管理（退款申请 + 退款单）
- 美国站 USD 与中国香港站 HKD 的交易归属冻结
- 支付方式按 Market 与实际商户能力展示；退款按原通道、原账号和原币种执行
...
| 更多市场与币种 | 在 US／USD、HK／HKD 基础上按同一交易归属模型继续扩展 |
...
| 文件 | 说明 |
|------|------|
| looply-订单管理后台原型-v25.html | 支付方式主数据含 PayPal、Credit Card、Apple Pay、Klarna；US 四条路由，HK 三条路由且不含 Klarna |
...
| looply-订单模块实体关系图-v9.2.svg | V1.4 全量数据模型基线 |
| looply-订单支付-多国家交易增量实体关系图-v1.0.svg | 全量关系参考；交易归属与支付退款链路为现有能力，V1.5 只新增 Market 收单路由 |
| looply-订单支付-按Market收单配置增量流程图-v1.1.svg | V1.5 收单路由配置；下单、支付和退款标记为既有流程 |
| looply-支付渠道-香港站增量对接要求-v1.0.md | 香港支付方式开放前的验证口径 |
| looply-订单支付-多国家开站支持-迭代PRD-V1.5-20260922.md | 本版本开发增量稿 |
