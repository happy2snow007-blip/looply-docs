...
| 国家 | 输出全称 `United States`，不输出 ISO 码 `US`——快递系统识别全称 |
...
| **枚举与金额格式** | 枚举一律导出**中文展示值**（不导出 `paid`/`cancelled` 这类英文枚举名）；金额按币种标准小数位，USD 保留 2 位 |
| **空值** | 一律留空，不填「无」「未知」「-」 |
...
| 页面 | 设计稿 |
|------|--------|
| 订单列表 | 后台原型: looply-订单管理后台原型-v15.html — OrderListPage |
| 订单详情 | 后台原型: looply-订单管理后台原型-v15.html — OrderDetailPage |
...
| 页面 | 设计稿 |
|------|--------|
| 弃单列表 | 后台原型: looply-订单管理后台原型-v15.html — AbandonedCartPage |
| 弃单详情 | 后台原型: looply-订单管理后台原型-v15.html — AbandonedDetailPage |
| 导出弹窗 | 后台原型: looply-订单管理后台原型-v15.html — AbandonedCartPage 导出弹窗（形态复用 ReconciliationPage）|
...
| 页面 | 设计稿 |
|------|--------|
| 支付单列表 | 后台原型: looply-订单管理后台原型-v15.html — PaymentOrderListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 退款单列表 | 后台原型: looply-订单管理后台原型-v15.html — RefundOrderListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 支付流水列表 | 后台原型: looply-订单管理后台原型-v15.html — PaymentTransactionListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 对账管理列表 | 后台原型: looply-订单管理后台原型-v15.html — ReconciliationPage |
...
| 文件 | 说明 |
|------|------|
| looply-订单管理后台原型-v15.html | 后台 8 个页面（订单管理、弃单管理、支付单管理、支付流水、对账管理、退款单管理） |
...
| looply-订单模块实体关系图-v9.2.svg | 数据模型设计（18 张表，含结算优惠码尝试记录）|
...
| v1.3 | 2026-08-31 | **弃单数据补齐与导出**：① 结算会话新增采集买家邮箱、收货/账单地址完整快照（含填写完整度判定）、结算进度（六值、只进不退）、优惠码尝试记录（每次校验记一条，含码值/来源/结果/未通过原因九值枚举/减免金额/被移除时间）；② 弃单列表新增 4 个筛选项（优惠码、优惠码使用情况、收货地址填写情况、结算进度），**列表沿用原有 7 列不增列**；③ 弃单详情基本信息加结算进度，地址区块由「未填写不显示」改为始终显示 + 空状态，**优惠码尝试并入事件时间线、不单独成模块**；④ 新增弃单数据导出（43 列、一行一会话、含导出留痕与 5 万条上限）；⑤ 口径订正：删除 §1.2 与 §7 中「Discount Code / 优惠券 — 后续迭代，结算页不展示优惠码输入框」——该能力已由营销活动模块 V1.3（2026-08-25）交付；⑥ 明确与营销模块的边界：营销的「码位不落库」不变，订单侧记录的是结算页每次校验的结果，校验逻辑零改动。**【2026-08-31 交付后修订】** 产品 review 两轮收敛展示面（**数据采集与导出口径始终未变**）：① 弃单列表撤销新增的 3 列、详情页取消「优惠码尝试记录」独立表格改为并入事件时间线（原型 v8 → v9）；② 弃单列表再撤销新增的 4 个筛选项——**本页 v1.3 的唯一改动是多一个导出按钮**；并明确平台不判定买家输入的优惠码真伪，提交什么记什么（原型 v9 → v10）；③ 补两类被漏场景——**链接自动带入的码**（自动应用成功 / 买家侧静默失败 / 未达门槛中性引导**三种都记**）与**已登录买家自动带出**（时间线区分「自动带入 / 填写 / 修改」）；并拍板三条口径：T2 撤销**不区分**、自动带出**算已填**、来源**不落库**（原型 v10 → v12）；⑤ 导出补**商品小计**与**支付方式**两列（41 → 43 列），依据为与当前 Shopify 弃单导出逐列比对；④ 时间线**区分「地址填写完成」**——原「填写收货地址」拆为「开始填写收货地址」与「收货地址填写完成」，只有前者没有后者即买家卡在填地址（原型 v12 → v13）|
| v1.4 | 2026-09-09 | **运营后台订单管理优化**（需求来源：运营 2026-09-08）：① **订单导出扩充**——线上原有 9 列扩为 **40 列（保留 9 + 新增 31）**，粒度定为**一行一商品**（订单域的发货 / 退款 / 红布林建单 / 找货全部按件），订单级金额只在该单第 1 行输出以保证求和正确；补收货完整地址、HBLitemID、品牌、商品名称、优惠码、支付通道与支付方式、退款金额与申请日期；时区一律**美东**、枚举导**中文**、金额按币种标准小数位。**退款按两段优先级取数**（订单发起的退款单 → 售后单发起的退款单），取不到但商品退款状态 = 已退款时**必须输出「记录缺失」、不得静默留空**（实测 100 单中 9 单命中，留空会被读成「没退款」导致对账对不上）；② **订单详情页**——优惠信息表新增「优惠码」列、商品名称改为「品牌 + 名称」两行、收货与账单地址加**一键复制**（美国面单格式、国家输出全称）；③ **订单列表页**——新增 5 个查询维度（渠道商品ID / HBLitemID / 红布林订单号 / 物流单号 / 收件人·地址模糊），筛选区把 6 个精确匹配 ID 维度并成**一个统一搜索框**（不按格式猜类型、六个维度全查）+ 独立筛选控件两行；「订单号」列与「查看详情」改为**新标签页打开**并须满足四条浏览器原生行为。**本版不介入 ER 设计**——数据结构由技术方案保证，PRD 只规定取值口径：品牌 / 标题 / 成色 / HBLitemID / 渠道商品ID / 优惠码码值**一律取下单时冻结的快照**，不在展示或导出时回查商品域与营销域。⏳ 遗留待拍板：退款金额是否落到具体商品行（退款申请侧与订单商品行的标识不是同一套编码），不做则降级为订单级合计并在来源列标注 |
