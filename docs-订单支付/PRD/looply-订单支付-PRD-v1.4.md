...
只会单独使用的**精确匹配 ID 类**收进一个「搜索类型下拉 + 关键词输入框」的组合；需要与日期、状态互相组合的**筛选类**保持独立控件。

| 字段 | 控件类型 | 说明 |
|------|---------|------|
| 搜索类型 + 关键词 | 下拉 + 文本输入 | **精确匹配**。下拉可选：订单号（默认）/ 父订单号 / 渠道商品ID / HBLitemID / 红布林订单号 / 物流单号。输入框 placeholder 随类型变化 |
| 买家邮箱 | 文本输入 | 模糊搜索 |
...
**为什么 ID 类可以合并**：这六项都是「手上有一个号，找它属于哪张单」的场景，一次只会用一个；而买家邮箱、收件人/地址、卖家ID、日期、状态是筛选，天然要互相组合，必须独立。若全部平铺会有 11 个控件、占三行，把列表可视区压掉。
...
| 页面 | 设计稿 |
|------|--------|
| 订单列表 | 后台原型: looply-订单管理后台原型-v14.html — OrderListPage |
| 订单详情 | 后台原型: looply-订单管理后台原型-v14.html — OrderDetailPage |
...
| 页面 | 设计稿 |
|------|--------|
| 弃单列表 | 后台原型: looply-订单管理后台原型-v14.html — AbandonedCartPage |
| 弃单详情 | 后台原型: looply-订单管理后台原型-v14.html — AbandonedDetailPage |
| 导出弹窗 | 后台原型: looply-订单管理后台原型-v14.html — AbandonedCartPage 导出弹窗（形态复用 ReconciliationPage）|
...
| 页面 | 设计稿 |
|------|--------|
| 支付单列表 | 后台原型: looply-订单管理后台原型-v14.html — PaymentOrderListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 退款单列表 | 后台原型: looply-订单管理后台原型-v14.html — RefundOrderListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 支付流水列表 | 后台原型: looply-订单管理后台原型-v14.html — PaymentTransactionListPage |
...
| 页面 | 设计稿 |
|------|--------|
| 对账管理列表 | 后台原型: looply-订单管理后台原型-v14.html — ReconciliationPage |
...
| 文件 | 说明 |
|------|------|
| looply-订单管理后台原型-v14.html | 后台 8 个页面（订单管理、弃单管理、支付单管理、支付流水、对账管理、退款单管理） |
