...
| 轨迹节点 | 描述 | 物流服务返回的可读事件描述。 | 服务端本地化/回退结果，前端直接展示。 |
...
3. 物流状态和事件描述直接展示物流服务返回的本地化结果；禁止前端维护 `internal_status → 英文文案` 映射，禁止对平台 tag、subtag 自行翻译。
...
18. 前端代码中没有写死本 PRD 所列固定用户文案；所有固定文案均通过对应 i18n key 获取。
19. 状态和事件描述不在前端二次映射或翻译，日期时间不通过字符串拼接生成。
20. 复制、关闭和 loading spinner 均具有通过语言包生成的可访问辅助名称。
