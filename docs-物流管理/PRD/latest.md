...
- 若同步取消失败需异步重试，旧值持久化到异步任务记录中（含平台、运单号、订阅ID等）
...
| 新 tracking_no 与已有 active 运单冲突 | 返回 `TRACKING_NO_CONFLICT`，提示该运单号已被占用 |
...
| 异常 | internal_status ∈ { AttemptFail, Exception, Expired } 或 tracking_health = abnormal |
...
该区域数据来自 tracking_platform_carriers 表，展示该承运商在各追踪平台的代码映射：
...
| Exception | 当前节点标红 | 顶部显示异常提示 + Contact Support |
| Expired | 当前节点标红 | 顶部显示「Tracking expired, please contact support」|
...
| internal_status | VARCHAR(50) | 物流状态（Pending/InfoReceived/InTransit/OutForDelivery/AvailableForPickup/AttemptFail/Delivered/Exception/Expired） |
...
| status | VARCHAR(20) | 连接状态（normal/abnormal） |
| last_check_at | TIMESTAMP | 最后检测时间 |

#### tracking_platform_carriers 表（平台承运商映射表）
...
| occurred_at | TIMESTAMP | 发生时间（UTC，前台展示时转换为用户本地时区） |
| location | VARCHAR(200) | 地点 |
| message | TEXT | 事件描述（fallback：承运商原始 message → AfterShip subtag_message → tag） |
| tag | VARCHAR(50) | AfterShip tag |
| subtag | VARCHAR(50) | AfterShip subtag |
| created_at | TIMESTAMP | 创建时间 |
...
- API 错误处理规范文档（AfterShip 版 v1.0 + 快递100 版 v1.0）与 PRD 独立维护
- 规范文档更新时，PRD 中引用该规范的章节（2.1.2、2.1.3 等）需同步评审是否需要调整
