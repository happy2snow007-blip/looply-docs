...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "looply-物流管理后台原型-v1.html")
...
MENU = [
    ...
    {"label": "物流追踪", "icon": "target", "children": [
        ...
        {"id": "tracking-platform", "label": "追踪平台配置", "icon": "settings"},
    ]},
]
...
DATA = {
    ...
    "tracking": [
        {"order_id": "ORD-20260508-001", "tracking_no": "1Z999AA10123456784", "carrier": "UPS", "status": "运输中", "last_update": "2026-05-08 16:30", "dest": "New York, NY"},
        {"order_id": "ORD-20260507-003", "tracking_no": "1Z999AA10123456785", "carrier": "UPS", "status": "已签收", "last_update": "2026-05-08 09:15", "dest": "Los Angeles, CA"},
        {"order_id": "ORD-20260506-007", "tracking_no": "9400111899223100012", "carrier": "FedEx", "status": "已发货", "last_update": "2026-05-07 14:00", "dest": "Chicago, IL"},
        {"order_id": "ORD-20260505-012", "tracking_no": "1Z999AA10123456786", "carrier": "UPS", "status": "异常", "last_update": "2026-05-06 11:20", "dest": "Houston, TX"},
    ],
    "status_mapping": [
        {"aftership": "InTransit", "looply": "运输中", "trigger_action": "更新订单状态", "enabled": "启用"},
        {"aftership": "OutForDelivery", "looply": "派送中", "trigger_action": "推送买家通知", "enabled": "启用"},
        {"aftership": "Delivered", "looply": "已签收", "trigger_action": "触发确认收货倒计时", "enabled": "启用"},
        {"aftership": "AttemptFail", "looply": "投递失败", "trigger_action": "通知运营介入", "enabled": "启用"},
        {"aftership": "Exception", "looply": "异常", "trigger_action": "创建异常工单", "enabled": "启用"},
        {"aftership": "Expired", "looply": "超时未更新", "trigger_action": "通知运营跟进", "enabled": "停用"},
    ],
}
...
def page_tracking_list():
    ...
    tb = toolbar_html('<input class="input" placeholder="搜索订单号/运单号"><select><option>全部状态</option><option>运输中</option><option>已签收</option><option>异常</option></select><select><option>全部承运商</option><option>UPS</option><option>FedEx</option></select><button class="btn">查询</button>')
    add_btn = '<button class="btn primary" onclick="showSubPage(\'tracking\',\'create\')">录入运单号</button>'
    head = page_head("运单追踪", "录入运单号并追踪物流轨迹，系统通过 AfterShip 自动获取状态更新", add_btn)
    body = table_html(['订单号', '运单号', '承运商', '状态', '目的地', '最后更新', '操作'], rows)
    return f'''
...
def page_tracking_platform():
    ...
    <div class="card">
      {page_head("追踪平台配置", "配置 AfterShip 物流追踪平台的 API 连接和 Webhook 设置")}
      <div class="form-section">
        <div class="section-title">AfterShip API 配置</div>
        <div class="form-grid">
          <div class="label">平台</div><div class="field">AfterShip</div>
...
def page_status_mapping():
    ...
    for m in DATA["status_mapping"]:
        rows.append([
            f'<code style="background:#F3F4F6;padding:2px 6px;border-radius:4px;font-size:12px">{m["aftership"]}</code>',
            status_tag(m["looply"]),
            m["trigger_action"],
    ...
    <div class="card">
      {page_head("状态映射", "配置 AfterShip 物流状态与 Looply 订单状态的映射关系，以及状态变更时触发的自动操作")}
      <div class="rail-note">AfterShip 通过 Webhook 推送状态变更时，系统根据以下映射规则自动更新订单状态并触发相应操作。</div>
      {table_html(['AfterShip 状态', 'Looply 状态', '触发操作', '启用状态', '操作'], rows)}
    </div>
    </div>'''
