...
| v2.11 | 2026-07-22 | Codex 根据产品确认补充 | 收口 Recommended for You 行为来源、概览与跨页去重、个性化资格和加载更多；Recently Viewed 概览卡与完整页统一为删除浏览记录，并补齐删除补位、同步失败和迟到响应；明确 Price Drop 跳转与缓存失败、Wishlist 延迟更新、匿名操作、主体级缓存隔离、App / H5 Mobile 刷新恢复、完整集合售罄契约和多语言归属；旧子模块 PRD 标记为历史版本。 |
| v2.12 | 2026-07-24 | Codex 根据产品确认补充 | 明确 Wishlist 少量商品、两个模块 10 件预览上限、2.5 张横滑提示、Price Drop View 入口及仅 H5 Mobile 的范围；补齐单模块、双模块与整页空 / 失败状态矩阵，组合 Retry 只重试两个核心模块，模块结果变化后重新判断组合状态；统一空态视觉、恢复 H5 Mobile 全局整页错误组件占位，完成 Sol / high 正式自检和普通版 / 评审版浏览器回归，确定为评审基线。 |
| v2.13 | 2026-07-24 | Codex 根据模拟评审确认补充 | 新建模拟评审收口稿；Recommended 首次请求中不视为隐藏，两个核心模块同时为空或失败时先展示对应合并轻量状态，不进入整页状态；推荐请求完成后由三个模块共同重新判断页面组合状态；从可改变 Wishlist 或浏览历史的页面返回 Favorites 时同步受影响模块并恢复位置；固定 UI 的无障碍文案随页面语言切换并纳入 `en-US / es-US` 验收，具体 key 复用不在 PRD 重复定义；澄清全部售罄不改变 Wishlist / Recently Viewed 的原有入口阈值。 |
