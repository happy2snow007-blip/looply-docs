...
| v2.9 | 2026-07-16 | Codex 根据产品确认收口 | 写入模拟评审的 4 项确认结论：同屏相同 `listing_id` 心形乐观联动及失败统一回滚；Recommended 有缓存时刷新失败保留 Feed 并静默处理；补齐稳定 i18n key、`en-US` / `es-US` 文案与复数规则；Sold Out 未收藏统一为置灰空心并禁用点击。 |
