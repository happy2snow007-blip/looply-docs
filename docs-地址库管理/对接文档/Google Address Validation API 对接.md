...
PO Box、APO/FPO/DPO 及自定义限制不属于 Address Validation 的全局拦截规则。地址保存阶段不做配送限制校验；仅在结算页，按购物车商品所属商家已启用的固定/自定义配送限制规则校验并拦截。详细规则及面向用户的配送提示以《地址库管理 PRD》2.1.8 为唯一来源。
...
| 新增/编辑地址统一频控 | 单账号、IP、设备任一维度；新增与编辑合计 30 次/自然日 | 当日已累计 30 次可提交的保存尝试后，下一次新增或编辑操作展示 reCAPTCHA；Token 经服务端验证通过后，仅继续当前操作 |

频控口径：

- 统计对象为已通过本地硬校验、发起地址保存的新增/编辑尝试；Autocomplete 请求不计入该 30 次。
- 新增与编辑共用同一计数器；账号、IP、设备任一维度达到上限即触发，不能通过切换操作类型绕过。
- reCAPTCHA Token 必须由服务端验证。验证失败、过期或用户取消时，不创建/更新地址，保留表单供用户再次验证；成功只放行当前操作，不重置或豁免后续计数。
- 本章节的频控规则适用于 MVP；PRD 中的产品流程与异常处理以《地址库管理 PRD》2.1.5 为准。
