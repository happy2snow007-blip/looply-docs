...
>
> **本版变更（V2.1）**：按归档口径收口新开站点支持：将香港站表述统一为“新 Market / 新站点通用规则，香港站为首期样例”；补齐翻译 Fallback 只能回退当前 Market 源文；依赖与待补充项从“Shipping & Returns Market 维度”扩展为“CMS PDP 模板 Market 归属”；更新 CMS 后台原型索引为 v5 Market 版，并修正 PC/APP 差异表中的旧品牌色描述。
> **本版变更（V2.0）**：补齐 CMS 配置规则中的 Market 维度：商详页配置调整为“Market → PDP 模板 → 模块配置”，明确现有模板属于 US Market；新 Market 需建立独立模板记录，可从其他 Market 复制初始化；新建、编辑、删除、模块开关、变更记录和前台读取均限定在当前 Market，禁止运行时回退 US。
> **本版变更（V1.9）**：新增香港站商详支持——锁定 HK Market、香港 listing 与 HKD；新增香港不可售状态和购买前二次校验；推荐/浏览历史按香港可售商品过滤。依据现网 CMS 核对结果，纠正 Shipping & Returns“前端固定文案”的旧描述，并在现有第 4 模块增加 Market 维度；HK 内容禁止回退 US。
>
...
商详页**自身产生**、需多语言的业务字段包含 Certified Authentic、Condition 等级体系、成色详细信息展示名、Description 属性展示名，以及已上线的 Shipping & Returns 内容。前四类由商详页 CMS 配置后台按多语言 v3.2 标准**新建 4 张翻译卡片**，统一归入“商详页”聚合组；Shipping & Returns 沿用现网商品翻译模块的既有资源映射，本期只让资源实例增加 Market 归属，不重复建卡。
...
**上游模块产生的内容（第三类，商详页只读取展示）**：商品标题、描述、品牌名、类目名、属性名/值、质检文本、成色等级 grade 值等，由**商品系统**负责翻译接入。商详页按用户当前语言向商品系统取数用于展示，**不建卡、不写入、不感知商品的翻译资源映射**；数据获取链路（是否经翻译服务、由商品系统封装等）由技术方案确定。
...
| `country_code` | 当前 Market 的站点配置；香港站为 `HK` | 可售范围、政策内容、支付方式和推荐商品过滤 |
...
新站点以站点域名、URL 或用户主动选择的 Market 为准，进入后不因 IP 定位、浏览器历史或旧偏好覆盖为其他 Market。页面只查询当前 Market / 国家 / 渠道下的 listing；同一商品没有有效当前 Market listing 时进入“当前站点不可售”，不使用 US Market 或其他 Market 的 listing、价格、政策或购买链接兜底。香港站首期样例：当前 Market 为 HK Market，国家为 HK，币种为 HKD。
...
#### 2.2.1 商详自产内容（第一 / 二类）资源映射

> 本表只列**商详页自产、自建卡**的内容（第一类静态文案 + 第二类 CMS 配置）。第三类（商品标题/描述/品牌/类目/属性名与值/质检文本/成色 grade 值等）由**商品系统**负责翻译接入，商详页只按用户语言取数展示，**不在此列出其资源映射**（见 §1.7 B 段、§2.2.3、第三章）。
...
| Shipping & Returns 标题/富文本 | 第二类（既有） | 沿用现网 `content` 资源 | 沿用现网字段 | `market_id + template_id` 对应配置记录 | 商品翻译模块 · translation_table |
| 静态 UI 文案（按钮/标题/Toast 等） | 第一类 | `page-pdp` | 各 Key | -- | 聚合组 pdp · language_pack |

> **grade 与成色等级体系是两套独立内容**：当前商品的成色 grade 值由**商品系统**维护并接入翻译（商详只读取），驱动"当前商品等级徽章"；CMS 配置的成色等级体系（商详自建卡 `pdp_condition_grade`）驱动"进度条分段 + Condition Guide"。二者互不打通、各自单独接入，不存在二选一。
...
| 保存 Shipping & Returns | 沿用现网写入链路；源文记录增加 `market_id` 归属，资源实例按 `market_id + template_id` 唯一定位 |
...
**第二类（商详自产 CMS 内容）**：商详页自建卡片，按 `resourceType` + `resourceId` + `fieldName` + `language_code`（用户当前语言）调用翻译服务查询；翻译服务内部处理 Fallback（含当前语言=Market 默认语言时的源语言短路），商详页只接收最终结果。一次加载涉及多个 resourceType 时应使用批量查询，一次请求获取所有译文，避免逐字段查询。

**第三类（商品内容）**：商详页按用户当前语言向**商品系统**取数用于展示，具体获取方式（是否经翻译服务、由商品系统封装等）由技术方案确定；商详页不感知商品的 resourceType / 翻译资源映射。

#### 2.2.4 Fallback 策略

引用多语言 PRD §4.6 统一策略，三类内容统一按当前 Market 的源语言兜底，不跨 Market 兜底：

| 优先级 | 条件 | 展示内容 |
|-------|------|---------|
| 1 | 目标语言有译文（translated） | 展示译文 |
| 2 | 目标语言有过期译文（outdated） | 展示旧译文（翻译服务内部处理，商详页不感知） |
| 3 | 无译文记录 | 展示当前 Market 的源语言文本 |

> 第一类（语言包）缺目标语言时由前端 i18n 框架降级显示当前 Market 源语言；第二/三类（translation_table）Fallback 由翻译服务内部处理。商详页均只接收最终结果。CMS 内容、Shipping & Returns、商品内容均不得因为目标语言缺失而跨 Market 回退到 US 内容。
...
商详页部分模块的展示内容由 CMS 后台配置驱动。新开站点后，商详页配置不再只按 PDP 模板组织，而是先确定 **Market**，再管理该 Market 下的 **PDP 模板（Template）**，最后进入模板内模块配置。后台操作流程详见 CMS 配置后台原型文档。

配置层级如下：

`Market → PDP 模板 → 模块配置 → 类目级规则`

- **Market**：配置归属的市场站点，如 US Market、HK Market。不同 Market 的模板和模块配置是独立记录。
- **PDP 模板**：当前 Market 下的商详内容模板，关联一组类目，承载 Certified Authentic、Condition、Description、Shipping & Returns 等模块配置。
- **模块配置**：模板内的模块内容、开关、文案、富文本、链接和预览。
- **类目级规则**：Condition 成色详细信息、Description 属性展示等按类目差异化配置的规则。

现网 watch、bag、jewelry、accessories 模板均属于 **US Market**。新开 Market 时，不能直接复用 US Market 的模板记录给前台读取；必须在新 Market 下创建或复制出独立模板记录，运营确认后再上线。

#### Market 维度

**Market 选择**

CMS 商详页配置入口进入后，页面顶部先展示当前 Market 选择器。运营切换 Market 后，页面只展示和操作该 Market 下的 PDP 模板、关联类目、模块内容和变更记录。

| 配置项 | 规则 |
|---|---|
| 默认 Market | 进入页面默认选中运营账号上次使用的 Market；无历史记录时默认 US Market |
| Market 切换 | 切换后刷新模板 Tab、模块列表、关联类目、模块开关和变更记录 |
| Market 权限 | 当前不做细分，沿用 CMS 现有运营权限；后续可按 Market 增加可见和可编辑范围 |
| Market 缺失 | 未选择 Market 时禁止新建、编辑、删除和保存模板 |
| 当前 Market 展示 | 页面标题区需展示当前 Market，避免运营误改其他 Market 内容 |

**Market 与模板关系**

| 关系 | 说明 |
|---|---|
| 一个 Market 可有多个 PDP 模板 | 如 US Market 下有 watch、bag、jewelry、accessories；HK Market 下也可有同名模板 |
| 同名模板可存在于不同 Market | US Market 的 watch 与 HK Market 的 watch 是两条独立记录，后续修改互不影响 |
| 一个模板只属于一个 Market | 模板创建后必须写入所属 Market，不允许无 Market 模板 |
| 一个类目在同一 Market 下只能关联一个模板 | 类目互斥范围限定在当前 Market；同一类目可在不同 Market 下关联不同模板 |
| 前台按当前 Market 定位模板 | 前台必须使用 `market_id + template_id / category_id` 查询模板，不得跨 Market 查询 |

**从其他 Market 复制模板**

新 Market 初始化时支持从已有 Market 复制模板，用于减少重复录入。复制只是一次性初始化数据，不建立后续联动。

| 复制内容 | 规则 |
|---|---|
| 模板基础信息 | 复制模板名称、描述、关联类目；允许复制后编辑 |
| 模块内容 | 复制 Certified Authentic、Condition、Description、Shipping & Returns 的标题、说明、富文本、链接和开关状态 |
| 类目级规则 | 复制 Condition 成色详细信息配置、Description 属性配置和 Size Guide 开关 |
| 翻译源文 | 复制英文源文到目标 Market 的资源实例；目标 Market 译文需按多语言链路重新生成或维护 |
| 变更记录 | 不复制历史变更记录；目标 Market 只记录复制动作和后续编辑动作 |
| 复制后联动 | 无联动。源 Market 后续修改不会影响目标 Market，目标 Market 修改也不会回写源 Market |

复制时必须选择来源 Market 和来源模板，并指定目标 Market。目标 Market 下已有同名模板时，系统提示运营选择“覆盖现有模板”或“另存为新模板”；覆盖操作需二次确认。

#### PDP 模板

**模板概念**

PDP 模板是某个 Market 下商详内容配置的组织单元。一个模板定义了一组商详页的 Certified Authentic、Condition 等级体系、Description 属性展示规则和 Shipping & Returns 政策说明。

| 属性 | 说明 |
|------|------|
| 所属 Market | 必填；模板只属于一个 Market，创建后前台按该 Market 读取 |
| 模板名称 | 运营自定义（如 watch、bag、jewelry、accessories），同一 Market 内不允许重名，跨 Market 可重名，最大 50 字符 |
| 模板描述 | 选填，用于标注模板用途（如“适用于奢侈品箱包、珠宝、腕表类目”） |
| 关联类目 | 一个模板关联一组类目；一个类目在同一 Market 下只能属于一个模板 |
| 模板级配置 | Certified Authentic、Condition 等级体系、Shipping & Returns——同一模板下所有关联类目共享 |
| 类目级配置 | Condition 成色详细信息、Description 属性展示——模板内按类目差异化配置 |
| 模块开关 | 当前 Market 当前模板内独立生效，不影响其他 Market 的同名模板 |

**模板与类目的关系**

- 一个类目在同一 Market 下只能关联到一个 PDP 模板（1:N 关系，模板:类目）。
- 同一类目可在不同 Market 下关联不同模板。例如 Watches 在 US Market 关联 watch 模板，在 HK Market 也可关联 HK Market 的 watch 模板。
- 未关联当前 Market 任何模板的类目，商详页上 Certified Authentic 不展示、Condition 仅展示基本成色信息（无进度条和等级体系）、Description 不展示、Shipping & Returns 不展示 CMS 配置内容。
- 修改模板关联的类目后，对当前 Market 前台即时生效；不影响其他 Market。

**模板操作**

| 操作 | 规则 |
|---|---|
| 新建模板 | 新建入口与 watch/bag/jewelry/accessories 模板 Tab 同一层级；创建后只属于当前 Market |
| 编辑模板 | 仅编辑当前 Market 当前模板，不影响其他 Market 的同名模板 |
| 删除模板 | 支持删除当前 Market 下的模板；删除前二次确认；删除后该 Market 对应类目的 CMS 配置内容不再展示 |
| 复制模板 | 可从其他 Market 复制模板初始化；复制后独立维护 |
| 模板重名校验 | 同一 Market 内不允许重名；不同 Market 允许同名 |
| 保存校验 | 所属 Market、模板名称、关联类目、必填模块内容缺失时阻止保存 |

#### 可配置模块

| 模块 | 配置粒度 | 可配置内容 | 详见章节 |
|------|---------|-----------|---------|
| Certified Authentic 鉴定认证 | Market + 模板级 | 标题、描述文案、详细说明（半层弹窗内容） | 2.9 |
| Condition 成色 | Market + 模板级 + 类目级 | 模板级：成色等级体系（等级名称 + 描述）；类目级：成色详细信息展示配置（定义展示名 + 绑定数据源字段 + 排序） | 2.10 |
| Description 描述 | Market + 模板内类目级 | 属性展示配置（定义展示名 + 绑定数据源 + 排序），含 Size Guide | 2.11 |
| Shipping & Returns 配送与退货 | Market + 模板级 | 模块标题、配送标题/说明、退货标题/说明、政策链接、富文本内容、前台预览 | 本节 |

#### Shipping & Returns 配置规则

Shipping & Returns 是 PDP 模板的第 4 个配置模块。该模块必须随 Market 归属独立维护，不能把 US Market 的政策文案直接作为新 Market 的前台兜底。

| 配置项 | 配置粒度 | 规则 |
|---|---|---|
| 模块标题 | Market + 模板 | 如 Shipping & Returns；当前 Market 当前模板独立配置 |
| Free Shipping 标题/说明 | Market 内公共或模板级复用 | 只能在当前 Market 内复用，不得跨 Market 共用 |
| Easy Returns 标题/说明 | Market + 模板 | 各 Market × 各模板分别维护 |
| Policy 链接 | Market + 模板 | 指向当前 Market 对应政策页；香港站不得指向美国政策页 |
| 富文本内容 | Market + 模板 | 支持结构化段落、链接、列表和预览 |
| 模块开关 | Market + 模板 + 模块 | 当前 Market 当前模板独立开关，不影响其他 Market |

前台读取时，必须按当前页面上下文中的 `market_id + template_id` 定位 Shipping & Returns。当前 Market 无模板、模板未关联类目或 Shipping & Returns 配置缺失时，该模块不展示；禁止运行时回退 US Market 的配置。

新 Market 首期上线前，需要在该 Market 下建立 watch、bag、jewelry、accessories 四个模板记录；每个模板补齐 Shipping & Returns 的配送文案、退货文案和当前 Market 政策链接。香港站首期样例：在 HK Market 下建立四个模板，政策链接指向香港政策页。

#### 其他模块的配置边界

Breadcrumb、Gallery、商品信息区、CTA 按钮、推荐区、浏览历史、Footer、Lightbox 的数据来源固定，无需后台配置。上述模块读取当前 Market 的商品、价格、库存、推荐和链接上下文，但不在 CMS 商详页配置里维护。
...
- 同一模板内不同类目可配置不同的字段集合和展示名，互斥范围限定在当前 Market 当前模板内。
...
- 同一模板内不同类目可配置不同的属性集合，互斥范围限定在当前 Market 当前模板内。
- **新增规则时类目选择互斥**：下拉列表仅展示当前 Market 当前模板关联的类目，已配置规则的类目置灰显示“xxx（已配置）”不可选择。所有类目均已配置时，提示“当前模板下所有类目已配置，如需调整请编辑已有规则”。
- 类目规则**整条替换**，不做属性级合并。
...
- 当前不设草稿态，保存即生效。
- 运营点击「保存」后，配置立即应用到当前 Market 前台商详页。
- 保存时必须写入 `market_id`、`template_id`、模块编码和操作人。
- 当前 Market 必填项不完整时阻止保存，并定位到缺失字段。
- 不设审批流程。

#### 模块开关

- Certified Authentic / Condition / Description / Shipping & Returns 模块各有独立开关。
...
- 开关切换即时生效，不需要额外保存操作。
- 开关作用于当前 Market 当前模板；不影响其他 Market 的同名模板。

#### Size Guide 默认值策略

- 新增 Description 规则时 Size Guide 作为固定模块自动带入属性列表，默认开启。
- 运营可通过开关控制该类目是否展示 Size Guide 入口。
- 关闭 Size Guide = 当前 Market 当前模板下该类目商详页不展示 Size Guide 入口。
...
记录所有 CMS 配置操作，用于审计追溯。变更记录必须带 Market，便于运营区分同名模板在不同 Market 下的修改。

| 记录字段 | 说明 |
|---------|------|
| 所属 Market | 该操作所在的 Market，如 US Market、HK Market |
| 操作人 | 执行配置操作的运营人员 |
| 操作时间 | 精确到分钟 |
| 所属模板 | 该操作所在的 PDP 模板 |
| 所属模块 | 模板管理 / Certified Authentic / Condition / Description / Shipping & Returns / 模块管理 |
| 操作类型 | 切换 Market / 从其他 Market 复制模板 / 创建模板 / 编辑模板 / 删除模板 / 编辑配置 / 新增规则 / 编辑规则 / 删除规则 / 关闭模块 / 开启模块 |
| 变更详情 | 具体变更内容，示例见下表 |
...
| 操作类型 | 所属模块 | 变更详情示例 |
|---------|---------|------------|
| 从其他 Market 复制模板 | 模板管理 | 从 US Market 复制 watch 模板到 HK Market，生成 HK Market / watch |
| 创建模板 | 模板管理 | 在 HK Market 创建「watch」，关联 Watches |
| 编辑模板 | 模板管理 | 修改 HK Market「watch」名称或关联类目 |
| 删除模板 | 模板管理 | 删除 HK Market「accessories」模板 |
| 编辑配置 | Certified Authentic | 修改 HK Market / watch 的 Certified Authentic 标题和描述文案 |
| 编辑配置 | Shipping & Returns | 修改 HK Market / bag 的香港退货政策链接 |
| 新增规则 | Description | 在 HK Market / watch 新增 Watches 类目级 Description 属性配置 |
| 编辑规则 | Condition | 修改 HK Market / bag 类目级成色详细信息配置：新增 Care Notes 字段 |
| 删除规则 | Description | 删除 HK Market / jewelry 类目级 Description 属性配置 |
| 关闭模块 | 模块管理 | 关闭 HK Market / accessories 的 Condition 模块 |
| 开启模块 | 模块管理 | 开启 HK Market / accessories 的 Shipping & Returns 模块 |

- 永久保留，不设滚动清理（配置变更频率低，存储成本可忽略）。
- 支持按所属 Market、所属模板和所属模块筛选。

#### 操作权限

- 当前不做权限细分，所有后台运营人员均可配置规则和查看变更记录。
- 后续接入权限系统后按角色区分（CMS 管理员 / 只读运营），并支持按 Market 控制可见和可编辑范围。

#### CMS 配置内容的翻译接入

- CMS 配置的可翻译文本（Certified Authentic 标题/描述/详情、Condition 等级名/描述、成色详细信息展示名、Description 属性/Size Guide 展示名、Shipping & Returns 标题/说明/富文本）均属**第二类内容**，由商详页 CMS 配置后台按多语言 v3.2 标准接入：**先建卡或沿用既有资源 → 注册字段 → 保存时写入翻译服务触发 AI 翻译**。建卡/注册/写入规范详见 §2.2.2，卡片与字段清单见 §1.7 B 段。
- 保存写入以当前 Market 的源语言录入；译文由翻译服务自动生成。翻译完成前，前台只允许回退当前 Market 的源语言文本，不得跨 Market 回退 US 内容。
- Shipping & Returns 沿用现网商品翻译模块的既有资源映射，但资源实例必须增加 `market_id` 归属，按 `market_id + template_id` 唯一定位。
...
| Tax 提示 | CMS 或税务模块 | 按当前 Market 返回最终文案；香港站不得展示美国税费口径。具体来源以现网归属为准 |
...
- 香港站只开放 HKD，Header 不提供其他币种切换；页面、购物车和结算金额统一显示 `HK$`
- 香港 listing 的定价货币必须为 HKD；币种缺失或不是 HKD 时不可购买
- 本规则只作用于香港站，美国站现有币种规则保持不变
...
新站点还需满足：存在当前 Market / 渠道下的 active listing，listing 定价货币与当前 Market 交易币种一致，且库存服务返回 `available_qty > 0`。商品存在但没有有效当前 Market listing 时，CTA 区显示当前站点不可售文案，购买按钮禁用，并提供返回当前站点 Shop 的入口；商品主体信息仍可浏览，不读取 US Market 或其他 Market listing 兜底。香港站首期样例：存在 HK Market / 渠道下的 active listing，listing 定价货币为 HKD；不可售文案为 `Not available in Hong Kong`。
...
新站点执行 Add to Cart 或 Buy Now 前，重新校验 Market、当前 Market listing、当前 Market 交易币种价格和库存。写入购物车/结算的 `market_id`、`listing_id`、币种和金额必须与当前商详一致，不得跳转其他 Market 结算或替换为其他 Market listing。香港站首期样例：校验 HK Market、香港 listing、HKD 价格和库存，不得跳转美国结算或替换为美国 listing。
...
| 成色等级 | `product_inspection.grade` | 商品系统固定枚举值（如 `Like New` / `Excellent` / `Very Good` / `Good` / `Fair`），商品录入时选取，走翻译（由商品系统接入） |
...
展示物流配送政策和退换货政策。内容由商详 CMS 的 PDP 模板 → Shipping & Returns 模块配置。新开站点改造的核心不是新增前台模块，而是让 PDP 模板先归属具体 Market，再由当前 Market 下的模板提供各模块内容。香港站是首期落地样例。

**数据来源与取值规则**

| 页面元素 | 数据源 | 取值逻辑 |
|---------|-------|---------|
| 模块标题 | CMS | 按 `market_id + template_id` 读取当前 Market 模板配置；香港站首期读取 HK Market 模板 |
| Free Shipping 标题/说明 | CMS | 作为当前 Market 内可复用的配送文案；US 与 HK 各自维护，不跨 Market 共用 |
| Easy Returns 标题/说明 | CMS | 按 `market_id + template_id` 维护；watch、bag、jewelry、accessories 可分别配置 |
| Policy 链接 | CMS 富文本 | 链接随对应 Market 模板文案维护；新站点进入当前 Market 政策页，香港站首期进入香港政策页 |

**展示规则**

- PC 端默认展开，APP 端默认折叠（减少首屏滚动深度）
- Policy 链接：PC 端新标签页打开，APP 端 Webview 或原生页面打开
- CMS 配置页先选择 Market，再展示该 Market 下的 PDP 模板；当前现网四个模板属于 US Market
- 新 Market 需要建立自己的 watch、bag、jewelry、accessories 模板记录；可从 US Market 或其他已配置 Market 复制初始化，但复制后独立维护。香港站首期为 HK Market
- 新建模板入口与模板 Tab 同一层级；删除模板只删除当前 Market 下的模板，不影响其他 Market 的同名模板
- 保存英文源文案后沿用现有生效与变更记录机制，变更记录需包含 Market 与模板；繁中等语言在商品翻译模块维护
- HK 无模板、无配置或必填项不完整时不得运行时回退 US；上线前必须完成 HK 四类模板验收
...
推荐结果先过滤为当前 Market/国家可售的 listing。新站点只展示有效当前 Market listing，并以当前 Market 交易币种展示价格；过滤后不足 2 条时沿用现有隐藏规则。香港站首期样例：只展示有效香港 listing，并以 HKD 展示价格。
...
- 当前商品不在浏览历史中重复展示
- 交互行为同推荐区（卡片点击、收藏）
- 新站点只展示仍有有效当前 Market listing 的历史商品；只有其他 Market listing 的记录不展示。香港站首期样例：只有美国 listing 的记录不展示
...
| 商品系统（第三类内容） | 商品内容的多语言翻译接入 | **已接入** | 商品模块已接入多语言翻译；商详页按用户语言向商品系统取数展示即可，不感知商品的翻译资源映射 |
| CMS 系统 | PDP 模板管理、模块配置（含 Shipping & Returns） | 已上线 | 现状模板归属 US Market；新站点需增加 Market 选择、PDP 模板 Market 归属、复制初始化、删除模板和当前 Market 配置数据 |
...
| Market / 商品域 | 新 Market、渠道和当前 Market listing | 上线前完成 | 当前 Market 无有效 listing 时不得使用 US Market 或其他 Market listing 兜底；香港站首期为 HK Market / HK listing |
| 购物车 / 结算 | 当前 Market 购物车、当前 Market 币种金额和当前 Market 结算 | 上线前联调 | Add to Cart 与 Buy Now 全链路保持当前 Market；香港站首期为 HK Market / HKD |
...
| CMS PDP 模板仍只有模板维度 | 新站点会读取 US Market 模板、政策和费用 | 现有商详页配置增加 Market 选择，模板和模块配置归属具体 Market；当前 Market 内容不允许回退 US |
| 当前 Market listing、库存或价格上下文串到其他 Market | 误报售罄、错价或跨站下单 | 购买动作前重新校验 Market + listing + currency + inventory；香港站不得串到 US |
...
- CMS 后台配置（PDP 模板管理、Certified Authentic / Condition / Description / Shipping & Returns 四模块配置）
...
- 新开站点支持：当前 Market 上下文、当前 Market listing、当前 Market 币种、当前站点不可售状态、CMS PDP 模板 Market 归属、Shipping & Returns 当前 Market 配置、推荐与浏览历史按当前 Market 过滤；香港站首期样例为 HK Market / HK listing / HKD
...
| CMS 后台 | looply-商详页CMS配置后台原型-v5-market-online-antd.html | -- |
...
| 缩略图选中样式 | 黑色边框 + 透明度 0.7→1 | 红色强调边框 + 透明度 0.6→1 |
...
| CMS PDP 模板 Market 归属 | 现有能力扩展 | CMS 域 | 商详页配置增加 Market 选择；PDP 模板、模块配置、Shipping & Returns、模块开关、变更记录均归属具体 Market；补齐 HK × 4 商品模板内容 |
...
| CMS 后台原型 | `~/Desktop/海外业务/商详/原型/looply-商详页CMS配置后台原型-v5-market-online-antd.html` | 当前确认原型；商详页配置按 Market → PDP 模板 → 模块配置组织，支持从其他 Market 复制模板、新建模板和删除当前 Market 模板 |
...
| 数据来源与实现逻辑 V2.1 | `~/Desktop/海外业务/商详/商详-交付开发 V1.3-20260922/looply-商详页-数据来源与实现逻辑-V2.1.md` | 页面字段到各业务域的数据来源映射，含新开站点与 CMS 政策取数 |
