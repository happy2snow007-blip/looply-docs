...
香港站以站点域名、URL 或用户主动选择的 Market 为准，进入后不因 IP 定位覆盖为美国站。页面只查询香港 Market/国家/渠道下的 listing；同一商品没有有效香港 listing 时进入“香港不可售”，不使用美国 listing、价格或购买链接兜底。
...
香港站首期要求：在 HK Market 下建立 watch、bag、jewelry、accessories 四个模板记录；每个模板补齐 Shipping & Returns 的配送文案、退货文案和香港政策链接。
...
香港站还需满足：存在当前香港 Market/渠道下的 active listing，listing 定价货币为 HKD，且库存服务返回 `available_qty > 0`。商品存在但没有有效香港 listing 时，CTA 区显示 `Not available in Hong Kong`，购买按钮禁用，并提供返回香港 Shop 的入口；商品主体信息仍可浏览，不读取美国 listing 兜底。
...
香港站执行 Add to Cart 或 Buy Now 前，重新校验 Market、香港 listing、HKD 价格和库存。写入购物车/结算的 `market_id`、`listing_id`、币种和金额必须与当前商详一致，不得跳转美国结算或替换为美国 listing。
...
展示物流配送政策和退换货政策。内容由商详 CMS 的 PDP 模板 → Shipping & Returns 模块配置。香港站改造的核心不是新增前台模块，而是让 PDP 模板先归属具体 Market，再由当前 Market 下的模板提供各模块内容。

**数据来源与取值规则**

| 页面元素 | 数据源 | 取值逻辑 |
|---------|-------|---------|
| 模块标题 | CMS | 按 `market_id + template_id` 读取当前 Market 模板配置；香港站读取 HK 模板 |
...
| Policy 链接 | CMS 富文本 | 链接随对应 Market 模板文案维护；香港站进入香港政策页 |
...
- HK Market 需要建立自己的 watch、bag、jewelry、accessories 模板记录；可从 US Market 复制初始化，但复制后独立维护
...
推荐结果先过滤为当前 Market/国家可售的 listing。香港站只展示有效香港 listing，并以 HKD 展示价格；过滤后不足 2 条时沿用现有隐藏规则。
...
- 当前商品不在浏览历史中重复展示
- 交互行为同推荐区（卡片点击、收藏）
- 香港站只展示仍有有效香港 listing 的历史商品；只有美国 listing 的记录不展示
...
| CMS 系统 | PDP 模板管理、模块配置（含 Shipping & Returns） | 已上线 | Shipping & Returns 现状按商品模板配置；香港站需增加 Market 选择和 HK 配置数据 |
...
| Market / 商品域 | 香港 Market、渠道和香港 listing | 上线前完成 | 香港无有效 listing 时不得使用美国 listing 兜底 |
| 购物车 / 结算 | 香港购物车、HKD 金额和香港结算 | 上线前联调 | Add to Cart 与 Buy Now 全链路保持香港 Market |
...
| CMS Shipping & Returns 仍只有模板维度 | 香港站会读取美国政策和美元费用 | 现有模块增加 Market 选择；HK 内容不允许回退 US |
| 香港 listing、库存或价格上下文串到美国 | 误报售罄、错价或跨站下单 | 购买动作前重新校验 Market + listing + currency + inventory |
...
- 香港站首期：香港 Market/HK listing/HKD、香港不可售状态、Shipping & Returns 增加 Market 维度、香港推荐与浏览历史过滤
...
| Shipping & Returns Market 维度 | 现有能力扩展 | CMS 域 | 在现有模块中增加 Market 选择，并补齐 HK × 4 商品模板内容 |
...
| CMS 后台原型 | `~/Desktop/海外业务/商详/原型/looply-商详页CMS配置后台原型-v3-antd.html` | 历史原型；现网已包含 Shipping & Returns 第 4 模块，本期只在该模块增加 Market 选择 |
...
| 数据来源与实现逻辑 V2.1 | `~/Desktop/海外业务/商详/looply-商详页-数据来源与实现逻辑-V2.1.md` | 页面字段到各业务域的数据来源映射，含香港站与 CMS 政策取数 |
