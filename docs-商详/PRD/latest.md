...
- 模块开关关闭后，前台商详页不展示该模块的 CMS 配置内容（完全隐藏，无默认内容）
  - **Condition 例外**：Condition 模块开关仅控制 CMS 配置的部分（进度条 + Condition Guide）。关闭后进度条和 Condition Guide 不展示，但成色描述、外观描述、配件信息、补充描述等来自 `product_inspection` 的质检信息仍保留展示（属于商品固有属性，不受 CMS 开关控制）
- 开关切换即时生效，不需要额外保存操作
