...
- Certified Authentic 标题/描述/详情、Condition 等级名/描述同样保存后自动进入翻译队列
- 翻译完成前，前台按原始语言（英文）展示
...
| CMS 后台原型 | `~/Desktop/海外业务/商详/原型/looply-商详页CMS配置后台原型-v3-antd.html` | PDP 模板管理 + Certified Authentic / Condition / Description 三模块后台配置（antd 版） |
