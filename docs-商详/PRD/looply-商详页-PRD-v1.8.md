...
>
> **本版变更（V1.8）**：收敛第三类（上游商品内容）的越界描述，对齐多语言接入规范 §5 场景三——① §2.2.1 删除第三类（商品域）resource_type/fieldName/storage_mode 映射，仅保留商详自产（第一/二类）内容；② 第三类改为一句话交代归属（商品系统负责翻译接入），商详页只按用户语言取数展示、**不感知商品翻译资源映射**，数据获取链路由技术方案确定；③ 商品模块**已接入多语言翻译**，删除"第三类 resourceType 待对接/待收敛"依赖与风险。商详自产的第二类 CMS 建卡/写入规范不变。
>
