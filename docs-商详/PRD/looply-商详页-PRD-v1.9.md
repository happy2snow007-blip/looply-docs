...
Breadcrumb、Gallery、商品信息区、CTA 按钮、推荐区、浏览历史、Footer、Lightbox 的数据来源固定，无需后台配置。

Shipping & Returns 是现有 PDP 模板的第 4 个配置模块：模块标题、配送标题/说明、退货标题/说明均在 CMS 中维护，支持富文本和前台预览。现状按商品模板/关联类目配置，尚无 Market 维度；英文源文案在 CMS 保存，其他语言在商品翻译模块维护。香港站只给该模块补 Market 维度，不新增独立 CMS 页面。
