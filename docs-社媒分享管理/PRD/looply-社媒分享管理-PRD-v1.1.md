...
1. 为商品详情页提供移动端（Mobile Web / iOS App / Android App）基础社媒分享能力，满足用户基本预期。PC 端不做分享入口。
...
**交互说明**：分享按钮在 PDP 常驻显示。本期 PC Web 不提供分享入口（仅 Mobile Web / iOS / Android）。

**UI 关联**：
- 前端高保真原型（商详页内）：Figma [Looply v1.0 - Product Detail + Share](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=452-127&p=f&t=RhbcJEtFj0saHgTl-0)
- 参考线框（.pen，作形态示意）：Mobile Web `NPfew` / iOS `rMtyB` / Android `9wWk7`

#### 2.2.2 分享弹窗 / 面板（渠道列表）

**功能描述**：展示可用分享渠道，移动端为底部面板，面板顶部带商品摘要卡（商品主图 + 商品名 + 价格）。
...
② 判断 User-Agent：
   ...
   └── 普通真人 →
        ...
        4. 合并 query 参数：long_url 原有参数优先，请求 URL 参数只补充不覆盖
           （规则见下方【参数合并规则】）
        5. 302 重定向到合并后的最终 URL（带完整 UTM 参数，GA4 可识别）
```
...
用户从 Facebook / Instagram 等平台点击短链时，平台会自动在 URL 上追加参数（如 `utm_source=facebook`、`fbclid`）。跳转前必须把请求 URL 的 query 与 `long_url` 的 query 做合并，**核心原则：`long_url` 原有参数优先，外部参数只补充、不覆盖**。

| 参数类型 | 参数 | 合并策略 |
|---------|------|---------|
| UTM 参数 | `utm_source` / `utm_medium` / `utm_campaign` / `utm_content` / `utm_term` | **以 `long_url` 已设置的值为准**，请求 URL 的同名参数直接丢弃 |
| Click ID | `fbclid` / `gclid` / `ttclid` / `msclkid` / `twclid` / `li_fat_id` 等 | `long_url` 未设置则**透传**请求 URL 的值；已设置则保留 `long_url` 原值 |
| 其他 query 参数 | 任意 | `long_url` 已有同名参数则**保留原值**；无同名参数则透传请求 URL 的值 |

**实现细节**：
- 判定"已设置"以 **key 是否存在**为准，不看值是否为空——`long_url` 中 `utm_content=`（空值）也算已设置，不被外部覆盖。
- 多值参数（同名 key 出现多次）：只要 key 已存在于 `long_url`，则整体保留其全部值，外部同名参数的所有值一并丢弃，不做逐值合并。
- 参数顺序：`long_url` 原有参数在前，透传补充的外部参数追加在后，便于人工核对。
- Fragment（`#` 后内容）：始终以 `long_url` 为准（浏览器不会把 fragment 发给服务端）。
- **不引入开关参数**：Short.io 时期为阻止平台参数覆盖需额外加 `shortio_nomerge=1`，属其私有约定。自建短链默认即"目标 URL 优先"，短链对外形态保持纯路径 `looply.com/s/{short_code}`，不识别、不沿用该参数。

**示例**：

```
场景 1：Facebook 点击，平台追加 utm_source + fbclid
  short_link : looply.com/s/sh_8f3k29
  long_url   : looply.com/product/1001?utm_source=imessage&utm_medium=social_share
  请求 URL   : looply.com/s/sh_8f3k29?utm_source=facebook&fbclid=IwAR0abc123
  最终跳转   : looply.com/product/1001?utm_source=imessage&utm_medium=social_share&fbclid=IwAR0abc123
  → utm_source 保持 imessage（未被 facebook 覆盖）；fbclid 透传补充

场景 2：Instagram 点击，仅追加 click ID
  long_url   : looply.com/product/1001?utm_source=instagram&utm_medium=social_share&utm_campaign=product_share
  请求 URL   : looply.com/s/sh_8f3k29?igshid=MzRlODBiNWFlZA
  最终跳转   : looply.com/product/1001?utm_source=instagram&utm_medium=social_share&utm_campaign=product_share&igshid=MzRlODBiNWFlZA
  → 无冲突，外部参数全部补充

场景 3：普通业务参数冲突
  long_url   : looply.com/product/1001?ref=share&utm_source=whatsapp
  请求 URL   : looply.com/s/sh_8f3k29?ref=ad&color=black
  最终跳转   : looply.com/product/1001?ref=share&utm_source=whatsapp&color=black
  → ref 保留 long_url 的 share（丢弃外部 ad）；color 为新 key，透传补充
```

**异常处理**：
- short_code 不存在 → 302 到 Looply 首页（`looply.com`），不报 404。
- 目标商品已下架 → 真人点击 302 到 Looply 首页（`looply.com`）；社媒爬虫返回通用品牌 OG 卡片（见 2.1.2）。（更丰富的"商品已下架"兜底页 / 推荐相似商品为 TBD-1）
- 外部参数异常（合并环节）→ 单个参数值超长（建议阈值 512 字符）或含控制字符 / 换行的，**丢弃该参数**，不影响其余参数与跳转；合并后 URL 总长超 2048 字符时，按"UTM → click ID → 其他参数"优先级从低到高裁掉外部补充参数，`long_url` 原有参数永不裁剪。
- 外部参数不参与跳转地址决策 → 跳转目标只取自 `long_url`，请求 URL 的参数**仅能补充 query，不能改写 host / path**，避免 open redirect。
...
- C 端商品详情页分享（Mobile Web / iOS / Android 三端；PC 端不做分享入口）。
...
| 编号 | 事项 | 说明 | 状态 |
|------|------|------|------|
| TBD-1 | 用户分享短链目标下架后的落地处理 | 目标商品 / 页面已下架但链接仍被点击。MVP 已先 302 到 Looply 首页兜底；后续规划更友好的"商品已下架"提示页 / 推荐相似商品 | 后续规划 |
...
|------|--------|----------|-----------|
| C 端商品分享 | 商品详情 + 分享弹窗 | — （PC 不做分享） | 前端高保真：[Figma Looply v1.0](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=452-127&p=f&t=RhbcJEtFj0saHgTl-0)；参考线框 .pen：`rMtyB`（iOS）/ `9wWk7`（Android）/ `NPfew`（Mobile Web） |
