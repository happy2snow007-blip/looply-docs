...
>
> Version 1.0 · 2026-07-08 · 面向：前端 / 后端开发
...
| 商品图片 | order_item.product_image | PC 端：每件商品完整行展示；APP 端：最多展示 3 张缩略图 |
| 总金额 | order.total_amount | PC 端格式 "Total: $1,524 (Total N Items)"；APP 端格式 "$1,524" + "(N Items)" |
| 商品件数 | count(order_items) | 单件商品不显示件数标注；多件显示件数 |
| 退款标记 | 聚合 order_item.refund_status | 若有 refunded 的商品，对应商品图上叠加 "Refund" 角标 |

**状态标签样式：**

| order_status | 标签文案 | 标签色调 |
|-------------|---------|---------|
| paid | "Confirmed" | 蓝色系（具体色值以 Figma 设计稿为准） |
| shipped | "On its way" | 橙色系（具体色值以 Figma 设计稿为准） |
| completed | "Completed" | 绿色系（具体色值以 Figma 设计稿为准） |
| cancelled | "Cancelled" | 灰色系（具体色值以 Figma 设计稿为准） |
...
当一个订单包含多个发货组（包裹）时，按组平铺展示，每组结构为：物流状态卡 + 该包裹内的商品行。
...
- 不显示 "Package 1 / Package 2" 标题头部，物流状态卡本身即为分组的视觉分隔
- 每个物流状态卡可独立点击查看该包裹的物流轨迹
- 单包裹订单只有一组，结构相同（物流状态卡 + 商品行）

### 3.5 商品区域（按组平铺）

订单详情页的商品区域采用**按组平铺**结构。一个商品只会处于发货状态或退款状态，不会同时存在两种状态。多包裹、部分退款等场景统一按组展示：
...
退款摘要卡下方紧跟该笔退款关联的商品行。多笔退款则有多个退款组。
...
| 商品区域 | 按组平铺展示：每个包裹独立一组（物流状态卡 + 商品行），退款独立一组（退款摘要卡 + 商品行） |
...
| 商品区域 | 按组平铺展示：发货组（物流状态卡 + 商品行）+ 退款组（退款摘要卡 + 商品行，若有） |
...
| keyword | string | 否 | 搜索关键词（匹配订单号、商品名） |
| page | int | 否 | 页码，默认 1 |
...
| currency | 币种 |
| item_count | 商品件数 |
| items[] | 商品数组，每项含 image_url, brand, product_name, condition, price, has_refund（PC 端展示全部商品完整行；APP 端取前 3 件缩略图） |
