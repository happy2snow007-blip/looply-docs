...
PC UI 的 `Who We Are` 与 Mobile UI 的 `Who we are` 统一为 `Who We Are`。三项信念统一为 `Personal Style`、`Trust, Clearly Earned`、`Never Secondary`；PC 与 Mobile 使用相同标题及正文。
...
| Who We Are | 居中标题与五段正文 | 居中标题与五段正文；保持段落顺序 |
...
标题展示 `Who We Are`，正文按以下五段顺序展示：

1. `A loop is never just a return—it is the beginning of a new chapter.`
2. `At Looply, we believe that beautifully made pieces should not be confined to a single chapter. The pieces we cherish—the ones shaped by craftsmanship and intention—deserve to be rediscovered, reinterpreted, and passed on, gathering meaning as they move from one life to another.`
3. `Looply exists to make that movement feel effortless. Through careful curation and rigorous authentication, we replace the uncertainty long associated with resale with confidence, clarity, and ease.`
4. `Here, designer pieces don’t simply change hands—their stories, beauty, and significance live on.`
5. `Based in Los Angeles, Looply was founded in 2026 to bring this belief to life: creating a more trusted and considered way for exceptional pieces to continue their stories.`
...
| 2 | `Trust, Clearly Earned` | `That trust should come standard. Through rigorous authentication and clear condition details, we help you understand every piece and choose with confidence.` | 已发布的信任主题图片 |
...
标题展示 `Why Our Authentication Stands Apart`，正文展示：

`Every item offered on Looply undergoes a dual authentication process combining expert human review with our proprietary AI technology. Our authentication specialists draw on collective experience gained through the inspection and authentication of more than 1.28 million items—expertise that has helped shape and refine the rigorous methods now applied at Looply. By combining human discernment and judgment with the speed, consistency, and precision of AI, this approach allows us to stand behind every eligible item with the Looply Lifetime Authenticity Guarantee.`
...
| 终端 | 设计稿 |
|---|---|
| PC | `/Users/zz/Desktop/img_v3_02149_501c101d-c208-460e-b101-11b1ae55d40g.png` |
...
| `who_we_are_paragraph_1` 至 `who_we_are_paragraph_5` | Who We Are |
...
| 页面 | PC UI | Mobile UI |
|---|---|---|
| About Looply | `img_v3_02149_501c101d-c208-460e-b101-11b1ae55d40g.png` | `img_v3_02149_a4236bac-a454-4c54-81bf-28f591dceefg.png` |
...
| Who We Are | 标题 + 五段正文 | 一致 | 一致；标题大小写统一为 `Who We Are` | 已对齐 |
| What We Believe | 3 张图片卡片；统一标题和正文 | 标题 2 由 `Trust` 更新为 `Trust, Clearly Earned` | 标题 2、3 与正文更新为统一口径 | 需同步 UI |
| What You'll Find | 标题 + 5 项编号清单；PC 右图、Mobile 无图 | 一致 | 一致 | 已对齐 |
| Authentication | 图片、标题、正文、收束句、CTA | 一致 | 一致 | 已对齐 |
