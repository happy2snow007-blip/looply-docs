...
- Mobile Web 首页搜索框为入口型控件，不接收真实输入；点击右侧搜索图标时展示轻量 Toast `Please enter a search term.`，保持当前页面。
...
- PC Web 与 Mobile Web 提交搜索时，先去除首尾空格并将连续空格归一；处理后为空则不发起搜索，展示轻量 Toast `Please enter a search term.`，并保留当前页面和搜索发现内容。
...
|---|---|---|
| 首页/Header 搜索入口 | Header 固定展示导航栏和搜索图标 | 首页展示入口型搜索框、轮播底纹词和右侧搜索图标；点击搜索框进入全屏搜索页，点击图标时按空输入规则展示 Toast |
| 结果页顶部 | 保留全局 Header、导航栏和搜索图标；点击图标在导航栏下方展开搜索层 | 返回按钮 + 当前搜索词入口框 + 搜索图标；点击后进入统一全屏搜索层 |
| 搜索发现形态 | Header 下方展开搜索层 | 全屏搜索页 |
| 搜索提交 | 键盘 `Enter` 或搜索框内图标；未输入或仅输入空格时展示 Toast | 首页搜索图标、全屏搜索页搜索图标或系统键盘 `Search` 均遵循相同校验；未输入或仅输入空格时展示 Toast，不提交底纹词 |
