...
### 10.2 PC Web 全站 Header 常驻搜索框

1. PC Web 全站 Header 将现有搜索图标入口改为常驻搜索框，首页、Shop、Collection、搜索结果、商品详情及其他使用全站 Header 的页面统一生效。
2. 搜索框默认展示现有本地化 placeholder 和搜索按钮；页面加载后无需先点击搜索图标即可输入。
3. 点击或聚焦搜索框后，展示现有搜索发现层；输入内容后按现有规则展示 Sug。
4. 用户按 Enter 或点击搜索按钮时，按现有全局搜索提交规则进入搜索结果页。
5. Header 吸顶或页面滚动时搜索框保持可见、可输入；具体宽度、间距和响应式收缩以最新 UI 为准。
6. Mobile Web 保持现有搜索入口和全屏搜索发现页交互不变。
