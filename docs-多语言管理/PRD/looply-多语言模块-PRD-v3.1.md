...
> 日期：2026-06-23  
> 状态：评审版  
> 变更：基于 prd-writing skill 自检补全——新增依赖与风险（§八）、版本规划（§九）；补充 translatable_field_config.status 枚举定义、字段停用/废弃处理规则、页面状态变体、端到端数据流示例；明确仅 PC 端后台
...
| 系统管理员 | 负责翻译模块配置 | 管理可翻译字段（Key 管理页查看、编辑、停用；页面文案类 Key 的手动注册和批量导入） |
...
**translatable_field_config.status 定义**

| 状态值 | 中文显示 | 含义 | 触发时机 |
|--------|---------|------|---------|
| active | 已激活 | 翻译模块接受该字段的源内容写入和翻译触发 | 业务模块通过 API 注册字段时默认状态；运营手动注册页面文案时默认状态 |
| inactive | 未激活 | 翻译模块拒绝该字段的新写入，不触发新翻译，已有翻译记录保留可查 | 运营在 Key 管理页手动停用 |
| deprecated | 已废弃 | 同 inactive，标识该字段已被业务侧废弃 | 运营在 Key 管理页手动废弃 |

状态转换：active ↔ inactive（可双向切换）；active/inactive → deprecated（单向，废弃后不可直接恢复为 active，需先设为 inactive 再激活）。
...
```
用户请求（Accept-Language: fr）
  → 前端/业务层查询翻译服务：resource_type=product, resource_id=123, field_name=title, language_code=fr
  → 翻译服务查 translation 表
    → 有 fr 记录且 status=translated → 返回法语标题
    → 有 fr 记录且 status=outdated → 返回旧法语标题（源文本已变更，待重译）
    → 无 fr 记录 → 返回英文源标题（Fallback）
```

同一查询模式适用于所有资源类型和字段，调用方无需关心源语言是什么、翻译是 AI 还是人工产出。
...
每个资源类型在导航页显示为一张卡片，有三种状态：
- **已对接**（实线卡片）：业务模块已完成翻译服务对接，卡片可点击进入翻译详情页，显示翻译进度
- **待对接**（实线卡片，灰色）：业务模块已设计但尚未完成翻译服务对接，卡片不可点击。完成对接后切换为已对接状态
- **占位**（虚线卡片）：资源类型已规划但业务模块尚未设计，卡片不可点击，无翻译进度。业务模块完成对接后自动切换为实线状态

| 域 | resource_type | 显示名称 | 对接状态 |
|----|--------------|---------|---------|
| 商品域 | listing | 渠道商品 | 待对接 |
| 商品域 | product | 实物商品 | 待对接 |
| 商品域 | spu | 标品SPU | 待对接 |
| 商品域 | sku | 标品SKU | 待对接 |
| 商品域 | category | 类目 | 待对接 |
| 商品域 | brand | 品牌 | 待对接 |
| 商品域 | series | 系列 | 待对接 |
| 商品域 | attribute | 属性 | 待对接 |
| 商品域 | attribute_option | 属性选项 | 待对接 |
| 商品域 | enum | 成色等级 | 待对接 |
| 前端页面 | page-auth | 登录注册 | 占位 |
| 前端页面 | page-home | 首页 | 占位 |
| 前端页面 | page-collection | 集合页 | 占位 |
| 前端页面 | page-pdp | 商详页 | 待对接 |
| 前端页面 | page-cart | 购物车 | 占位 |
| 前端页面 | page-checkout | 结账页 | 占位 |
| 前端页面 | page-order-success | 支付成功页 | 占位 |
| 前端页面 | page-orders | 订单列表 | 占位 |
| 前端页面 | page-account | 用户中心 | 占位 |
| 前端页面 | page-search | 搜索页 | 占位 |
| 前端页面 | page-nav | 导航栏 | 占位 |
| 前端页面 | page-help | 帮助 / 错误页 | 占位 |
| 前端页面 | legal-tos | 服务条款 | 占位 |
| 前端页面 | legal-privacy | 隐私政策 | 占位 |
| 前端页面 | legal-return | 退货政策 | 占位 |
| 前端页面 | legal-cookie | Cookie政策 | 占位 |
| 通知模板 | notif-email | 邮件模板 | 占位 |
| 通知模板 | notif-message | 站内消息 | 占位 |

> **前端页面包含两类多语言内容**：
> - **静态文案**（static_content）：按钮文字、提示语、表单标签等固定 UI 文本，实现方式由技术方案决定（见 §2.2）
> - **业务动态内容**（entity_field）：协议正文、Banner、运营活动文案等，需各自业务模块单独对接翻译服务（见 §3.2）
>
> 占位卡片的 resource_type 为当前规划，各业务模块对接时可能调整。
...
| 废弃字段 | active/inactive → deprecated | 翻译记录保留不删除 | 已有翻译内容仍可正常查询返回 |
| 重新激活 | inactive → active | 系统回扫该字段已有资源，补齐缺失的目标语言翻译 | 恢复正常 |
| 业务模块新增字段写入 | 字段未注册 | 写入被拒绝 | - |

**设计考量**：停用/废弃不删除翻译数据，避免前端页面因字段配置变更而出现内容丢失。业务侧如需彻底清除，通过资源删除接口处理（见 §3.5）。
...
| 各业务模块（商品、协议、Banner 等） | 通过翻译服务 API 写入源语言记录、注册可翻译字段 | 业务模块需按 §3 集成规范完成对接，字段注册是上线前置条件 |
...
| 服务 | 用途 | 关键要求 | 风险 |
|------|------|---------|------|
| AI 翻译服务 | 自动翻译文本内容 | 支持术语约束注入（禁译词/强制术语）；单次翻译响应时间 ≤ 5s；支持批量并发调用 | 服务不可用时翻译功能降级为纯人工，已有内容不受影响 |
...
| 业务模块对接进度延迟 | Translation 导航页卡片长期处于"待对接"状态 | 翻译模块可独立部署运行，不依赖业务模块全部对接完成 |
...
- 多语言管理后台（Translation 导航、翻译详情、Key 管理、术语管理）
...
| V3.1 | 2026-06-23 | 基于 prd-writing skill 自检补全：新增依赖与风险（§八）、版本规划（§九）；补充 translatable_field_config.status 枚举定义、字段停用/废弃处理规则、页面状态变体、端到端数据流示例；明确仅 PC 端后台；完善系统管理员角色描述 |
