...
> - **业务动态内容**（entity_field）：协议正文（服务条款、隐私政策等）、Banner、运营活动文案等由业务模块管理的内容，通过各自业务模块的 API 对接翻译服务，与商品域走相同的集成链路（见 §3.1-§3.3）
>
> 目前已对接的业务模块为商品域（见 §3.2）。协议管理、Banner 配置、通知模板等业务模块尚未设计，待各模块设计时按 §3 的集成规范对接翻译服务。
