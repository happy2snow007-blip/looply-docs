...
> 日期：2026-08-18  
> 状态：全页面操作补充稿，待确认后冻结  
> 适用范围：PC Web 和 Mobile Web；Tablet Web、App 不在本期范围  
> 文档定位：本文件是一方数据采集与埋点的产品需求工作稿，统一回答采集什么、何时成立、在哪里触发，以及存量搜推埋点如何处理。技术实现由开发方案确定。

## 一、背景、目标与范围

### 1.1 背景

Looply 已存在搜推存量事件，以及数据报表需要的一方数据。已有建设服务于不同目标，但缺少一套统一的产品口径，容易出现同一行为被重复记录、同一业务事实使用不同名称或时点、商品与订单无法稳定关联，以及不适合的数据进入普通分析。
...
2. 让同一个业务事实只有一个权威表达，供一方分析使用，并在搜推需要时复用同一事实。
3. 建立可解释的身份、Session、来源、商品、订单和触点关联，支持经营分析、用户旅程、归因和搜推迭代。
4. 保证事件不重复、不漏关键业务信息，不绕过业务模块采集输入原文，也不让敏感数据进入普通分析。

### 1.3 当前范围

- PC Web 和 Mobile Web 的页面访问和有意义交互；Shop、Favorites 独立页面仅适用于 Mobile Web。
- 搜索、推荐、商品浏览与意向、购物车、Checkout、订单、支付、退款、售后和归因链路。
- 一方公共业务信息、身份与一方 Session 口径。
- 一方分析与搜推存量数据的使用边界。
- 存量事件的产品处理原则与产品验收标准。

本期仅覆盖 PC Web 和 Mobile Web。PC Web 不包含独立 Shop、Favorites 页面；Tablet Web 尚未适配，不纳入本期页面与交互验收。App 不在本期范围；当前统一业务事件、公共业务信息和业务标识需支持未来 App 复用。待 Tablet 或 App 产品方案确定后，再补充对应页面及有意义用户操作的采集需求。
...
| 一方数据与搜推存量数据的使用范围和禁止重复原则 | SDK、Nacos、Snowplow及具体配置 |
...
Looply 统一采集记录一方公共上下文和统一业务事件，直接进入一方数据平台。搜推需要使用的用户行为复用同一业务事实；候选集、模型、实验、排序、过滤、耗时和降级等内容继续作为搜推专项遥测。

### 2.2 统一治理原则

1. **一个业务事实只形成一次统一事件。** 一方分析与搜推需要使用同一事实时复用同一记录，不能分别重新定义或重复产生同义事实。
2. **成功事实按真实结果记录。** 点击只表示意图；收藏、加购、Checkout、订单、支付和退款等成功事件必须以对应业务状态真实成立为准。
3. **存量记录与新增采集不得重复。** 搜推已有记录能够表达同一业务事实时直接复用；需要调整时修改现有记录，不新增第二条同义事件。
...
| 一方 Session | `session_id` | 客户端事件必填 | 关联事件发生时所属行为Session；不与登录授权会话混用 |
...
| `cart` | `cart` |
| `checkout` | `checkout`、`payment_success` |
| `auth` | `login`、`sign_up`、`forgot_password`、`verify_code`、`set_password`、`change_password` |
| `account` | `account`、`profile`、`addresses`、`address_edit`、`account_privacy` |
| `order` | `orders`、`order_detail` |
| `returns` | `returns` |
| `content` | `authentication`、`privacy_policy`、`your_privacy_choices`、`terms_of_service`、`accessibility`、`shipping_returns`、`contact_us`、`help`、`about_us` |
...
| 搜索 | `search_id`、`search_term`、`search_term_version`、`trigger_type`、`filters[]`、`sort_type` | 每次正式提交或正式应用筛选、排序、重置时生成一个唯一搜索请求标识；`search_term`由搜索模块输出可用于分析的最终值；`filters[]`记录结构化筛选条件，`sort_type`记录稳定排序值。采集侧不得自行读取或处理输入原文 |
| 推荐 | 推荐请求、模块、稳定展示位、入口和商品位置 | 支持从请求到曝光、点击和成交的关联；商品位置`position`从1开始，无法合法取得时留空并作为数据质量缺口，不使用0或猜测值 |
| 交易 | `checkout_start_event_id`、订单、支付尝试、退款和售后业务标识 | `checkout_start_event_id`引用现有 `checkout_start.event_id`；其余各类事实使用自身稳定业务标识，不相互代替 |
...
10. 一方首次识别是匿名身份生命周期中的派生事实，通过匿名身份、`first_seen_time` 和首次合法触点确定；不主动创建 `first_visit` 业务事件。
...
7. 一方 Session 开始通过 `session_id` 及该 Session 的首个有效事件派生，不主动创建 `session_start` 业务事件。
8. Session 以单个设备／浏览器实例中的连续行为为边界。不同设备或浏览器实例分别产生 `session_id`，即使属于同一用户也不合并为一个 Session；用户层分析可通过确定性身份关系汇总这些 Session。
9. 一方平台直接沿用搜推现有 `checkout_start` 作为 `begin_checkout`。该记录保留发生时的当前 `session_id`，并以自身 `event_id` 作为后续 Checkout 行为与订单事实的关联标识，后文称 `checkout_start_event_id`。后续订单创建和成交事实能够回指时，保留该记录对应的 `origin_session_id`；无法回指时仍保留权威订单和成交，并标记为未关联，不猜测 Session。支付可在 Session 结束后完成，但不另建或延长行为 Session。
...
本章统一事件用于Looply一方数据平台建设。搜推只处理第5.4节列出的存量调整。
...
| 搜索 | `search`｜正式提交搜索 | 用户正式发起或重新提交一次站内搜索 | 搜索轮播词、手动输入后回车或点击搜索按钮、点击输入联想、搜索历史或热门搜索词，以及正式应用筛选、排序或重置时形成一次提交，并生成唯一搜索 ID | 输入变化、建议仅展示、面板内未提交筛选；点击热门品牌或热门Collection直接进入Collection页面，不形成搜索；后续失败不撤销提交事实 | `search_id`、`search_term`、`search_term_version`、`entry_context`、`trigger_type`、`filters[]`、`sort_type`；联想触发时增加`suggestion_type`、`suggestion_position`、`suggestion_object_id` | 九种触发方式均能区分；一次提交只形成一条`search`和一个搜索 ID；筛选条件和排序使用稳定结构化值；`search_term`来自搜索模块正式输出 |
...
| Checkout | `add_shipping_info`｜配送信息就绪 | Checkout 的配送信息和配送方式达到可继续状态 | 与同一条 `checkout_start` 关联的新填写、复用已有或修改已有配送信息，首次校验通过并进入可继续状态 | 选择未保存、校验失败、仅打开区域、同一条 `checkout_start` 下相同就绪状态重复渲染；不得记录姓名、电话、地址明文 | `checkout_start_event_id`、`shipping_tier`、`shipping_info_source`、`items[]` | 每条 `checkout_start` 首次 ready 一次；三种信息来源均覆盖；失败不计成功；无地址或其他 PII；商品快照一致 |
| Checkout | `add_payment_info`｜支付信息就绪 | Checkout 的付款方式达到可提交状态 | 与同一条 `checkout_start` 关联的新填写、复用已有或修改已有支付信息，首次校验通过并进入可提交状态 | 仅选择支付方式、校验失败、同一条 `checkout_start` 下相同就绪状态重复渲染；不得记录卡号、令牌或网关原文 | `checkout_start_event_id`、`payment_type`、`payment_info_source`、`items[]` | 每条 `checkout_start` 首次 ready 一次；三种信息来源均覆盖；selected 与 ready 不混用；失败不计成功；无支付敏感信息 |
| 订单 | `order_created`｜订单创建成功 | 唯一订单及订单明细行成功形成 | 订单业务事务真实成功，订单和订单明细行可使用 | Pay Now点击、页面跳转、Checkout就绪；不能冒充成交 | `order_id`、`session_link_status`、`created_at`、`value`、`currency`、`inventory_lock_expires_at`、`items[]`（每项含`order_item_id`和商品三ID）；能够回指时增加`origin_checkout_start_event_id`和对应的`origin_session_id`并记录`linked`，否则记录`unlinked` | 一个订单一次；无法回指 `checkout_start` 或 Session 不阻止权威订单成立且不得猜测；每个订单行可回指商品；重复提交不形成第二个订单事实 |
...
第五章“必要业务信息”中列出的字段默认必填。`search`自身创建并必填`search_id`，`view_search_results`引用同一`search_id`；每件`view_item_list`逻辑曝光自身创建并必填`exposure_id`。其他事件中的`search_id`、`exposure_id`、`request_id`、`recommendation_request_id`、`source_module_id`和`source_event_id`仅存在对应前序事实时填写。Checkout页面行为、`add_shipping_info`和`add_payment_info`必须引用本次进入结算时的`checkout_start_event_id`；`origin_checkout_start_event_id`和`origin_session_id`仅在订单能够回指对应 `checkout_start` 时填写，无法回指时保留权威订单／成交并填写`session_link_status=unlinked`。其余条件必填规则如下：`user_id`仅登录身份已确定时填写；`previous_page_type`仅存在上一页面时填写；`config_id`仅实际存在运营或配置对象时填写；`result_count`仅搜索成功有结果或成功无结果时填写；`position`仅能够合法取得商品位置时填写，值从1开始，缺失时不得填0或猜测；`failure_type`和`failure_stage`仅失败状态填写；`discount`、`shipping`、`tax`没有对应金额时填0。直达商品详情使用`source_type=direct`，前序来源字段留空；无曝光点击保留点击事实，`exposure_id`留空。除此之外不得缺失或用其他ID、展示文案代替。字段技术类型和嵌套结构由技术方案确定。
...
| `trigger_type` | `carousel_term_search`、`manual_enter`、`manual_search_button`、`suggestion_select`、`history_select`、`popular_term_select`、`filter_apply`、`sort_apply`、`filter_reset` |
| `suggestion_type` | `query`、`designer`、`category` |
| `filter_type` | `price`、`condition`、`brand`、`category`、`color`、`size`、`material` |
| `sort_type` | `recommended`、`newest`、`price_asc`、`price_desc` |
...
每个`filters[]`元素记录`filter_type`和对应的稳定筛选值：枚举型筛选记录稳定值ID；价格筛选记录最小金额、最大金额和币种。未应用筛选时`filters[]`为空，不上传筛选展示文案或完整URL参数。

### 5.4 搜推存量埋点调整

当前搜推事件 ID `search` 实际记录搜索结果，没有记录用户正式提交搜索的动作。现有搜推算法取数链路保持不变，本期新增事件 ID `search_submit` 记录搜索提交，并用同一个 `search_id` 关联两条记录。

| 调整内容 | 事件 ID | 需要怎么改 | 一方统一业务事实 | 不处理的后果 | 优先级 |
|---|---|---|---|---|---|
| 新增搜索提交动作 | `search_submit` | 用户通过建议、热门词、回车或搜索按钮正式提交时发送；为本次提交生成唯一`search_id` | `search` | 只能看到返回过哪些搜索结果，无法准确统计用户实际发起了多少次搜索；失败或取消的搜索意图可能丢失 | P0 |
| 现有埋点增加`search_id` | `search` | 保持现有事件 ID、其他字段和算法取数链路不变；新增`search_id`，取对应`search_submit`生成的同一个值 | `view_search_results` | 搜索提交和搜索结果无法逐次对应，连续搜索或并发返回时可能串错 | P0 |

## 六、页面与业务节点事件触发矩阵

PC Web和Mobile Web中语义相同的操作使用相同事件名、字段和成立条件；布局、控件形态或入口位置变化不产生新的事件。Shop、Favorites独立页面仅按Mobile Web设计验收；Tablet Web不在本期范围。
...
- P14继承P13打开搜索入口时的页面与入口；P15保留同一搜索入口，并将事件页面记录为提交动作实际发生时的`page_type`和`page_id`。成功到达搜索结果页后，由P01另行记录`listing/search_results`页面访问。
...
| P14 | 按搜索入口所在页面 | `search_suggestions` | 当前搜索入口 | `search_suggestions`／`suggestion`、`popular_navigation` | `search_input`、`suggestion_item`／建议业务对象ID；`popular_brand_item`、`popular_collection_item`／目标`collection_id` |
| P15 | 按搜索提交实际发生页面 | `search_submit` | 当前搜索入口 | —／`search` | `search_submit`／`search_id` |
...
| P20 | `home`／`home` | `home_navigation` | `home` | —／`page_navigation` | `back_to_top_button`／`page_top` |
...
| P28 | `listing`／`favorites`、`wishlist`、`recently_viewed` | `saved_product_actions` | 当前页面ID | `saved_products`／当前页面场景 | `product_card`、`wishlist_button`、`explore_button`、`add_more_card`／商品或目标页面ID |
...
| P36 | `checkout`／`checkout` | `checkout_contact` | `checkout` | —／`checkout_contact` | `marketing_subscription_checkbox`／`checkout_start_event_id` |
| P37 | `checkout`／`checkout` | `checkout_shipping` | `checkout` | —／`checkout_shipping` | `shipping_form`、`shipping_tier_option`／`checkout_start_event_id` |
| P38 | `checkout`／`checkout` | `checkout_payment` | `checkout` | —／`checkout_payment` | `payment_method_option`／`checkout_start_event_id` |
| P39 | `checkout`／`checkout` | `checkout_coupon` | `checkout` | —／`checkout_coupon` | `coupon_apply_button`／`checkout_start_event_id` |
| P40 | `checkout`／`checkout` | `checkout_submit` | `checkout` | —／`checkout_submit` | `pay_now_button`／`checkout_start_event_id` |
...
| P50 | `product`／`product_detail` | `product_actions` | `product_detail` | —／`product_share` | `share_button`／`listing_public_code` |
| P51 | `content`／`contact_us` | `contact_form` | `contact_us` | —／`contact_form` | `contact_submit_button`／`contact_form` |
| P52 | 按当前页面映射 | `global_header`、`footer` | `pc_header`、`mobile_header`、`footer` | —／`global_navigation` | Logo、导航、More、Market／Language、Account、Favorites、Cart、Newsletter、Social控件／实际目标ID |
| P53 | `auth`／当前认证步骤 | `auth_recovery`、`auth_legal`、`auth_consent_modal` | 触发认证的页面入口 | —／`authentication` | 记忆账号、找回密码、验证码、密码提交、返回、法律链接、同意更新控件／实际步骤或目标页面 |
| P54 | `product`／`product_detail` | `product_header`、`product_gallery`、`product_information`、`product_recommendations`、`similar_products_entry` | `product_detail` | 商品图片或推荐场景 | 返回、购物袋、图片预览、信息弹层、推荐轨道、View all、Find More控件／实际目标 |
| P55 | `order`／`orders`、`order_detail` | `order_search`、`order_refund_modal`、`order_support`、`order_detail_header` | 当前订单页面 | `orders`／`order_search` | 搜索、退款说明、客服、返回控件／查询、订单或目标页面 |
| P56 | `returns`／`returns` | `returns_list`、`returns_application`、`returns_information`、`returns_status` | `returns` | `return_cases`、`return_items`／实际售后场景 | 筛选、售后单、商品、原因、凭证、编辑、重提、取消、说明和状态按钮／订单或售后对象 |
| P57 | `order`／`order_detail` | `delivery_tracking` | `order_detail` | —／`delivery_tracking` | 物流详情打开／关闭控件／`shipment_id` |
| P58 | `cart`／`cart` | `cart_items`、`cart_empty_state`、`cart_coupon`、`cart_authentication`、`cart_unavailable_items` | `cart` | `cart_items`／实际购物袋场景 | 数量、空态、优惠、认证、失效商品控件／商品、购物袋或目标页面 |
| P59 | `checkout`／`checkout`、`payment_success` | `checkout_delivery_address`、`checkout_shipping`、`checkout_payment`、`payment_success` | `checkout` | —／实际Checkout步骤 | 地址、Phone说明、安全码说明、成功页按钮／地址对象、Checkout或目标页面 |
| P60 | `account`／`account`、`profile`、`addresses`、`address_edit`、`account_privacy` | `account_orders`、`account_navigation`、`account_profile`、`account_preferences`、`account_privacy`、`account_authentication` | 当前Account页面 | `account_sections`／实际账户场景 | 订单状态、账户菜单、资料、偏好、隐私、登出及认证入口控件／实际目标 |
| P61 | `content`／`contact_us` | `contact_information`、`contact_footer` | `contact_us` | —／`contact_navigation` | 联系邮箱、Your Privacy Choices入口／站外或站内目标 |
| P62 | `home`／`home`、`content`／`authentication` | `home_authentication`、`authentication_guarantee` | `home`、`authentication` | —／`authentication_navigation` | Explore Details、See Guarantee Details控件／`authentication`或`shipping_returns` |

公共导航目标使用封闭稳定枚举。PC Header首期至少包括`home`、`handbags`、`shoes`、`brands`、`jewelry`、`watches`、`designers`、`account`、`favorites`、`cart`及实际More菜单项；Mobile底部导航包括`home`、`shop`、`favorites`、`account`。Footer首期至少包括`handbags`、`jewelry`、`watches`、`accessories`、`brands`、`account`、`shipping_returns`、`contact_us`、`about_us`、`authentication`、`accessibility`、`privacy_policy`、`your_privacy_choices`、`terms_of_service`及已上线社交平台。显示文案或大小写变化不得生成新ID；未上线入口不得预报事件。

### 6.2 站内找货入口映射

本节定义报表所需的稳定入口分类与原始事件上下文之间的映射。`entry_type`由分析环节根据原始事件字段派生，客户端不新增上报该字段。原始事件继续按实际场景记录`page_id`、`module_id`、`scene_id`、`entry_context`及对应关联ID。

| 报表入口 | `entry_type` | 页面 | `module_id` | `scene_id`／细分场景 | 关键关联信息 | 归类规则 |
|---|---|---|---|---|---|---|
| 首页 Feed | `home_feed` | `home` | `home_product_feed` | `for_you`、`new_arrivals`等实际Feed场景 | `recommendation_request_id`、`placement_id`、`exposure_id` | 首页Feed商品曝光、点击及后续意向归入本入口；不同Tab继续按`scene_id`下钻 |
| Collection | `collection` | `collection` | `collection_product_list` | `collection` | `collection_id`、`placement_id`、`exposure_id` | 用户在具体Collection中产生的商品曝光、点击及后续意向归入本入口 |
| 商详 You May Also Like | `product_recommendation` | `product_detail` | `product_recommendations` | `you_may_also_like` | `recommendation_request_id`、`placement_id`、`exposure_id` | 仅包含You May Also Like，不与Recently Viewed混合 |
| 搜索热词 | `search_popular_term` | 搜索提交实际发生页面＋`search_results` | 提交：`search_submit`；结果：`search_results_list` | `search` | `trigger_type=popular_term_select`、`search_id`、`exposure_id` | 点击热词本身不算商品曝光；只有通过同一`search_id`进入结果页后实际曝光的商品才归入本入口 |

以下场景继续保留自己的页面、模块和场景标识，但不纳入首期四类报表入口：商详Recently Viewed、搜索无结果推荐、普通搜索结果、Favorites／Wishlist／Recently Viewed列表、Shop类目或品牌入口。没有可关联前序入口的商品详情访问保留原始直达事实，不强行归入上述四类。

### 6.3 `ui_interaction`稳定命名
...
| P04 | `home_banner` | `banner` | `select` | `banner_item` | 目标页面ID |
...
| P11 | `collection_list_controls` | `filter_sort` | `close_filter` | `filter_control` | 筛选面板ID |
| P11 | `collection_list_controls` | `filter_group` | `expand_filter_group` | `filter_group` | 筛选分组ID |
| P11 | `collection_list_controls` | `filter_group` | `collapse_filter_group` | `filter_group` | 筛选分组ID |
| P11 | `collection_list_controls` | `filter_group` | `view_all_filter_options` | `filter_group` | 筛选分组ID |
| P11 | `collection_list_controls` | `filter_sort` | `open_sort` | `sort_control` | 排序面板ID |
| P11 | `collection_list_controls` | `filter_sort` | `close_sort` | `sort_control` | 排序面板ID |
...
| P17 | `search_list_controls` | `filter_sort` | `close_filter` | `filter_control` | 筛选面板ID |
| P17 | `search_list_controls` | `filter_group` | `expand_filter_group` | `filter_group` | 筛选分组ID |
| P17 | `search_list_controls` | `filter_group` | `collapse_filter_group` | `filter_group` | 筛选分组ID |
| P17 | `search_list_controls` | `filter_group` | `view_all_filter_options` | `filter_group` | 筛选分组ID |
| P17 | `search_list_controls` | `filter_sort` | `open_sort` | `sort_control` | 排序面板ID |
| P17 | `search_list_controls` | `filter_sort` | `close_sort` | `sort_control` | 排序面板ID |
...
| P14 | `search_suggestions` | `popular_navigation` | `select_popular_brand` | `popular_brand_item` | 目标`collection_id` |
| P14 | `search_suggestions` | `popular_navigation` | `select_popular_collection` | `popular_collection_item` | 目标`collection_id` |
| P20 | `home_navigation` | `page_navigation` | `back_to_top` | `back_to_top_button` | `page_top` |
...
| P28 | `saved_product_actions` | `saved_items_navigation` | `select_add_more` | `add_more_card` | 实际目标`page_id` |
| P30 | `cart_coupon` | `coupon` | `apply` | `coupon_apply_button` | `cart_id` |
| P39 | `checkout_coupon` | `coupon` | `apply` | `coupon_apply_button` | `checkout_start_event_id` |
| P39 | `checkout_coupon` | `coupon` | `remove` | `coupon_remove_button` | `checkout_start_event_id` |
...
| P31 | `cart_selection` | `cart_selection` | `unselect_all` | `select_all_checkbox` | `cart_id` |
...
| P36 | `checkout_contact` | `marketing_subscription` | `change` | `marketing_subscription_checkbox` | `checkout_start_event_id` |
| P38 | `checkout_payment` | `payment_method` | `select` | `payment_method_option` | `checkout_start_event_id` |
| P40 | `checkout_submit` | `pay_now` | `submit` | `pay_now_button` | `checkout_start_event_id` |
...
| P50 | `product_actions` | `product_share` | `select` | `share_button` | `listing_public_code` |
| P51 | `contact_form` | `contact_form` | `submit` | `contact_submit_button` | `contact_form` |
| P52 | `global_header` | `header_navigation` | `select` | `home_logo`、`header_nav_item`、`account_entry`、`favorites_entry`、`cart_entry` | 实际目标ID |
| P52 | `global_header` | `header_more_navigation` | `open`、`close`、`select` | `more_navigation_button`或`more_nav_item` | More面板或实际目标ID |
| P52 | `global_header` | `market_language` | `open`、`close`、`select_market`、`select_language` | `market_language_control` | Market或Language稳定值 |
| P52 | `footer` | `newsletter_subscription` | `submit` | `subscribe_button` | `newsletter` |
| P52 | `footer` | `footer_social_navigation` | `select` | `social_link` | `facebook`、`tiktok`、`instagram`、`youtube` |
| P53 | `auth_form` | `remembered_login` | `continue`、`switch_account` | `remembered_account`或`other_account` | 当前认证步骤 |
| P53 | `auth_recovery` | `password_recovery` | `start`、`submit_password`、`back` | 找回密码、密码提交或返回控件 | `forgot_password`、`set_password`、`change_password`或目标认证页 |
| P53 | `auth_recovery` | `verification_code` | `send`、`resend`、`submit` | 验证码控件 | `verification_code` |
| P53 | `auth_legal` | `legal_navigation` | `select` | `terms_link`或`privacy_link` | `terms_of_service`或`privacy_policy` |
| P53 | `auth_consent_modal` | `consent_update` | `confirm`、`close` | 同意更新控件 | 同意版本ID |
| P53 | `auth_blocked_state` | `blocked_state_action` | `back`、`close`、`contact_support` | 实际阻断页控件 | 目标认证页或`contact_us` |
| P54 | `product_header` | `product_navigation` | `back`、`select_cart` | `back_button`或`cart_entry` | 上一页面、首页或`cart` |
| P54 | `product_gallery` | `product_image_preview` | `open`、`close`、`zoom_in`、`zoom_out` | 主图、关闭或缩放控件 | 图片位置ID |
| P54 | `product_information` | `product_info_modal` | `open`、`close` | 信息入口或关闭控件 | `certified_authentic`、`condition_guide`、`size_guide` |
| P54 | `product_recommendations` | `recommendation_rail` | `scroll`、`previous`、`next`、`view_all` | 推荐轨道或导航控件 | 推荐模块ID或`recently_viewed` |
| P54 | `similar_products_entry` | `similar_products_navigation` | `select` | `find_more_button` | 相似商品目标页 |
| P50 | `product_actions` | `product_share` | `close`、`select_channel`、`copy_link`、`open_system_share` | 分享弹层对应控件 | 分享渠道枚举或`listing_public_code` |
| P55 | `order_search` | `order_search` | `open`、`close`、`submit`、`auto_query`、`select_result` | 搜索入口、提交或结果项 | `order_search`或`order_id` |
| P55 | `order_refund_modal` | `refund_information` | `open`、`close` | Refund说明控件 | `order_id` |
| P55 | `order_support` | `support_navigation` | `select` | `contact_entry` | `contact_us` |
| P55 | `order_detail_header` | `order_navigation` | `back` | `back_button` | `orders`或上一页面 |
| P56 | `returns_list` | `returns_filter` | `select_order`、`clear`、`select_return_case` | 筛选控件或售后单 | `order_id`或`return_application_id` |
| P56 | `returns_application` | `return_application` | `unselect_item`、`edit`、`resubmit` | 商品或申请控件 | `order_item_id`或`return_application_id` |
| P56 | `returns_application` | `return_reason` | `open`、`close` | `return_reason_control` | `return_application` |
| P56 | `returns_application` | `return_evidence` | `upload`、`delete`、`replace` | 凭证控件 | `return_application` |
| P56 | `returns_application` | `return_application` | `open_cancel`、`confirm_cancel`、`close_cancel` | 取消控件 | `return_application_id` |
| P56 | `returns_information` | `refund_fee_information` | `open`、`close` | 退款扣费说明控件 | `return_application_id` |
| P56 | `returns_information` | `return_shipping_information` | `open`、`close` | 寄回说明控件 | `return_application_id` |
| P56 | `returns_status` | `return_status_action` | `back` | `back_button` | `returns`或上一页面 |
| P56 | `returns_status` | `return_status_action` | `contact_support` | `support_button` | `contact_us` |
| P56 | `returns_status` | `return_status_action` | `update_request` | `update_request_button` | `return_application_id` |
| P56 | `returns_status` | `return_status_action` | `open_cancel_request` | `cancel_request_button` | `return_application_id` |
| P56 | `returns_status` | `return_status_action` | `copy_return_tracking` | `return_tracking_copy_button` | `shipment_id` |
| P56 | `returns_status` | `return_status_action` | `view_return_tracking` | `return_tracking_item` | `shipment_id` |
| P56 | `returns_information` | `return_shipping_information` | `open`、`close` | `how_to_ship_control`或`information_close_button` | `return_application_id` |
| P56 | `returns_information` | `refund_fee_information` | `open`、`close` | `return_deduction_control`或`information_close_button` | `return_application_id` |
| P57 | `delivery_tracking` | `tracking_details` | `open`、`close` | `tracking_details_entry`或`tracking_details_close` | `shipment_id` |
| P58 | `cart_items` | `cart_quantity` | `increase`、`decrease` | `quantity_control` | `listing_public_code` |
| P58 | `cart_empty_state` | `continue_shopping` | `select` | `continue_shopping_button` | 实际目标`page_id` |
| P30 | `cart_coupon` | `coupon` | `remove` | `coupon_remove_button` | `cart_id` |
| P58 | `cart_authentication` | `authentication_entry` | `select` | 登录或继续控件 | `login`或实际目标 |
| P58 | `cart_unavailable_items` | `unavailable_item_action` | `clear_unavailable_items` | `clear_all_button` | `cart_id` |
| P59 | `checkout_delivery_address` | `delivery_address` | `add`、`edit`、`select`、`save` | 地址控件 | `address_id`或`new_address` |
| P59 | `checkout_shipping` | `phone_information` | `open`、`close` | `phone_information_control` | `checkout_start_event_id` |
| P59 | `checkout_payment` | `security_code_information` | `open`、`close` | 安全码说明控件 | `checkout_start_event_id` |
| P59 | `payment_success` | `payment_success_action` | `copy_order_id` | `order_id_copy_button` | `order_id` |
| P59 | `payment_success` | `payment_success_navigation` | `continue_shopping` | `continue_shopping_button` | `shop` |
| P60 | `account_orders` | `order_status_entry` | `select` | 订单状态入口 | `processing`、`shipped`、`delivered`、`refunds` |
| P60 | `account_navigation` | `account_section` | `select` | 账户菜单项 | `profile`、`addresses`、`policies`、`account_privacy`等实际目标 |
| P60 | `account_profile` | `account_profile` | `edit`、`save`、`back` | 资料或地址控件 | `nickname`、`gender`、地址对象或目标页面 |
| P60 | `account_profile` | `account_profile` | `open_delete`、`confirm_delete`、`cancel_delete` | 地址删除控件 | `address_id` |
| P60 | `account_preferences` | `preference` | `open`、`select`、`close` | 偏好控件 | `country_region`、`language`、`currency` |
| P60 | `account_preferences` | `subscription` | `subscribe`、`unsubscribe` | 订阅控件 | 订阅类别ID |
| P60 | `account_privacy` | `privacy_action` | `open_download_data`、`open_delete_account`、`open_privacy_preferences` | 隐私功能入口 | 实际目标ID |
| P60 | `account_authentication` | `logout` | `open`、`confirm`、`cancel` | 登出控件 | `logout` |
| P60 | `account_authentication` | `authentication_entry` | `login`、`register` | 登录或注册入口 | `login`或`sign_up` |
| P61 | `contact_information` | `email_contact` | `select` | `contact_email_link` | `email_client` |
| P61 | `contact_footer` | `policy_navigation` | `select` | `your_privacy_choices_link` | `your_privacy_choices` |
| P62 | `home_authentication` | `authentication_entry` | `select` | `explore_details_button` | `authentication` |
| P62 | `authentication_guarantee` | `guarantee_navigation` | `select` | `guarantee_details_link` | `shipping_returns` |
...
| P14 `select_popular_brand`、`select_popular_collection` | `collection_id`、`position` | 点击热门品牌或热门Collection入口时必填；位置从1开始 |
| P21 `change` | `previous_position`、`selected_position` | 图片实际切换时必填，位置从1开始 |
| P50 `select` | 商品三ID、`result_state` | 用户主动点击Share时必填；不记录分享内容或外部账号信息 |
| P50 `close`、`select_channel`、`copy_link`、`open_system_share` | 商品三ID、`result_state`；选择渠道时增加`share_channel` | `share_channel`仅取UI实际提供的`whatsapp`、`message`、`facebook`、`pinterest`、`x`、`messenger`等稳定枚举；不记录分享内容、联系人、外部账号或外部应用最终行为 |
...
| P40 `submit` | `checkout_start_event_id`、`items[]` | 提交 Checkout 时必填；每项包含商品三ID、数量和事件时价格 |
...
| P51 `submit` | `result_state`、明确失败时的`failure_type` | 只记录`success`、`validation_failed`、`failed`等低基数结果；不得记录姓名、邮箱、消息正文或自由错误文本 |
| P52 `select_market`、`select_language` | `preference_type`、`previous_value`、`selected_value` | 选择实际变化时必填；使用稳定枚举，不记录自由文本 |
| P52 `submit` | `result_state`、明确失败时的`failure_type` | Newsletter提交必填；不得记录邮箱原文 |
| P53 验证码／密码提交／同意更新 | `auth_step`、`result_state`、明确失败时的`failure_type`；同意更新另带`consent_version` | 不记录账号、验证码、密码或弹窗文案原文 |
| P54 图片预览／缩放 | `previous_position`、`selected_position`、`result_state` | 图片位置可合法取得时填写；位置从1开始 |
| P55 搜索查询 | `result_state`、明确失败时的`failure_type` | 不记录订单查询输入原文；选择结果时填写`order_id` |
| P56 凭证操作 | `evidence_count`、`result_state` | 只记录数量和低基数结果，不记录图片、文件名、URL或自由说明 |
| P57 `open`、`close` | `order_id`、`shipment_id`、`result_state` | 物流对象合法取得时必填 |
| P58 `increase`、`decrease` | 商品三ID、`quantity_before`、`quantity_after`、`result_state` | 数量实际变化时必填；删除至0继续使用`remove_from_cart` |
| P58 `clear_unavailable_items` | `cart_id`、`affected_item_count`、`result_state` | 点击Clear all时必填；实际移除成功的商品继续按`remove_from_cart`记录，不把点击冒充移除成功 |
| P59 地址操作 | `checkout_start_event_id`、`address_info_source`、`result_state` | `address_info_source`取`new`、`existing`、`modified`；不记录地址明文 |
| P59 `copy_order_id`、`continue_shopping` | `order_id`、`result_state`；继续购物另带目标`page_id` | 仅支付成功页明确操作时填写；不得再次生成`purchase` |
| P60 偏好／订阅 | `preference_type`或`subscription_type`、合法前后值、`result_state` | 对应操作发生时必填；不记录自由输入或敏感值 |
...
| P03 | 通用失败态 | 点击 Retry；用户主动发起一次重试 | 每次主动重试成立；同一次操作的重复界面反馈不形成新动作；点击不代表重试成功 | `ui_interaction` | 原操作、页面／模块、业务请求、重试结果；Favorites至少区分Wishlist、Recently Viewed和两模块组合加载 | 可区分重试意图与后续成功／空／失败结果 |
...
| P14 | Search 输入与建议 | 输入字符；展示建议／Popular；选择联想；点击热门品牌或热门Collection入口 | 普通建议展示不记录；选择联想后只形成一次正式搜索提交；热门品牌和热门Collection点击记录入口选择，成功到达目标Collection后另记`page_view`；采集侧不读取输入原文 | 联想提交走P15；热门品牌／Collection入口使用`ui_interaction`，不产生`search` | 联想提交时由P15记录建议类型、位置及稳定业务对象ID；热门品牌／Collection记录目标`collection_id`和位置 | 输入和建议展示不增加事件；联想选择不与搜索提交双记；热门品牌／Collection可回溯入口且不计入搜索次数 |
| P15 | Search 正式提交 | 搜索轮播词、手动输入后回车或点击搜索按钮、点击输入联想、搜索历史或热门搜索词 | 按5.1 `search`；事件页面为提交动作实际发生页面 | `search` | 搜索入口、提交页面、`trigger_type`及对应来源信息；联想触发增加建议上下文 | 六种首次提交方式均可区分；一次提交只形成一条`search`；未到达结果页时不把事件页面写成`search_results` |
| P16 | Search 结果 | 搜索请求进入唯一结果终态；结果商品达到统一曝光条件 | 结果终态按5.1 `view_search_results`；结果商品曝光按5.1 `view_item_list`，并携带本次搜索的`search_id` | `view_search_results`；`view_item_list` | 搜索结果状态、搜索版本；曝光商品、位置、模块及`search_id` | 一次请求只有一个终态；搜索结果商品曝光可回指对应搜索提交 |
| P17 | Search 筛选／排序 | 打开、选择、Reset、Apply、Sort；用户形成新的搜索请求及最终结果 | 正式应用筛选、排序或重置后形成新`search_id`；面板内临时选择不提前提交 | 面板动作用 `ui_interaction`；正式请求复用 `search`＋`view_search_results` | `search_id`、`search_term`、`trigger_type`、`filters[]`、`sort_type`及结果 | `filter_apply`、`sort_apply`、`filter_reset`可区分；具体筛选条件和排序可还原；一次正式请求对应一组搜索提交与结果终态 |
...
| P20 | Mobile 首页 Back to Top | 用户主动点击返回顶部按钮 | 每次明确点击成立；自动滚动、页面初始化或程序调整滚动位置不成立 | `ui_interaction` | 页面、模块、按钮和结果 | 可区分用户主动返回顶部与普通滚动；不重复记录内部图标点击 |
...
| P25 | Product Buy Now | 通过 Buy Now 进入 Checkout | 复用搜推现有 `checkout_start`，成立条件按5.3 `begin_checkout` | `begin_checkout` | `source_type=buy_now`及现有 `checkout_start` 记录 | 不补记加购，不新增第二条开始结算事件 |
...
| P28 | Mobile Favorites 商品与入口 | 点商品、取消收藏、点击 Explore Items或Add More Card；商品选择、收藏取消或进入目标页面 | 每个真实结果复用对应事实；Explore Items和Add More Card只记录入口选择，目标页面成功到达后另记`page_view`，不冒充推荐曝光 | 复用 `select_item`／`remove_from_wishlist`；入口导航用 `ui_interaction` | 页面、入口、商品三 ID、推荐场景、实际目标`page_id`和结果 | 同一结果不增加平级成功事件；Explore Items与Add More Card可区分且来源可回溯 |
...
| P35 | Cart Check Out | 从购物车进入 Checkout | 复用搜推现有 `checkout_start`，按第5.3节`begin_checkout` | `begin_checkout` | 现有 `checkout_start` 记录及购物车 | 按第5.3节验收，不新增第二条开始结算事件 |
| P36 | Checkout 联系方式 | 切换营销订阅；用户选择变化；保存／生效是另一业务事实 | 选择状态真实改变；不带邮箱；点击不冒充偏好已生效；营销订阅不等于Analytics选择 | `ui_interaction` | `checkout_start_event_id`、`subscription_selected` | 选择与生效分开；不与隐私或Analytics选择混用 |
| P37 | Checkout 配送 | 配送信息进入就绪状态 | 按第5.3节`add_shipping_info` | `add_shipping_info` | `checkout_start_event_id`、配送选项及`shipping_info_source` | 新填写、复用已有和修改已有信息均按第5.3节验收 |
| P38 | Checkout 支付方式 | 选择方式；付款信息校验；selected是选择行为；ready是付款信息就绪事实 | 选择不冒充ready；校验失败不记成功；不记录卡号、支付凭据或网关原文 | 选择用 `ui_interaction`；ready用 `add_payment_info` | `checkout_start_event_id`、`payment_type`、`payment_info_source` | 选择与ready分开；三种信息来源均覆盖；`payment_type`使用第五章枚举，只有实际可用的方式才上报 |
| P39 | Checkout 优惠 | 输入并 Apply；applied／invalid／failed及金额变化 | 一次Apply一个`coupon_result`；不保存优惠码原文；金额以Checkout结果为准 | `ui_interaction`；与Cart优惠使用同一优惠动作语义，以页面和业务对象区分 | `checkout_start_event_id`、`coupon_result`、`discount_value`、`value_before`、`value_after` | 三种结果可区分；失败不记成功优惠 |
| P40 | Checkout Pay Now | 点击并提交当前 Checkout；提交付款意图；不是订单、支付或成交 | 提交动作成立才记录；失败保留失败结果；重复点击可为多次意图，但不能产生多个业务订单事实 | `ui_interaction`中的稳定 Pay Now动作；不设独立`pay_now_click`事件 | `checkout_start_event_id`、提交时间、提交结果、商品三 ID | 点击、提交失败、订单创建、支付开始、支付成功五者可区分 |
...
| P50 | Product Share | 用户主动点击商详Share按钮 | 每次明确点击成立；分享面板展示、取消或外部分享结果不得冒充已分享成功 | `ui_interaction` | 商品三ID、页面、按钮和低基数结果 | 点击意图与后续外部结果分开；不采集分享内容、联系人或外部账号信息 |
| P51 | Contact Us 表单提交 | 用户点击提交并进入成功、校验失败或提交失败结果 | 一次提交形成一次操作；输入变化、聚焦和逐字段校验不单独形成业务事件 | `ui_interaction` | 页面、表单、提交结果和低基数失败分类 | `success`、`validation_failed`、`failed`可区分；不采集姓名、邮箱、消息正文或自由错误文本 |
| P52 | Header／Footer公共入口 | 点击Logo、顶部导航、More、Market／Language、Account、Favorites、Cart、Footer链接、Newsletter或Social入口 | 只记录用户明确操作；目标页成功另记P01；菜单打开／关闭与选择分开 | `ui_interaction` | 当前页面、入口ID、目标、前后偏好值和结果 | 公共入口逐项可定位；Newsletter不含邮箱，Social不含外部账号信息 |
| P53 | 登录注册辅助流程 | 记忆账号继续／切换、找回密码、验证码发送／重发／提交、设置／修改密码、返回、法律链接、同意更新 | 操作成立才记录；认证成功仍只由C13／C14表达；不记录账号、验证码或密码 | `ui_interaction`＋目标页P01 | 当前认证步骤、稳定动作、目标步骤、结果和低基数失败分类 | 所有辅助动作可区分且不冒充登录／注册成功 |
| P54 | 商详辅助交互 | 返回、进入购物袋、图片预览／关闭／缩放、说明弹层、推荐轨道、View all、Find More | 仅状态真实变化或导航选择成立；推荐商品曝光／点击仍复用统一商品事件 | `ui_interaction`＋目标页P01 | 商品三ID、图片位置、说明类型、推荐模块、实际目标和结果 | 图片、说明、推荐导航互不混用；不新增第二条商品点击事实 |
| P55 | 订单搜索与辅助入口 | 打开／关闭订单搜索、提交／自动查询、选结果、Refund说明、Contact、返回 | 查询文本不采集；订单状态和退款事实继续读取业务表 | `ui_interaction`＋目标页P01 | 订单ID、稳定动作、查询结果状态、目标和结果 | 查询、说明、客服、返回可独立验收 |
| P56 | Returns完整操作 | 按订单筛选、选择售后单、取消选品、原因弹层、凭证、编辑／重提、取消确认、扣费／寄回说明；状态页返回、联系客服、Update My Request、Cancel Request、复制／查看寄回物流 | 页面动作不冒充售后业务状态；图片和自由说明内容不采集；不存在操作按钮的状态仅展示，不生成用户事件 | `ui_interaction`＋目标页P01 | 订单／订单行／售后对象、稳定动作、低基数结果 | Figma各状态页可执行控件均有稳定映射；受理、状态、结案和退款继续读取权威表 |
| P57 | Delivery物流详情 | 打开／关闭物流详情弹层 | 打开或关闭状态真实变化才记录；轨迹滚动和物流状态展示不记录用户事件 | `ui_interaction` | `order_id`、`shipment_id`、动作和结果 | 用户查看动作与物流业务状态分开 |
| P58 | Shopping Bag辅助操作 | 数量增减、空态Continue Shopping、移除优惠、未登录入口；失效商品区点击Clear all | 状态真实变化或明确导航成立；优惠码原文不采集；Clear all点击记录一次操作，实际移除结果按受影响商品复用`remove_from_cart` | `ui_interaction`或既有业务事件 | 购物袋、商品三ID、前后数量、优惠结果、受影响失效商品数量、目标和结果 | 辅助操作可区分且不重复核心购物车事件 |
| P59 | Checkout辅助操作 | 地址新增／编辑／选择／保存、Phone／安全码说明；支付成功页复制订单号、Continue Shopping | 选择动作与`add_shipping_info`／`add_payment_info`就绪事实分开；订单支付结果由服务端提供；成功页复制与导航不重复产生`purchase` | `ui_interaction`＋目标页P01 | `checkout_start_event_id`、地址内部对象ID、稳定动作、`order_id`和目标页面 | 新增／复用信息、最终就绪事实及成功页动作均可区分；不采集地址或支付敏感信息 |
| P60 | Account完整操作 | 订单状态入口、账户菜单、Profile／地址、偏好、订阅、隐私入口、登出确认及游客登录／注册入口 | 页面操作与业务最终状态分开；设计稿中的隐私功能只作为待实现UI，不声明已上线 | `ui_interaction`＋目标页P01 | 稳定菜单／目标ID、内部对象ID、前后偏好值、结果 | 所有账户入口可定位；不记录个人资料原文 |
| P61 | Contact Us辅助入口 | 点击联系邮箱或Your Privacy Choices | 明确点击成立；目标页成功另记P01；不记录邮箱地址、邮件内容 | `ui_interaction`＋目标页P01 | 当前页面、稳定入口、目标和结果 | 表单提交仍用P51，辅助入口不与提交重复 |
| P62 | Authentication内容入口 | 首页点击Explore Details进入Authentication；Authentication页点击See Guarantee Details | 明确选择成立；目标页成功另记P01；内容展示和普通滚动不记操作 | `ui_interaction`＋目标页P01 | 来源页面、稳定入口、目标页面和结果 | 首页入口和保证详情入口可分别定位，不把页面阅读冒充点击 |

Favorites中的Price Drop是商品展示状态，不单独形成点击事件。Loading、空态以及Wishlist／Recently Viewed的分模块或组合失败是页面状态，只有用户实际点击Retry、Explore Items或Add More Card时才按对应节点记录操作。当前Figma 1.1未确认Recently Viewed删除或View All控件，本期不增加对应埋点。
...
一方平台接收本方案第三章定义的公共业务信息、第四章定义的身份与 Session 结果，以及第五章定义的统一业务事件。字段含义和成立规则分别以对应章节为准，本章不重复定义。平台防重复统一遵守第2.2节。


## 八、搜推存量埋点调整边界

搜推存量埋点以第5.4节为准。存量迁移遵守第2.2节；搜推专项数据不得被统计为普通用户行为。

## 九、产品验收标准

| 维度 | 通过标准 |
|---|---|
| 页面覆盖 | PC Web和Mobile Web所有正式有意义交互均能在覆盖矩阵中找到唯一业务事实和事件；Shop、Favorites独立页面仅按Mobile Web验收，Tablet Web不纳入本期 |
| 触发准确 | 按第五章各事件的成立条件、不计入边界和验收标准执行；页面矩阵中的操作不得改变统一事件含义 |
| 不重不漏 | 按第2.2节的防重复原则验收 |
| 商品关联 | 单商品行为三个商品 ID 完整；订单行另有 `order_item_id`；同一业务对象在行为、订单和退款链路中可以稳定关联 |
| 身份与 Session | 按第四章验收Session与身份串联；能回指的成交使用`origin_session_id`，不能回指的权威成交保留并标记未关联，不猜测Session |
| 数据最小化 | 搜索输入原文、身份原文、支付凭据和敏感自由文本不得进入普通分析；各事件按第三章和第五章验收 |
