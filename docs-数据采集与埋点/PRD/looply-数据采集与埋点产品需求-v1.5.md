...
本章统一事件用于Looply一方数据平台建设。GA4变更独立维护在《Looply GA4首版变更清单》，搜推只处理第5.4节列出的存量调整；广告平台已完成接入，不在本次开发范围。
...
GA4首版变更以《Looply GA4首版变更清单》为准；该清单与旧《Looply GA4数据分析 PRD v1.3》的适用关系也以清单声明为准。搜推存量埋点以第5.4节为准。广告平台已完成接入，本次不调整。存量迁移遵守第2.2节；搜推专项数据不得被统计为普通用户行为。
...
| 数据最小化 | 搜索输入原文、身份原文、支付凭据和敏感自由文本不得进入普通分析；各事件和 GA4 的禁止信息分别按第三、五章及《Looply GA4首版变更清单》验收 |
