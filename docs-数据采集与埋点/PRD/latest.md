...
10. 一方平台直接沿用搜推现有 `checkout_start` 作为 `begin_checkout`。该记录保留发生时的当前 `session_id`，并以自身 `event_id` 作为后续 Checkout 行为与订单事实的关联标识，后文称 `checkout_start_event_id`。后续订单创建和成交事实能够回指时，保留该记录对应的 `origin_session_id`；无法回指时仍保留权威订单和成交，并标记为未关联，不猜测 Session。支付可在 Session 结束后完成，但不另建或延长行为 Session。
...
本章统一事件用于Looply一方数据平台建设。GA4变更独立维护在《Looply GA4埋点变更清单》，搜推只处理第5.4节列出的存量调整；广告平台已完成接入，不在本次开发范围。
...
GA4埋点变更以《Looply GA4埋点变更清单》为准；该清单与旧《Looply GA4数据分析 PRD v1.3》的适用关系也以清单声明为准。搜推存量埋点以第5.4节为准。广告平台已完成接入，本次不调整。存量迁移遵守第2.2节；搜推专项数据不得被统计为普通用户行为。
...
| 数据最小化 | 搜索输入原文、身份原文、支付凭据和敏感自由文本不得进入普通分析；各事件和 GA4 的禁止信息分别按第三、五章及《Looply GA4埋点变更清单》验收 |
