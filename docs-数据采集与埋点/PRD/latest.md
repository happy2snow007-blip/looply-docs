...
| PC＋Mobile／搜索结果 | `search_list_controls` | `search_filter` | `open` → `filter_control` → `search_filter`；`close` → `filter_close` → `search_filter`；`expand_group`／`collapse_group`／`view_all_options` → `filter_group` → 筛选组ID；`select` → `filter_option` → 筛选项稳定值；`reset` → `filter_reset` → `search_results`；`apply` → `filter_apply` → `search_results` |
