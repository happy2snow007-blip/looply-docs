...
| 业务域 | 统一事件名／业务事实 | 业务含义 | 成立条件 | 不计入边界 | 必要业务信息 | 一方平台 | GA4 | 广告 | 搜推 | 产品验收标准 |
|---|---|---|---|---|---|---|---|---|---|---|
| 页面 | `page_view`｜页面访问 | 用户实际进入一个新的页面实例 | 首次打开、有效路由切换或刷新并实际到达页面；业务内容成功、空或失败不改变页面访问事实 | 链接预加载、页面预取、未完成跳转、重复路由回调、组件重渲染、未实际到达的中间重定向页 | `page_type`、`page_id`、`page_version`、`page_instance_id`、`page_url_clean`、`previous_page_type` | 是 | 是 | 否 | 仅作上下文 | 每个`page_instance_id`一次；普通内容加载结果不另设统一事件；页面地址无敏感参数 |
| 通用交互 | `ui_interaction`｜有意义的通用操作 | 用户完成无专属业务事件的语义操作，或关键操作进入明确失败终态 | 稳定操作实际发生；失败记录使用封闭低基数结果 | 原始 DOM 点击、hover、焦点、组件渲染；已有专属成功事件的同一次操作；自由错误文本 | `interaction_name`、`element_id`、`target_id`、`action`、`result_state`；涉及商品时增加商品三ID | 是 | 否 | 否 | 否 | 一次用户意图一次；子元素／冒泡不重复；成功业务事件不双记 |
| 搜索 | `search`｜正式提交搜索 | 用户正式发起一次站内搜索 | 点击建议、热门词、回车或搜索按钮形成一次正式提交，并生成唯一搜索 ID；建议点击不另设事件 | 输入变化、建议仅展示、面板内未提交筛选；后续失败不撤销提交事实 | `search_id`、`search_term`、`search_term_version`、`entry_context`、`trigger_type`、`filter_ids[]`、`sort_type`；建议触发时增加`suggestion_type`、`suggestion_position`、`suggestion_object_id` | 是 | 是 | 否 | 是 | 四种提交方式各形成一次；建议点击只形成一次`search`；一个提交一个搜索 ID；`search_term`来自搜索模块正式输出 |
| 搜索 | `view_search_results`｜搜索结果终态 | 一次搜索请求得到最终业务结果 | 请求进入成功有结果、成功无结果、失败或取消中的唯一终态 | 搜索提交本身、请求中状态；失败不能记为零结果 | `search_id`、`result_status`、`duration_ms`、`search_version`、`index_version`；`result_count`仅成功有结果或成功无结果时填写；`failure_type`仅失败时填写 | 是 | 是 | 否 | 是 | 一次请求只有一个终态；`success`数量大于0，`no_results`数量为0，`failed/cancelled`数量留空；取消和失败可区分 |
| 商品发现 | `view_item_list`｜商品真实曝光 | 用户真实看到列表或推荐中的一件具体商品；每件达到阈值的商品形成一条逻辑曝光事实 | 商品满足 50% 可视且连续 1 秒；同一`page_instance_id`内，同一`placement_id + listing_public_code`只计一次；进入新的页面实例后可重新计数 | 接口返回、DOM 渲染、低于阈值、后台／失焦；算法候选不算曝光 | 每件逻辑曝光包含`page_instance_id`、商品三ID、`position`、`exposure_id`、`list_id`、`scene_id`、`module_id`、`placement_id`、`entry_context`、`request_id`；推荐场景增加`recommendation_request_id`。技术允许批量传输时，每个`items[]`元素分别携带商品三ID、`position`和`exposure_id` | 是 | 是 | 否 | 是 | 一方统计还原为每商品一条逻辑曝光事实；`position`从1开始；阈值、抖动、切后台和页面实例去重通过；换请求不重复计数；新页面实例可重新计数；算法返回不增加曝光 |
| 商品发现 | `select_item`｜商品选择 | 用户从搜索、推荐、Collection等站内列表主动选择具体商品 | 一次明确商品选择发生 | 嵌套元素重复回调；商详自动打开；没有曝光时不能补造曝光 | 商品三ID、`list_id`、`module_id`、`entry_context`、`request_id`、`exposure_id`、`search_id`、`position`、`source_type` | 是 | 是 | 否 | 是 | 一次选择一次；无曝光点击保留并标链路缺口；可回溯原始站内来源 |
| 商品 | `view_item`｜商品详情浏览 | 用户浏览一个具体上架商品 | 商详核心商品信息成功进入可分析状态 | 页面加载失败、预取、隐藏重复组件；直达不补造列表点击 | 商品三ID、`page_instance_id`、`price`、`currency`、`source_type`；存在前序站内来源时增加`source_module_id`、`source_event_id` | 是 | 是 | 否 | 是 | 每个`page_instance_id`一次；三 ID 一致；点击进入可串联，直达可识别 |

### 5.2 身份、商品意向与购物车

| 业务域 | 统一事件名／业务事实 | 业务含义 | 成立条件 | 不计入边界 | 必要业务信息 | 一方平台 | GA4 | 广告 | 搜推 | 产品验收标准 |
|---|---|---|---|---|---|---|---|---|---|---|
| 身份 | `sign_up`｜注册成功 | 新账户建立，当前`anonymous_id`与新内部用户ID建立确定性关系 | 注册事务真实成功一次 | 打开注册页、表单提交、验证码发送、注册失败；不得包含邮箱、电话、验证码 | `anonymous_id`、`user_id`、`sign_up_method`、`occurred_at`、`source_page_id`、`entry_context` | 是 | 否 | 否 | 否 | 一次注册成功一次；可串联注册前后行为；失败不计入；无 PII；不与自动表单事件重复 |
| 身份 | `login`｜登录成功 | 用户真实登录，当前`anonymous_id`与内部用户ID建立确定性关系 | 登录事务真实成功一次；登录不切断当前Session | 打开登录页、表单提交、验证码发送、自动表单互动、登录失败 | `anonymous_id`、`user_id`、`login_method`、`occurred_at`、`identity_state` | 是 | 否 | 否 | 否 | 一次成功一次；可串联登录前后行为；重复回调不增加；换号／共享设备不串人；无 PII |
| 收藏 | `add_to_wishlist`｜收藏成功 | 商品由未收藏变为已收藏 | 收藏关系真实新增 | 按钮点击、请求失败、状态未变化、重复回调 | 商品三ID、`wishlist_id`、`source_page_id`、`source_type`、`source_event_id` | 是 | 是 | 否 | 是 | 一次状态转换一次；三 ID 一致；失败不计成功 |
| 收藏 | `remove_from_wishlist`｜取消收藏成功 | 商品由已收藏变为未收藏 | 收藏关系真实取消 | 按钮点击、请求失败、状态未变化；不删除历史收藏事实 | 商品三ID、`wishlist_id`、`source_page_id` | 是 | 否 | 否 | 是 | 一次状态转换一次；原收藏历史保留；失败不计成功 |
| 购物车 | `add_to_cart`｜加购成功 | 商品成功加入购物车或数量真实增加 | 购物车内容或数量真实变化 | 点击按钮、请求失败、无状态变化、重复回调 | `cart_id`、商品三ID、`quantity_added`、`quantity_after`、`price`、`currency`、`source_type`、`source_event_id` | 是 | 是 | 否 | 是 | 事件数量和购物车事实一致；一次变化一次；失败不计成功 |
| 购物车 | `view_cart`｜非空购物车浏览 | 用户成功看到非空购物车内容 | 非空购物车成功展示；每个`page_instance_id`一次 | 空购物车、加载失败、勾选、改量、删除、Buy Now；同页面重复渲染 | `page_instance_id`、`cart_id`、`value`、`currency`、`items[]`（每项含商品三ID、`quantity`、`price`） | 是 | 是 | 否 | 否 | 空／失败无本事件；刷新或重新进入可记新页面实例；商品明细完整 |
| 购物车 | `remove_from_cart`｜移出购物车 | 商品被删除或数量真实减少 | 购物车内容或数量真实变化 | 打开滑动操作区、点击未成功、并发失败、无变化 | `cart_id`、商品三ID、`quantity_removed`、`quantity_before`、`quantity_after`、`price`、`currency` | 是 | 是 | 否 | 是 | 变化前后数量可核对；Move to Wishlist的两个真实结果分别成立；失败不计成功 |

### 5.3 Checkout、订单、支付与退款

| 业务域 | 统一事件名／业务事实 | 业务含义 | 成立条件 | 不计入边界 | 必要业务信息 | 一方平台 | GA4 | 广告 | 搜推 | 产品验收标准 |
|---|---|---|---|---|---|---|---|---|---|---|
| Checkout | `begin_checkout`｜开始 Checkout | 有效 Checkout 会话建立，用户进入结算 | 从购物车或 Buy Now 成功建立新的 Checkout 会话 | 仅点击入口、创建失败、恢复同一 Checkout；Buy Now 不补记加购 | `checkout_session_id`、`session_id`、`source_type`、`value`、`currency`、`items[]`（每项含商品三ID、`quantity`、`price`） | 是 | 是 | 否 | 是 | Cart／Buy Now均可区分；失败不计入；同一会话恢复不重复首次进入 |
| Checkout | `add_shipping_info`｜配送信息就绪 | Checkout 的配送信息和配送方式达到可继续状态 | 地址和配送选项校验通过，并进入配送就绪状态 | 选择未保存、校验失败、仅打开区域；不得记录姓名、电话、地址明文 | `checkout_session_id`、`shipping_tier`、`reuse_existing_info`、`items[]` | 是 | 是 | 否 | 否 | ready一次；失败不计成功；无地址或其他 PII；商品快照一致 |
| Checkout | `add_payment_info`｜支付信息就绪 | Checkout 的付款方式达到可提交状态 | 付款方式校验通过并进入支付信息就绪状态 | 仅选择支付方式、校验失败；不得记录卡号、令牌或网关原文 | `checkout_session_id`、`payment_type`、`items[]` | 是 | 是 | 否 | 否 | selected与ready不混用；失败不计成功；无支付敏感信息 |
| 订单 | `order_created`｜订单创建成功 | 唯一订单及订单明细行成功形成 | 订单业务事务真实成功，订单和订单明细行可使用 | Pay Now点击、页面跳转、Checkout就绪；不能冒充成交 | `order_id`、`checkout_session_id`、`session_link_status`、`created_at`、`value`、`currency`、`inventory_lock_expires_at`、`items[]`（每项含`order_item_id`和商品三ID）；能够回指Checkout来源Session时增加`origin_session_id`并记录`linked`，否则记录`unlinked` | 是 | 否 | 否 | 仅关联后续反馈 | 一个订单一次；无法回指Session不阻止权威订单成立且不得猜测；每个订单行可回指商品；重复提交不形成第二个订单事实 |
| 支付 | `payment_started`｜支付尝试开始 | 已创建订单真实发起一次支付尝试 | 新的支付尝试正式建立并开始处理 | 支付信息ready、Pay Now点击、前端校验失败 | `payment_attempt_id`、`order_id`、`payment_type`、`started_at` | 是 | 否 | 否 | 否 | 同一订单多次尝试可区分；一次真实尝试一次；与点击、建单分开 |
| 支付 | `payment_failed`｜支付失败 | 某次支付尝试进入明确失败终态 | 支付尝试得到权威失败结果 | 前端字段校验失败、用户未提交、处理中、可继续重试状态；不得带网关原文或支付敏感信息 | `payment_attempt_id`、`order_id`、`failure_stage`、`failure_type`、`failed_at` | 是 | 否 | 否 | 否 | 同一次尝试只有一个失败终态；无法可靠分类时使用未知；不产生 purchase |
| 成交 | `purchase`｜首次支付成功／成交 | 订单第一次进入支付成功，形成一次成交事实 | 订单首次支付成功 | 成功页、客户端缓存、重复回调、同订单后续成功回调、无新支付的换货／补发 | `order_id`、`payment_attempt_id`、`session_link_status`、`paid_at`、`value`、`currency`、`discount`、`shipping`、`tax`、`items[]`（每项含`order_item_id`和商品三ID）；能够回指Checkout来源Session时增加`origin_session_id`并记录`linked`，否则记录`unlinked` | 是 | 是 | 是 | 是 | 同一订单只计一次；无法回指Session不阻止权威成交成立且不得猜测；支付完成不新建或延长Session；GA4交易 ID使用订单 ID、商品 ID使用 `listing_public_code`；金额和订单行可核对 |
| 退款 | `refund`｜实际退款完成 | 一笔真实资金退款完成 | 支付渠道／财务确认资金已退回；支持部分和多次退款 | 退款申请、审核通过、处理中、退款失败、页面展示、售后结案但未退资金 | `refund_id`、`order_id`、`refunded_at`、`value`、`currency`、`items[]`（每项含`order_item_id`、商品三ID和退款金额） | 是 | 是 | 是 | 是 | 每笔真实退款一次；部分／多次退款可与订单核对；失败不产生退款金额；原 purchase保留 |
...
本方案不重新设计GA4全部字段映射，只规定现有GA4采集需要发生的产品变更。未列出的GA4自动数据和现有标准字段保持当前方式，由开发在技术方案中核验。

| 现有GA4内容 | 处理 | 变更要求 |
|---|---|---|
| 自动`page_view` | 调整 | 关闭GA4自动页面浏览，改由Looply主动发送真实页面到达，避免SPA路由重复。 |
| 自动站内搜索 | 停止 | 不再由URL规则自动形成第二套搜索结果；使用统一`search`和`view_search_results`。 |
| 自动表单互动 | 停止用于业务漏斗 | 不得用`form_start`或`form_submit`代替注册、登录、配送或支付信息就绪。 |
| `lead`／`generate_lead` | 停止 | 不纳入当前GA4业务事件。 |
| `sign_up`、`login` | 停止发送GA4 | 仅进入Looply一方平台。 |
| `search` | 调整 | 改为正式提交时成立；搜索词只使用搜索模块提供的`search_term`。 |
| `view_search_results` | 调整／补充 | 每个搜索请求只记录一个结果终态，区分有结果、无结果、失败和取消。 |
| `view_item_list` | 调整 | 成立条件和去重按5.1；不再以算法返回或普通渲染形成曝光。 |
| `select_item`、`view_item`、收藏、购物车和Checkout标准事件 | 调整 | 成立时点统一按第五章；点击或页面展示不得冒充业务成功。 |
| `purchase` | 调整 | 只使用订单首次支付成功的权威事实；浏览器成功页不得补发第二笔。 |
| `refund` | 调整 | 只在资金实际退回时成立；退款申请、审核和处理中不成立。 |
| GA4商品`items[].item_id` | 调整 | 统一使用`listing_public_code`，来源为`listings.public_code`；不得使用内部`listing_id`、实物商品编码、SKU或SEO slug。 |

GA4不得接收一方内部身份关系、`anonymous_id`、一方`session_id`、内部商品ID、Looply归因结果、支付敏感信息、地址、自由文本或搜索输入原文。具体GA4标准参数映射和发送实现由开发方案确定，不属于产品需求。

广告当前只使用`purchase`和`refund`，沿用已有广告转化接入路径；不得再从GA4导入同一成交或退款形成第二条路径。搜推接收第五章标记为“是”的统一行为，不接收支付敏感信息、地址、广告点击标识和自由文本。
...
| P37 | Checkout 配送 | 配送信息进入就绪状态 | 按第5.3节`add_shipping_info` | `add_shipping_info` | Checkout会话及配送选项 | 按第5.3节验收 |
| P38 | Checkout 支付方式 | 选择方式；付款信息校验；selected是选择行为；ready是付款信息就绪事实 | 选择不冒充ready；校验失败不记成功；不记录卡号、支付凭据或网关原文 | 选择用 `ui_interaction`；ready用 `add_payment_info` | `checkout_session_id`、`payment_type` | 选择与ready分开；`payment_type`使用第五章枚举，只有实际可用的方式才上报 |
...
| 通用数据 | 一方数据平台 | GA4 | 广告 | 搜推 | 限制说明 |
|---|---|---|---|---|---|
| `anonymous_id`、内部用户 ID及确定性身份关系 | 是 | 否 | 否 | 按确定性关系使用 | GA4使用其自身平台身份口径；不发送姓名、邮箱、电话等身份原文；跨设备无确定性关系不合并。 |
| 一方 `session_id` | 是 | 否 | 否 | 可作为行为上下文 | GA4 使用自己的 Session；两套 Session 不混算。 |
| GA4 Session、首次访问和活跃时间 | 否 | GA4 自动形成 | 否 | 否 | 仅作为 GA4 平台口径，不替代一方 Session。 |
| 页面类型、稳定页面 ID、页面版本和上一页面 | 是 | 是 | 否 | 是（作为行为上下文） | GA4 自动页面浏览与主动适配不得重复。 |
| 设备类别、浏览器和操作系统 | 是 | GA4 自动形成 | 否 | 是（作为行为上下文） | 一方与 GA4 各自形成，不依赖从 GA4 回取一方数据。 |
| 访问地域 | 是 | GA4 自动形成 | 否 | 是（作为行为上下文） | 两边各自保留口径；不得用 Market 或语言猜测地域。 |
| Market、模块、入口和原始站内来源 | 是 | 仅发送第五章批准的标准字段 | 否 | 是（作为行为上下文） | 一方保留完整业务上下文；GA4不接收模块高基数字段和Looply归因结果。 |
| 商品、搜索、购物车、Checkout、订单及订单商品行业务对象标识 | 是 | 仅发送对应标准事件需要的字段 | `purchase`、`refund`发送`order_id`、金额、币种和`listing_public_code` | 是（仅对应行为反馈所需对象） | GA4商品`items[].item_id=listing_public_code`；广告和GA4均不接收无必要的内部标识。 |

### 7.1 平台级防重复原则

统一事件的平台去向及防重复总原则以第二章和第五章为准。补充要求：同一广告平台的同一成交或退款，只能选择 GA4 导入或直接发送中的一种，不得重复。


## 八、存量埋点处理

| 存量埋点来自哪里 | 对应哪个统一事件 | 产品处理结论 | 说明 |
|---|---|---|---|
| GA4 自动页面浏览 | `page_view` | 调整 | 与 Looply 主动页面事件只保留一条正式路径，不得同时统计。 |
| GA4 自动 Session、首次访问、活跃时间 | 无对应统一事件 | 保留为专项数据 | 继续作为 GA4 自身分析口径，不替代 Looply 一方 Session、身份和行为口径。 |
| GA4 自动设备、浏览器、操作系统和地域信息 | 无对应统一事件 | 保留为专项数据 | 用于 GA4 分析；Looply 一方分析使用自身公共上下文，不依赖从 GA4 回取。 |
| GA4 自动滚动、站外点击、文件下载和视频互动 | 无对应统一事件 | 保留为专项数据 | 仅作为 GA4 通用互动数据，不作为商品曝光、业务成功或广告转化。 |
| GA4 自动站内搜索 | `search`、`view_search_results` | 停止 | 搜索提交与结果使用 Looply 统一事件，不再由 GA4 自动生成第二套同义事实。 |
| GA4 自动表单互动 | 无直接对应统一事件 | 停止 | 不用于代替注册、登录、Checkout、支付或其他业务成功事件。 |
| 历史 GA4 `lead`／`generate_lead` | 无对应统一事件 | 停止 | 不纳入当前统一业务事件体系。 |
| 历史 GA4 `sign_up`、`login` | `sign_up`、`login` | 调整 | 统一事件继续保留，平台去向按第五章执行。 |
| 历史 GA4 商品曝光、点击、商详、收藏和购物车事件 | 对应 `view_item_list`、`select_item`、`view_item`、`add_to_wishlist`、`add_to_cart`、`remove_from_cart`、`view_cart` | 调整 | 成立条件和业务对象标识统一以第五章为准。 |
| 历史 GA4 搜索事件 | `search`、`view_search_results` | 调整 | 搜索提交和结果终态分别记录，不重复生成结果事件。 |
| 历史 GA4 Checkout、配送和支付信息事件 | `begin_checkout`、`add_shipping_info`、`add_payment_info` | 调整 | 以统一事件定义的业务状态成立为准，不把普通点击当作成功。 |
| 历史 GA4 成交和退款事件 | `purchase`、`refund` | 调整 | 只使用权威业务事实；同一订单或退款不得由浏览器和服务端重复形成正式记录。 |
| Snowplow `page_view` | `page_view` | 调整 | 按当前页面成立条件和页面字典收口。 |
| Snowplow `item_impression` | `view_item_list` | 调整 | 按当前商品曝光条件、商品标识和来源信息收口。 |
| Snowplow `item_click` | `select_item` | 调整 | 按当前商品选择定义和来源信息收口。 |
| Snowplow `view_item` | `view_item` | 调整 | 按商品详情进入可分析状态的统一定义收口。 |
| Snowplow 收藏、购物车事件 | 对应 `add_to_wishlist`、`remove_from_wishlist`、`add_to_cart`、`remove_from_cart` | 调整 | 只有业务状态真实变化时成立。 |
| Snowplow `checkout_start` | `begin_checkout` | 调整 | 映射为有效 Checkout 会话建立，不以入口点击作为成立条件。 |
| Snowplow `search` | `search`、`view_search_results` | 调整 | 搜索提交与唯一结果终态按统一事件拆分。 |
| Snowplow `sign_up`、`login` | `sign_up`、`login` | 调整 | 保留注册成功和登录成功语义，并按第四、五章补齐当前匿名身份、登录身份、来源页和入口信息。 |
| Snowplow 历史计划中的 `order_created` | `order_created` | 调整 | 只表达订单创建事实，不等同于成交。 |
| Snowplow 历史计划中的 `purchase_complete` | `purchase` | 调整 | 统一映射为订单首次支付成功。 |
| Snowplow `struct` 兜底条目 | 无对应统一事件 | 停止 | 不作为具体用户行为或正式业务指标。 |
| 广告产品已有成交埋点 | `purchase` | 调整 | 若表达订单首次支付成功，则并入统一成交事实；同一广告平台不得保留第二条成交路径。 |
| 广告产品已有退款或转化调整 | `refund` | 调整 | 仅表达实际资金退回，不以售后状态代替退款。 |
| 广告产品已有其他浏览或转化埋点 | 依据对应统一事件判断 | 停止 | 当前平台清单未批准的事件不继续作为正式广告转化或优化事实。 |
| 搜推已有搜索、曝光、点击及后续行为埋点 | 对应统一业务事件 | 调整 | 同义用户行为并入统一事件，不再形成第二套正式行为指标。 |
| 搜推候选集、召回、排序、模型、实验、性能和错误数据 | 无对应统一事件 | 保留为专项数据 | 继续服务搜推算法与工程分析，不计作用户曝光、点击或其他普通业务行为。 |

### 8.1 迁移必须避免的情况

存量迁移遵守第二章和第7.1节的平台防重复原则。迁移过程中，搜推专项数据不得被统计为普通用户行为，广告专项数据不得反向成为一方业务事实。
