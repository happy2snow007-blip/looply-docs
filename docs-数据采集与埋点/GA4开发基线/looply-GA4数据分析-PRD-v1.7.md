...
2. 通过统一`trackEvent`产生Looply主动`page_view`和15类浏览器业务/电商事件，经dataLayer与GTM发送至GA4；Purchase与Refund由服务端发送，全部17类事件的字段、触发时机和金额口径一致。
...
| 页面事件 | 每个真实页面实例恰好一条Looply主动page_view；当前GA自动page_view可并存但不替代主动事件；硬刷新视为新页面生命周期 |
...
| view_item_list | looply_ga4_view_item_list | view_item_list | 至少一件商品达到50%可视且持续1秒 | page_instance_id、placement_id、item_list_id/name、商品真实位置、items；搜索结果页增加URL解析出的搜索上下文 |
...
| add_shipping_info | looply_ga4_add_shipping_info | add_shipping_info | 新建、复用或修改后的有效配送信息形成一次新的保存事实 | checkout_id、step_version、shipping_tier、shipping_info_source、金额、items |
...
主动事件的`page_referrer`从当前页面实例上下文读取动作发生前的真实上一页面。新页面实例建立时同步更新来源；直达页面允许为空，不复用前两跳或更早的过期路由，也不猜测来源。
...
`view_item_list`按单件商品达到50%可视且持续1秒后形成真实曝光。后台、页面失焦或未达到阈值不发送；没有任何商品达标时不发送空事件。同一`page_instance_id`内按`placement_id + listing_public_code`去重。事件只携带本次实际达标的商品，不得在每件商品达标时重复发送整页商品数组。商品位置从1开始，缺失时留空，不猜测。

`select_item.items[].index`使用筛选、排序和插入完成后的真实商品位置，同样从1开始。同一业务列表的曝光与点击必须使用一致的`placement_id`、`item_list_id`和`item_list_name`；Wishlist统一使用稳定标识`favorites_wishlist`，不得分别使用展示文案或不同常量。
...
| `page_referrer` | 适用的主动页面和业务事件 | Web公共页面实例上下文 | 动作发生前的真实上一页面 → dataLayer → GTM → GA4 | 直达或无法确定时省略；不得使用前两跳或更早的过期值 |
...
| `shipping_info_source` | `add_shipping_info` | Checkout配送信息模块 | 新建、复用或修改后的本次有效保存事实分别映射为`new`、`existing`、`modified` → dataLayer → GTM → GA4 | 能确定时携带；不得上传地址明文或根据页面展示猜测 |
| `step_version` | `add_shipping_info`、`add_payment_info` | Checkout步骤状态 | 同一`checkout_id`内每次新的有效保存事实递增一次；同一事实重试保持同值 → dataLayer → GTM → GA4 | 取得不到时省略，事件继续发送；不得因渲染、离页或无变化操作递增 |
...
`add_shipping_info`只表示Checkout中形成了一次新的有效配送信息保存事实。新填写后保存、复用已有信息并确认保存、修改已有信息后再次保存，均可各形成一条事件，并分别携带`shipping_info_source=new`、`existing`或`modified`。

同一`checkout_id`内，每次新的有效保存事实使用递增且稳定的`step_version`；一次用户保存动作只能形成一条对外成功事件，地址保存与配送方式内部更新不得拆成两条。仅进入或离开Checkout、返回其他页面、组件重渲染、信息未变化或校验失败均不发送，也不递增版本。
...
| O07 | 商品曝光 | 没有商品达到50%可视且持续1秒时不发送；达标后只携带本次真实达标商品，不出现空`items[]` |
| O08 | 商品点击 | 各入口点击位置与真实列表顺序一致；同一列表的曝光与点击使用一致的展示位和列表标识，Wishlist统一为`favorites_wishlist` |
| O09 | 页面来源 | 首页、列表、Search、PDP、Cart和Checkout连续跳转后，事件`page_referrer`等于动作发生前的真实页面；直达允许为空 |
...
| O12 | 配送信息保存 | 一次新的有效保存事实只发送一条`add_shipping_info`；修改后再次保存产生递增的新`step_version`；离页、重渲染和无变化不发送 |
...
本版本继承v1.6的地区门控、页面语言、金额、Refund、五事件和动态筛选规则，并按已发布《Looply 数据采集与埋点产品需求 v1.8》统一Purchase口径：订单／支付服务端首次确认父订单进入最终paid即成立Purchase，由唯一服务端发送者通过Measurement Protocol投递；浏览器成功页不发送、不claim、不确认、不补发Purchase。规则确认不代表生产已经实现，仍须按第九章验收。
