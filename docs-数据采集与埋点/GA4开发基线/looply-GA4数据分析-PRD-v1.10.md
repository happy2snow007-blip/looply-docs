...
| Custom Event触发器 | 3.3中的15类浏览器业务事件（不含暂停的`looply_ga4_page_view`） | 只匹配GA4命名空间，不匹配`looply_ads_*` |
...
1. 页面实例仍按真实进入、刷新和有效路由重新进入来识别；骨架屏和预取不改变页面实例。
2. 本版本页面浏览由GA4自动页面采集记录；Looply不再产生主动`page_view`。
3. 搜索、筛选、排序、组件展开和同页接口刷新沿用当前页面实例，不因这些动作单独形成新的页面浏览事件。
4. React组件重复渲染、StrictMode重复挂载、监听器重复注册和接口重复回调不得造成业务事件重复。
...
首次结果请求或已提交搜索词、筛选、排序形成的新首屏结果请求最多产生一条`view_search_results`；无限滚动、分页追加请求不产生该事件。`result_status`只能取以下一个值：
...
`select_item.items[].index`使用筛选、排序和插入完成后的真实商品位置，同样从1开始。同一业务列表的曝光与点击必须使用一致的`placement_id`、`item_list_id`和`item_list_name`；Wishlist统一使用稳定标识`favorites_wishlist`，不得分别使用展示文案或不同常量。PC Favorites Drawer不创建新页面实例、不发送新的`page_view`。首次展示非空商品列表时发送一次列表展示；筛选或Tab形成不同结果时发送一次新列表展示；滚动追加和相同结果重开不重复发送；商品点击发送`select_item`。
...
|---|---|---:|---:|
| page_view | GA4自动采集；Looply不产生主动幂等事件 | 由GA4自动处理 | 由GA4自动处理 |
| view_item_list | page_instance_id + list_instance_id | 新页面实例重新判断列表结果 | 新列表结果发送一次 |
...
| E07 | 一方商品曝光边界 | 未达到50%、达到不足1秒、后台或失焦不发送；达到50%且持续1秒后仅记录一方`item_impression`，不发送GA4 `view_item_list` |
...
当前保留GA自动`page_view`；本版本不由Looply显式push`looply_ga4_page_view`。自动事件不携带`page_view_source`。Measurement ID只存在GTM环境变量或受控配置中。
