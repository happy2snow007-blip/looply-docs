...
| `filter_ids` | 筛选后的结果和搜索结果曝光 | 搜索结果页 | 当前URL中已Apply的离散筛选条件 | 无筛选或解析失败时省略 |
...
筛选、排序和Reset更新URL时保留原`trigger_type`，并将实际生效的筛选和排序条件写入当前URL。`view_search_results`及搜索结果`view_item_list`均从当前URL读取同一套上下文。
