...
结果页URL携带编码后的搜索词、`trigger_type`以及已生效的筛选和排序上下文。Search与Collection的动态筛选都使用后台配置提供的稳定`dimension_code`和`option_code`形成同一份已Apply筛选事实；前端不维护固定维度白名单，也不使用展示名称作为ID。`view_search_results`和Search／Collection的商品曝光、点击从该公共事实读取筛选上下文。直达搜索URL、刷新或页面恢复不补造`search`；URL没有`trigger_type`时留空，不猜测。
...
本版本继承v1.4的地区门控、页面语言、金额、Purchase、Refund及五事件规则，并把Search／Collection筛选上下文改为后台动态配置驱动。离散筛选统一使用稳定`dimension_code`和`option_code`，删除前端固定维度白名单；GA4按`dimension_code:option_code`生成`filter_ids`，价格区间继续排除。后台新增`Series`等维度时无需再次调整前端枚举。

2026-08-26生产验收发现的事件触发、字段和次数问题仍以《Looply GA4线上验收问题与开发修复单 v1.2》为证据与复测索引；本PRD只维护目标规则，不把待修复问题写成已完成。
