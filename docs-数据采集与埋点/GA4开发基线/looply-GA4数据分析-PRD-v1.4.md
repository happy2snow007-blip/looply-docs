...
一期建立一个统一业务事件入口、一条`dataLayer → Google Tag Manager → GA4`浏览器链路和一条Purchase/Refund服务端补充链路。GA4不负责广告点击触点、用户来源归因、广告花费或ROAS。

### 1.2 一期目标

1. 创建并配置独立GA4 Property与Web Data Stream，接入美国和香港已启用市场的网站。
2. 通过统一`trackEvent`产生基础`page_view`和17类业务/电商事件，经dataLayer与GTM发送至GA4，字段、触发时机和金额口径一致。
3. 正确定义首次页面、SPA路由、硬刷新、组件重渲染和业务操作的排重边界。
4. 支付最终成功后以浏览器GA4 Purchase为主路径；浏览器未确认执行时由Measurement Protocol在规定延迟后兜底。
5. 退款成功只由服务端发送GA4 Refund，并以refund_id主动幂等。
6. 使用区域投递门控、浏览器claim、Outbox、幂等、重试、验证端点、告警和对账保证数据质量；一期不接入CMP、不读取同意状态。
...
| Purchase最终覆盖率 | ≥99.9% | 区域投递门控允许的已支付订单具有浏览器执行确认或服务端兜底accepted |
...
| Web开发 | 接入trackEvent、页面和业务触发点，维护dataLayer适配器 | Web代码 |
| 数据/标签运营 | 配置、审核和发布GA4相关GTM标签 | GTM、GA4界面 |
| 后端/支付开发 | 提供Purchase事实、上下文快照和Refund | 订单、支付、异步Worker |
...
- 基础`page_view`及17类业务事件。
- 统一事件Schema、按市场配置的页面语言、商品层级和金额合同。
- 区域投递门控：EEA、英国、瑞士不加载GA4、不发送事件；地区策略可配置且业务代码不写死US/HK允许名单。
- 浏览器Purchase、Measurement Protocol延迟兜底、服务端Refund。
- 浏览器幂等、服务端幂等、Outbox、状态、错误、告警和验证。
...
| 页面事件 | 首屏一次page_view；SPA每次有效路由一次；硬刷新视为新页面生命周期 |
| 普通事件排重 | GA4不提供通用event_id自动排重，Looply按事件语义主动幂等 |
| Purchase事实 | 支付验签回调第一次将订单推进paid |
| Purchase路由 | 浏览器主路径；5分钟内无有效执行确认时Measurement Protocol兜底 |
...
| Measurement Protocol | GA4服务端事件接口；一期只用于Purchase兜底和Refund |
...
| claim | 浏览器发送前向服务端取得的短期发送租约 |
| tag_invoked | GTM完成当前Purchase事件处理后的浏览器确认，不代表GA4最终完成入账 |
...
| GTM GA4资产 | Google Tag、17个浏览器Custom Event触发器、GA4 Event Tag及Data Layer Variables |
...
| GTM资产 | 触发器/配置 | 用途 |
|---|---|---|
| Google Tag | 区域策略允许后的GA4消费者初始化 | 使用环境Measurement ID；关闭自动page_view；禁止地区不加载共享容器 |
| Custom Event触发器 | `looply_ga4_page_view`及3.3中的16类浏览器业务事件 | 只匹配GA4命名空间，不匹配`looply_ads_*` |
| Data Layer Variables | `ga4_event_name`、页面参数、电商参数、业务参数 | 从规范dataLayer对象读取值，不在GTM猜测或补造业务字段 |
| GA4 Event Tag | 上述17个Custom Event触发器 | 事件名读取`ga4_event_name`，参数按3.2、3.3映射到同一Web Data Stream |
...
`page_view`是基础系统事件；dataLayer Custom Event固定为`looply_ga4_`加GA4事件名。下表为17类业务事件，其中16类由浏览器触发，`refund`仅由服务端触发：
...
1. 首次文档加载且页面主路由确认后发送一次page_view；骨架屏和预取不发送。
2. SPA路由变化、页面标题和规范URL更新、核心页面数据就绪后发送一次新的page_view。
3. query仅发生无业务意义变化时不重复发送；搜索、分页或筛选形成新的分析页面状态时可发送，并通过page_type或自定义参数区分。
4. 浏览器硬刷新创建新的page_instance_id，发送一次新的page_view。
5. React组件重复渲染、StrictMode重复挂载和监听器重复注册不得重复发送。

`looply_ga4_page_view`必须从Web公共页面上下文读取并携带当前`page_instance_id`。同一页面实例内的`view_search_results`、`view_item_list`、`select_item`和`view_cart`使用同一值；页面刷新或重新进入后使用新值。

### 3.5 五个变更事件规则

#### 3.5.1 搜索提交与结果

`search`只表示用户正式提交搜索，不由接口成功、零结果或失败反向决定。`trigger_type`使用以下六种枚举：
...
| `suggestion_select` | 用户点击输入联想中的搜索词建议 | 携带建议位置；存在稳定对象ID时可增加`suggestion_object_id` |
| `history_select` | 用户点击搜索历史中的搜索词 | `search_term`取搜索模块正式输出的分析词 |
| `popular_term_select` | 用户点击搜索页热门搜索词 | 热门品牌和热门Collection入口不属于搜索提交 |

结果页URL携带编码后的搜索词、`trigger_type`以及已生效的筛选和排序上下文。`view_search_results`和搜索结果页`view_item_list`从当前URL读取同一上下文。直达搜索URL、刷新或页面恢复不补造`search`；URL没有`trigger_type`时留空，不猜测。
...
`view_item_list`按单件商品达到50%可视且持续1秒后形成真实曝光。后台、页面失焦或未达到阈值不发送；同一`page_instance_id`内按`placement_id + listing_public_code`去重。事件只携带本次实际达标的商品，不得在每件商品达标时重复发送整页商品数组。商品位置从1开始，缺失时留空，不猜测。
...
|---|---|---|---|---|
| `event_id` | `view_search_results`及需要事件级幂等的事件 | Web公共事件上下文 | 事件成立时生成；同一事件重试保持同值 → dataLayer → GTM → GA4 | 无法生成时省略，事件继续发送 |
| `page_instance_id` | `page_view`、`view_search_results`、`view_item_list`、`select_item`、`view_cart`及其他需要同页关联的事件 | Web公共页面实例上下文 | 按3.5.4生成 → dataLayer → GTM → GA4 | 缺失时省略，事件继续发送；不得生成替代值 |
...
| `suggestion_position` | `suggestion_select`形成的`search` | 搜索联想列表组件 | 被点击建议在当前联想列表中的实际位置，从1开始 → dataLayer → GTM → GA4 | 能取得时携带；缺失时省略，事件继续发送 |
| `suggestion_object_id` | 有稳定建议实体的`suggestion_select` | 搜索模块 | 使用已有稳定建议对象ID → dataLayer → GTM → GA4 | 可选；没有稳定ID时省略，不生成临时ID |
| `filter_ids` | 筛选后的结果和搜索结果曝光 | 搜索结果页 | 从当前URL读取已Apply的离散筛选条件；每项编码为`维度:稳定值`，同一维度多选分别记录，按维度和值稳定排序、去重后用逗号连接，例如`brand:celine,color:black` → dataLayer → GTM → GA4 | 无筛选时省略；解析失败时省略，事件继续发送；价格区间本期不放入`filter_ids` |
...
| `placement_id` | `view_item_list` | 当前商品模块 | 按下表使用稳定常量 → dataLayer → GTM → GA4 | 缺失时省略，事件继续发送；不得伪造展示位 |
...
| `items[].index` | `view_item_list` | 商品列表组件 | 筛选、排序和插入完成后的业务列表位置，从1开始 → dataLayer → GTM → GA4 | 来源正常且能取得时携带；暂时无法取得时省略，事件继续发送；不得用0或猜测值 |
...
| GA4报表／探索维度 | `trigger_type`、`result_status`、`failure_type`、`placement_id`、`sort_type` | 需支持按搜索入口、结果状态、失败类型、商品展示位和排序方式分析；具体注册和配置方式由GA4技术方案确定 |
...
- Purchase不只依赖浏览器本地记录，必须先取得服务端claim。
...
1. 成功页查询后端状态；processing按1、2、3、5、8秒退避，最长60秒。
2. paid时取得order_id、GA4金额和items、短期report_token。
3. 使用token调用GA4 claim；服务端原子检查地区策略、浏览器确认、claim租约和兜底任务发送权。
4. 只有`ga4=true`时调用一次`trackEvent('purchase')`。
5. claim写`browser_claimed`并保留2分钟租约；兜底Worker不得在租约内发送。
6. GA4适配器push `looply_ga4_purchase`，并提供`eventCallback`和`eventTimeout`；GTM Custom Event触发唯一GA4 Event Tag。
7. `eventCallback`在超时前执行时使用report_token上报`tag_invoked`；超时、脚本异常或回调缺失时不写`tag_invoked`，保留Measurement Protocol兜底资格。确认只说明GTM完成事件处理，不证明GA4最终入账。
8. 刷新、多标签和重复查询复用同一order_id，不产生第二次浏览器Purchase。

### 5.3 Measurement Protocol兜底

支付Outbox派生一条`not_before=paid_at+5分钟`的GA4任务。执行时通过数据库条件更新争夺发送权：

- 有效claim未过期：等待租约结束。
- 已有`tag_invoked`：标记`suppressed_by_browser`，零Measurement Protocol请求。
- Worker已取得发送权：晚到成功页claim必须返回ga4=false。
- 无确认且区域策略允许：使用冻结的原client_id、session_id和transaction_id发送一次。

浏览器调用后确认丢失仍可能造成极端兜底重复；两端使用同一transaction_id并对重复交易进行监控。Measurement Protocol是补充，不是默认替代浏览器。
...
| GA4兜底Purchase | order_id + purchase + measurement_id |
| 浏览器GA4确认 | order_id + ga4 |
| GA4 Refund | refund_id + refund + measurement_id |

服务端任务状态：pending、processing、accepted、retryable_failed、permanent_failed。区域策略禁止为`suppressed_by_region`，浏览器已执行导致兜底取消为`suppressed_by_browser`；两者均为零网络请求的业务终态，不伪装成accepted。`suppressed_by_region`只表示一期地区门控，不表示用户曾作出拒绝。
...
| ga4_browser_purchase_ack | 浏览器claim和确认 | order_id唯一；report_token_hash、ack_status、claimed_at、expires_at、reported_at、error_code |
...
- Measurement Protocol API Secret只存Secret Manager，仅兜底/Refund Worker可读。
...
- Purchase监控浏览器claim、tag_invoked、兜底发送和重复transaction_id。
...
| E03 | 首次页面加载 | 核心路由就绪后一次page_view |
| E04 | SPA有效路由 | 每次路由一次page_view，预取不发 |
| E05 | 硬刷新 | 新page_instance，一次新page_view和一次view_item |
...
| E19 | 增强型衡量网站搜索 | 主动view_search_results存在时，URL搜索参数不再自动生成第二条同义事件 |
...
| P02 | 重复支付回调 | 一条业务Outbox、一条GA4兜底逻辑任务 |
| P03 | 浏览器5分钟内执行并确认 | tag_invoked；兜底suppressed_by_browser |
| P04 | 浏览器claim与Worker并发 | 只有一方取得发送权 |
| P05 | 用户关闭成功页 | 5分钟后MP发送一次 |
| P06 | 成功页晚到 | Worker已发送时claim返回ga4=false |
| P07 | 刷新/多标签 | 同order_id不重复浏览器Purchase |
...
- [ ] Purchase浏览器claim与5分钟MP兜底并发测试通过。
- [ ] Refund部分、多次和重复回调通过。
- [ ] DataLayer与日志PII扫描通过。
- [ ] GTM Preview确认每个浏览器业务事实只触发一次GA4标签，Purchase回调和超时分支通过。
- [ ] 已验证美国、香港已启用市场语言均按当前页面真实语言发送。
- [ ] 已验证浏览、加购、Purchase、Refund和MP兜底均取业务事实币种；市场币种不一致时不静默替换且有诊断告警。
- [ ] staging validation、生产灰度和关键指标告警就绪。

### 9.4 五个变更事件当前验收状态

以下结论来自2026-08-19至2026-08-20对`https://www.test.looply.com/`、`Looply-test`、Measurement ID `G-VFBQX6RSB7`及GTM容器`GTM-P4LZHX78`的测试环境验收。完整证据保存在《Looply GA4 五个埋点测试环境验收与开发修复单 v1.0》。

| 事件 | 已确认通过 | 当前未通过项 | 当前结论 |
|---|---|---|---|
| `search` | 请求到达GA4并返回HTTP 204；单次点击只观察到一条；`search_term`正确 | 缺本场景`trigger_type=manual_search_button` | 未通过，修复后复测 |
| `view_search_results` | 成功结果请求到达GA4；已有`result_status=success`、`duration_ms`、`result_count` | 缺URL解析出的`search_term`和`trigger_type`；直达无结果页未产生`no_results` | 未通过，修复后复测 |
| `view_item_list` | 请求到达GA4；`items[].item_id`已使用CP开头的`listing_public_code` | 一次首屏产生8条且每条重复携带同一批40件商品；缺`page_instance_id`、`placement_id`、商品位置和搜索上下文 | 未通过，修复后复测 |
| `view_cart` | 非空可购商品展示时发送；同页停留未重复；空购物车不发送；`item_id`和`value`正确 | GA4请求与当前GTM映射缺`currency` | 未通过，修复后复测 |
| `remove_from_cart` | 商品成功删除后发送；`item_id`、减少数量1和`value`正确 | GA4请求与当前GTM映射缺`currency` | 未通过，修复后复测 |

本轮已经确认GA4接收链路可用，但不得把“事件已到达GA4”解释为五个事件已经验收通过。开发完成3.5规定的修复后，必须按E14至E19重新取得事件次数和字段证据。
...
| 浏览器拦截 | Purchase缺失 | 区域允许时MP延迟兜底 | Web/后端 |
...
本版本保留v1.3的地区门控、页面语言、金额、Purchase和Refund基线，新增或调整`search`、`view_search_results`、`view_item_list`、`view_cart`、`remove_from_cart`五个事件。为闭合公共页面实例关联，`page_view`和`select_item`仅补充`page_instance_id`参数，不改变事件语义和触发点。搜索提交改为六种正式入口触发，搜索结果使用URL结构化上下文且不再要求`search_id`或`search_request_id`；商品曝光改为50%可视且持续1秒；购物车查看和删除补齐币种及成立边界。增强型衡量保持开启，但关闭“网站搜索”，避免自动与主动结果事件重复。

v1.4同时记录了五个事件在2026-08-19至2026-08-20测试环境的实际验收状态：GA4接收链路已通过，五个事件均仍有字段或次数问题，修复前不得标记为验收通过。
...
GTM中的Google Tag必须关闭自动`page_view`；首屏和后续SPA路由均由Looply显式push `looply_ga4_page_view`，避免自动与手动双发。Measurement ID只存在GTM环境变量或受控配置中。
...
window.dataLayer.push({
  ...
  eventCallback: function () {
    reportGa4TagInvoked('REPORT_TOKEN', 'ORDER_456');
  },
  eventTimeout: 2000
});
```

上述对象由GA4适配器生成，业务组件仍只调用`trackEvent('purchase', payload, options)`。GTM以`looply_ga4_purchase` Custom Event触发唯一GA4 Event Tag，并从Data Layer Variables读取交易参数；不得在GTM中再次生成Purchase或改写transaction_id。

### 12.4 Measurement Protocol Purchase兜底
...
### 12.5 Measurement Protocol Refund
