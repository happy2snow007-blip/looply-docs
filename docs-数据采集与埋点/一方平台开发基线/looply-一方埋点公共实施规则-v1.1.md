...
1. 《Looply 一方埋点新增与修改清单 v1.1》：事件、页面、模块、动作、触发时机、业务参数及存量迁移的唯一权威表。
2. 《Looply 一方埋点新增公共字段清单 v1.1》：相对现有埋点服务需要新增的公共字段及字段来源的唯一权威表。
...
- `result_state`和`failure_type`的逐动作适用范围，以《Looply 一方埋点新增与修改清单 v1.1》工作簿中的“结果字段映射”Sheet为唯一依据。
...
1. `anonymous_id`由身份模块生成，是用于串联登录前后行为的稳定匿名主体ID；埋点模块不得另建第二套长期匿名ID。
2. `domain_userid`由Snowplow生成，作为浏览器侧辅助标识与`anonymous_id`同时上传，但不替代`anonymous_id`，也不用于跨设备自动合并。
3. 已登录事件同时携带当前`user_id`；身份模块已提供时可携带`canonical_person_id`。
4. 登录态直接根据事件发生时是否携带`user_id`判断；公共字段表和事件协议均不新增`identity_state`。
5. 注册或登录成功后，由身份模块建立`anonymous_id → user_id`确定性关系。历史原始事件保留发生时身份，不回写。
6. 登出或切换账号后，后续事件使用身份模块返回的、已经隔离的当前身份上下文。
7. IP、User-Agent、设备信息、浏览路径或广告来源不得触发自动合并。
8. 登录Token和`auth_session_id`不得作为行为`session_id`。
9. 数据分析文档中的`visitor_id`对应本方案的`anonymous_id`，不新增第二套访客标识。

### 4.2 Session

1. 首个有效事件创建行为Session；连续30分钟没有新的有效事件，当前Session结束；之后的首个有效事件创建新Session。
2. 有效事件是页面处于前台且已经满足成立条件的客户端业务事件。技术心跳、接口重试、预加载、后台自动变化和服务端延迟事实不创建或延长Session。
3. 登录、登出和账号切换不强制切断Session；每条事件仍按发生时身份上下文归属。
4. 不同设备或浏览器实例分别产生Session，即使属于同一用户也不合并。
5. Session开始和结束时间根据同一`session_id`内的有效事件派生，不新增`session_start`或`session_end`业务事件。
...
| 搜索分析词 | `query` | 复用现有`search.query`；下游现有展开字段为`search_query`。结果页URL中的`search_term`在事件上报时映射为`query`。 |
...
| `canonical_person_id` | `user_id` | `user_id`表示事件发生时登录账号；`canonical_person_id`仅在身份模块已经完成确定性归并并返回时新增携带。 |
| `session_id` | 无同义现有字段 | 新增一方行为Session标识；登录Session和GA4 Session不得替代。 |
...
3. 搜索提交进入结果页时，结果页URL携带搜索模块正式输出的分析词、`trigger_type`、已生效筛选和排序。
4. `search`、`view_search_results`、搜索结果商品曝光和商品点击使用同一组结构化搜索信息。结果页URL保留业务字段，事件上报按下表映射到一方事件字段：

| URL业务信息 | 结果页URL字段 | 一方事件字段 |
|---|---|---|
| 搜索模块正式输出的分析词 | `search_term` | `query`；下游展开为现有`search_query` |
| 搜索触发方式 | `trigger_type` | `trigger_type` |
| 已生效筛选条件 | `filter_ids[]` | `filter_ids[]` |
| 已生效排序 | `sort_type` | `sort_type` |

5. `view_search_results`、搜索结果商品曝光和商品点击从当前结果页URL解析上述信息；其中`search_term`映射为事件字段`query`，不使用`search_id`。
6. 筛选、排序或重置只记录对应操作；结果更新后形成新的`view_search_results`，不新增`search`。
7. 直达搜索URL、刷新或页面恢复允许直接形成`view_search_results`，不得补造用户主动搜索。
8. 搜索词只使用搜索模块正式输出的分析字段；未经清洗的输入原文不得进入普通分析事件。
...
PC不定义`listing/shop`或`listing/favorites`聚合页面；PC现有`listing/wishlist`和`listing/recently_viewed`纳入本期。Delivery是订单页内物流详情模块，不定义独立`page_id`。
...
3. 站外来源字段的生产方、取值来源和缺失处理，以《Looply 一方埋点新增公共字段清单 v1.1》为唯一依据；本节不重复定义单字段规则。
