...
1. 《Looply 一方埋点新增与修改清单 v1.5》：事件、页面、模块、动作、触发时机、业务参数及存量迁移的唯一权威表。
2. 《Looply 一方埋点新增公共字段清单 v1.5》：相对现有埋点服务需要新增的公共字段及字段来源的唯一权威表。
3. 本文档：跨事件规则、稳定页面／模块／展示位字典和服务端权威事实的唯一权威位置。

以上三份v1.5材料共同构成当前开发实施基线；历史PRD、旧版本清单和评审记录不得补充或覆盖本基线。
...
- `result_state`和`failure_type`的逐动作适用范围，以《Looply 一方埋点新增与修改清单 v1.5》工作簿中的“结果字段映射”Sheet为唯一依据。
...
6. 筛选、价格区间、排序或重置只记录对应操作；同一搜索结果页内仅修改这些Query参数时，沿用当前`page_instance_id`，不新增`page_view`或`search`。结果请求完成后形成新的`view_search_results`，新达到曝光条件的商品继续形成`view_item_list`。
...
3. 站外来源字段的生产方、取值来源和缺失处理，以《Looply 一方埋点新增公共字段清单 v1.5》为唯一依据；本节不重复定义单字段规则。
...
| `click_id_type` | 允许值及选择优先级以5.4为准；字段缺失处理以《Looply 一方埋点新增公共字段清单 v1.5》为准 |
| `method` | `email`、`google`、`apple`、`facebook`；`sign_up`或`login`携带本次实际成功方式，无法取得时省略，不伪造其他值，也不沿用上一次认证方式 |
