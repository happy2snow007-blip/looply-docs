...
1. 《Looply 一方埋点新增与修改清单 v1.2》：事件、页面、模块、动作、触发时机、业务参数及存量迁移的唯一权威表。
2. 《Looply 一方埋点新增公共字段清单 v1.2》：相对现有埋点服务需要新增的公共字段及字段来源的唯一权威表。
...
- `result_state`和`failure_type`的逐动作适用范围，以《Looply 一方埋点新增与修改清单 v1.2》工作簿中的“结果字段映射”Sheet为唯一依据。
...
3. 已登录事件同时携带当前`user_id`。本期身份串联使用`anonymous_id → user_id`，不新增或上传`canonical_person_id`。
...
3. 站外来源字段的生产方、取值来源和缺失处理，以《Looply 一方埋点新增公共字段清单 v1.2》为唯一依据；本节不重复定义单字段规则。
