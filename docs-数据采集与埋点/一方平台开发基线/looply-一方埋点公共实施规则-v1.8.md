...
> 日期：2026-08-28  
> 状态：开发实施基线  
> 历史来源稿：《Looply 数据采集与埋点产品需求 v1.8》；开发实施无需另行读取该历史稿。

## 一、文档用途与唯一权威位置

一方埋点开发交付由以下三份材料共同组成：

1. 《Looply-v1.4-详细埋点需求定稿-v1.3》：事件、页面、模块、动作、触发时机、业务参数及存量处理的唯一权威表。
...
- `result_state`和`failure_type`的逐动作适用范围，以《Looply-v1.4-详细埋点需求定稿-v1.3》工作簿“详细点位”Sheet中对应行的“成立时机”和“专属参数”为唯一依据。
...
1. `pt-0192`用于记录用户复制退货物流信息。d3页面内直接复制与d4物流弹窗复制均发送该事件，统一使用`target_id=shipment_id`，并通过`copy_source=d3_inline/d4_modal`区分入口。
2. d3若当前详情数据只有`returnTrackingNo`，产品契约要求详情接口或同等数据链路同时提供`shipment_id`；不得把物流单号冒充`shipment_id`。事件成立但暂时无法取得`shipment_id`时，仍按第三章第5条发送并省略该字段，同时计入数据质量缺口。
3. `pt-0196`、`pt-0197`、`pt-0199`、`pt-0200`和`pt-0202`覆盖同一订单内一个或多个订单商品行的退货操作，不支持一次操作跨多个订单。
4. 上述五个点位按“一次用户操作一条事件”发送：`target_id=order_id`，`order_item_ids`为本次退货申请涉及的`order_item_id`数组；单商品也使用单元素数组。数组先去重，再按稳定顺序排列；不得按商品行拆成多条同义事件。
5. `pt-0202`继续使用详细点位表规定的`result_state`和`failure_type`表达提交成功、请求失败或校验失败；多商品不改变结果字段口径。
...
- 同一订单内的多商品退货操作使用`target_id=order_id + order_item_ids[]`关联：数组只包含本次操作涉及的订单商品行，去重并按稳定顺序排列；单商品也使用单元素数组。`order_item_ids`是售后点位专属字段，不进入公共基础字段表。
...
Search与Collection的筛选项均来自后台动态配置。前端使用后台稳定`dimension_code`和`option_code`形成同一份最终已Apply筛选事实，不维护固定维度白名单，也不使用展示名称作为ID。每个离散筛选项在一方事件中编码为`dimension_code:option_code`并进入`filter_ids[]`；多选项去重并按Code稳定排序。选择后取消、关闭面板或未Apply的值不进入最终集合；Reset后集合为空。

筛选项`select`交互的`target_id`使用本次`dimension_code:option_code`，筛选组展开／收起使用`dimension_code`。Search／Collection的`apply`交互携带本次最终`filter_ids[]`；对应商品曝光与点击也携带同一最终集合。后台／数据平台维护可追溯的Code到展示名称映射。
...
| 已生效筛选条件 | 后台筛选状态；Search同时写入`filter_*`URL参数 | `filter_ids[]`：离散筛选项使用`dimension_code:option_code`；价格区间使用`price:{币种}:{最低价或*}-{最高价或*}`；全部去重后按Code稳定排序 |
...
10. Collection的筛选Apply、商品曝光和商品点击使用与Search相同的动态离散筛选事实；Collection不生成搜索词、`trigger_type`或`view_search_results`。
...
| `copy_source` | `d3_inline`、`d4_modal`；仅用于`pt-0192`区分售后详情页面内直接复制与物流弹窗复制 |
...
下列12个已有点位ID对应的产品控件或生产入口当前不存在，本版本标记为“不适用”，不计入埋点覆盖率分母；点位ID继续保留且不得复用：

| 点位ID | 当前不实施原因 |
|---|---|
| `pt-0018` | Market & Language当前没有独立币种选择控件 |
| `pt-0037` | Shopping Bag游客态没有独立Login入口 |
| `pt-0038` | Shopping Bag直接支持游客结账，没有独立Continue入口 |
| `pt-0128` | H5 Wishlist与Recently Viewed为独立页面，没有Tab切换 |
| `pt-0189` | 售后列表当前没有按订单筛选器 |
| `pt-0190` | 因没有订单筛选器，也没有清除筛选按钮 |
| `pt-0157` | 风险受阻弹窗当前没有Retry入口；仅存在不可触发的事件Helper |
| `pt-0159` | 风险受阻弹窗当前没有Contact Support入口；仅存在不可触发的事件Helper |
| `pt-0170` | 已记忆账号页面当前路由不可达，Continue旧组件不属于生产入口 |
| `pt-0171` | 已记忆账号页面当前路由不可达，Switch Account旧组件不属于生产入口 |
| `pt-0201` | 当前售后凭证UI只有新增与移除，没有原位Replace入口 |
| `pt-0239` | Account游客态只有合并的Sign In or Register入口，没有独立Register入口 |

上述能力真正上线时，再重新评审并启用对应埋点，不为尚不存在的业务入口预埋事件，也不以已有Helper、不可达旧组件或契约声明代替真实生产覆盖。
...
已经分配但本期不适用的点位ID在`tracking-points.yaml`等机器契约中应明确标记为`disabled/not_applicable`或等价状态，不得删除、复用或继续计入覆盖率分母；未来产品入口上线后，须基于原ID重新评审触发、字段与验收用例后再启用。
