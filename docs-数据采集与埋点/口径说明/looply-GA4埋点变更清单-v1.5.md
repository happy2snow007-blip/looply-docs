...
> 历史版本：仅供追溯；当前开发基线以 GA4 PRD v1.10 为准。
> 适用范围：仅处理本清单列出的 GA4 事件；未列出的 GA4 事件、自动采集和现有标准字段保持当前方式。

本清单与《Looply GA4数据分析 PRD v1.5》共同构成GA4开发依据；本批五事件的独立实施规则见《Looply GA4五事件新增与修改 PRD v1.1》。三份文档中的事件成立条件和字段口径已经按本版本对齐。
...
| P0 | `view_item_list` | dataLayer Custom Event：`looply_ga4_view_item_list` | 列表数据成功展示即形成曝光 | 改为单件商品达到50%可视且持续1秒时形成真实曝光；同一`page_instance_id`内按`placement_id + listing_public_code`去重；Search／Collection筛选后读取后台Code驱动的最终已Apply筛选事实 | `page_instance_id`、`placement_id`、商品位置及`items[]`；每个`items[].item_id=listing_public_code`；Search结果曝光增加`search_term`、`trigger_type`及筛选／排序上下文；Search／Collection筛选曝光增加`filter_ids` | 曝光虚高，点击率和筛选效果无法按动态维度分析 | 未达到阈值、后台或失焦不发送；同一页面实例同一展示位的同一商品只计一次；筛选上下文只包含最终已Apply的`dimension_code:option_code`，不得维护前端固定维度清单 |

## 二、新增 GA4 事件

| 优先级 | GA4 事件 | dataLayer Custom Event | 新增要求 | 必要信息 | 不新增的后果 | 验收标准 |
|---|---|---|---|---|---|---|
| P0 | `view_search_results` | `looply_ga4_view_search_results` | 搜索结果页初始加载或URL参数生效后的结果实际展示并形成终态时发送一次；直达URL、刷新或页面恢复允许独立产生本事件，但不得补造`search` | `event_id`、`page_instance_id`、`search_term`、`result_status`、`duration_ms`；可得时增加`trigger_type`、`result_count`或`failure_type`；筛选生效时使用后台稳定`dimension_code:option_code`形成`filter_ids` | 无法准确计算搜索成功率、零结果率和失败率，也无法识别动态筛选后的结果 | 每次结果更新只有一个终态；最终筛选组合与页面状态一致，支持后台动态新增的`Series`等维度；选择后取消和Reset不得残留旧值 |
...
7. Search与Collection筛选配置中的稳定`dimension_code`和`option_code`形成公共已Apply筛选事实；GA4按`dimension_code:option_code`序列化，去重并稳定排序后用逗号连接。展示名称只用于页面和数据侧映射，不进入`filter_ids`。
...
- Search与Collection分别验证单选、多选、动态`Series`维度、选择后取消和Reset；`filter_ids`只包含最终已Apply的稳定Code，后台新增维度不要求前端增加枚举。
