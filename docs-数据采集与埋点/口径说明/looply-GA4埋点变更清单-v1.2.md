...
|---|---|---|---|---|---|---|---|
| P0 | `search` | dataLayer Custom Event：`looply_ga4_search` | 搜索接口成功或明确无结果时发送 | 改为用户通过搜索轮播词、手动输入后回车或搜索按钮、输入联想、搜索历史或热门搜索词正式提交时发送一次；使用本次提交的`search_id`；`search_term`只取搜索模块正式输出 | `search_id`、`search_term`、`trigger_type`；联想触发时增加建议类型和位置，有稳定实体ID时可选传`suggestion_object_id` | 搜索意图受接口结果影响，失败搜索消失，搜索次数口径不稳定 | 每次正式提交恰好一条`search`；接口结果不反向决定是否产生提交事件；输入变化和建议仅展示不发送；热门品牌和热门Collection入口不发送`search` |
| P0 | `view_item_list` | dataLayer Custom Event：`looply_ga4_view_item_list` | 列表数据成功展示即形成曝光 | 改为单件商品达到50%可视且持续1秒时形成真实曝光；同一`page_instance_id`内按`placement_id + listing_public_code`去重 | `page_instance_id`、`placement_id`、商品位置及`items[]`；每个`items[].item_id=listing_public_code`；搜索结果页曝光增加对应`search_id` | 曝光虚高，点击率和推荐效果被低估 | 未达到阈值、后台或失焦不发送；同一页面实例同一展示位的同一商品只计一次；新页面实例可重新计数 |

## 二、新增 GA4 事件

| 优先级 | GA4 事件 | dataLayer Custom Event | 新增要求 | 必要信息 | 不新增的后果 | 验收标准 |
|---|---|---|---|---|---|---|
| P0 | `view_search_results` | `looply_ga4_view_search_results` | 搜索结果实际展示并形成最终状态时发送一次；站内正式提交产生的结果复用对应`search_id`，直达URL、刷新或页面恢复没有前序提交时可独立生成`search_id`，但不得补造`search` | `search_id`、`search_term`、`result_status`、`duration_ms`；成功时记录`result_count`，失败时记录低基数`failure_type` | 无法准确计算搜索成功率、零结果率和失败率，也无法识别直达搜索结果页 | 每个结果展示实例只有`success`、`no_results`、`failed`、`cancelled`之一；失败不记作零结果；有前序`search`时两者使用相同`search_id` |
...
5. 站内正式提交产生的`search`与对应`view_search_results`使用完全相同的`search_id`；直达URL、刷新或页面恢复允许只有`view_search_results`，不得补造用户主动`search`。
...
- `view_search_results`验证站内提交关联、无前序提交的结果展示，以及成功、零结果、失败和取消场景。
- `view_item_list`完成可视阈值、持续时间、前后台和页面实例去重验证；搜索结果页曝光带对应`search_id`。
