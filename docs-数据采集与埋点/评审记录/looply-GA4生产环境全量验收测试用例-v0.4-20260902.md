...
本版依据文档中心提交 `ae1947b08d39ff8704d62938f01da0acca3922eb` 及该提交中的以下正式文件编制：

1. 《Looply GA4数据分析 PRD》v1.10；
2. 《Looply GA4五事件新增与修改 PRD》v1.2；
3. 《Looply GA4全量事件清单》v1.0；
4. 《Looply GA4埋点变更清单》v1.5；
5. 《Looply GA4线上验收问题与开发修复单》v1.2；
6. 《Looply GA4剩余问题收口开发包》v1.4。
...
| 主题 | 当前正式口径 | 本轮执行口径 |
|---|---|---|
| GA自动 `page_view` | GA4 PRD v1.10暂停Looply主动事件，保留GA自动事件 | 验证无`looply_ga4_page_view`主动事件；页面实例上下文仍由Web公共上下文生成并供其他适用事件关联。 |
| GA自动网站搜索 | GA4 PRD v1.10保持“网站搜索”自动衡量关闭 | 仅Looply主动 `view_search_results`；发布前后在GA4设置与`g/collect`复核无自动第二条同名事件。 |
| `view_item_list` | GA4 PRD v1.10改为新列表结果成功展示时每次一条 | 滚动、分页、单卡可视阈值不发送；新列表结果使用新的`list_instance_id`，点击复用当前列表实例。 |
| `purchase` | GA4 PRD v1.10规定只由唯一服务端发送者投递 | 浏览器成功页零发送、零claim、零补发；服务端首次确认paid后通过Measurement Protocol投递。 |
| `add_shipping_info` | GA4 PRD v1.10规定首次有效tier及后续tier变化时成立 | 地址新建、复用、切换或修改但最终tier未变化时为0条；只有有效tier变化才递增`step_version`。 |
| `view_item.item_brand` | GA4 PRD示例包含品牌；修复单v1.2 FIX-11明确本项不要求 | 不把 `item_brand`作为通过条件。 |

上述规则已在本版用例中直接收敛，不再保留相互冲突的两套预期。历史报告和旧用例只用于追溯，不作为本轮通过证据。
...
| 安全边界 | 只通过官方测试支付、零资金方式或受控后端测试事实创建测试订单；不输入真实支付卡；Refund不做浏览器伪造 |
...
| GL-11 | 主动来源参数隔离 | 当前不得产生`page_view_source`新值；`view_search_results_source`只出现在主动结果事件；其他事件不得携带 |
...
| PV-12 | D+1来源核对 | 全日 | 上线后新数据不得出现Looply主动`page_view`或新的`page_view_source=looply_custom`；历史已有值只作历史数据，不反推当前实现 |
...
| VSR-15 | Search无限滚动／分页追加请求成功或失败 | 不发送 `view_search_results`；仅保留一方请求/分页监控 |

### 5.7 `view_item_list`

所有入口同时检查新列表结果成立条件、非空商品载荷、真实位置、页面实例、列表实例、筛选上下文和去重；滚动、分页或单卡可视变化不形成新的GA4列表事件。
...
| 用例ID | 场景 | 终端 | 预期 |
|---|---|---|---|
| VI-01 | 直达PDP | PC/Mobile | 核心内容可见后1条；读取当前PDP页面实例上下文的 `page_instance_id`，不依赖Looply主动`page_view` |
...
| VI-15 | Search/首页/New Arrivals/Collection/推荐/Recently Viewed/Wishlist连续进入 | PC/Mobile | 前序 `select_item`与PDP `view_item`按页面实例上下文关联；不存在Looply主动`page_view`；不要求 `item_brand` |
...
| 用例ID | 场景 | 终端 | 预期 |
|---|---|---|---|
| SH-01 | 同一checkout首次形成有效`shipping_tier` | PC/Mobile | 1条；`checkout_id`、`step_version=1`、`shipping_tier`、金额和商品正确 |
| SH-02 | 新建、复用、切换或修改地址，最终有效tier未变化 | PC/Mobile | 0条；不递增`step_version` |
| SH-03 | 修改信息后有效`shipping_tier`发生变化 | PC/Mobile | 1条；`step_version`在当前版本上递增1且稳定 |
| SH-04 | 校验失败、保存失败、仅填写未保存或最终仍无有效tier | PC/Mobile | 0条 |
| SH-05 | 同一tier事实重复回调、重试或页面重渲染 | PC/Mobile | 0条新事件；沿用原版本 |
| SH-06 | 未修改离开Checkout、返回首页 | PC/Mobile | 0条；不得在首页补发旧配送事件 |
| SH-07 | 无变化再次点击、离焦或页面仅显示变化 | PC/Mobile | 0条新事件 |
...
只通过官方测试支付、零资金方式或受控后端测试事实创建测试订单；浏览器成功页只用于核对“零发送”，Purchase通过服务端运行证据与GA4接收结果验收：

| 用例ID | 场景 | 预期 |
|---|---|---|
| PU-01 | 支付状态processing | 0条 `purchase` |
| PU-02 | 服务端首次确认父订单进入最终paid | 唯一服务端发送者通过Measurement Protocol发送1条；`transaction_id=order_id` |
| PU-03 | 用户关闭或未打开成功页 | 不影响服务端Purchase投递 |
| PU-04 | 重复Worker、并发消费或重复支付回调 | 只有一个`order_id + purchase + measurement_id`逻辑任务；不新增Purchase事实 |
| PU-05 | 刷新成功页、多标签或成功页晚到 | 浏览器零`purchase`请求、零claim、零确认、零补发 |
| PU-06 | 金额、税、运费、币种和商品 | `transaction_id=order_id`；金额与业务事实一致，`value`不含税和运费 |
| PU-07 | MP失败重试 | 同幂等键和`transaction_id`重试，不形成新交易 |
| PU-08 | GA关联上下文缺失 | Purchase仍由服务端投递；缺失字段省略且不伪造，并记录数据质量告警 |
| PU-09 | 禁止地区订单 | 浏览器与服务端零请求；任务为 `suppressed_by_region` |
...
| FIX-04 主动页面事件暂停 | PV-01～10，覆盖首页、Search、Collection、PDP、Wishlist、Cart、Checkout |
| FIX-05 页面来源参数 | PV-10、PV-12，验证不产生新的`page_view_source` |
| FIX-06 搜索结果来源参数 | VSR-13、VSR-14 |
| FIX-07 空列表事件与滚动误发 | VIL-B03～B06 |
| FIX-08 点击位置/Wishlist标识 | SI-01～SI-13 |
| FIX-09 页面来源 | GL-09、PV-11、SI-15 |
| FIX-10 跨事件ID污染 | GL-10、VIL-B09、WL-05 |
...
6. 只在无真实资金影响且具备官方测试通道、零资金方式或受控后端测试事实时执行配送、支付信息和测试订单场景。
7. Purchase核对浏览器零发送、服务端唯一任务、Measurement Protocol投递与GA4接收；Refund只读取或触发受控测试退款的真实服务端证据。
