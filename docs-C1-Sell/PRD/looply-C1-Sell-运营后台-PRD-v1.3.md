...
邮件主题为 `We received your Looply sell request — {Request ID}`。邮件语言使用用户提交时的页面语言，西语模板由翻译系统提供；人工重发沿用该预约首次发送时的主题、正文模板、语言和收件邮箱，不读取运营人员当前后台语言。

三种 Selling method 对应的确认邮件模板、模板内容和空值展示规则详见 `docs/product/looply-C1-Sell-PRD-v0.9.md` 第 6.1 节。
...
| 翻译能力 | `seller_category` 支持英语源内容和西语展示；确认邮件的西语模板由翻译系统提供 |
...
7. 预约创建后由 `sell@looply.com` 自动发送与 Selling method 匹配的确认邮件；邮件内容符合 4.3 的对应模板和空值展示规则；后台准确展示 `Pending`、`Sent`、`Failed`，Failed 支持人工重发且不重复创建预约。

## 九、设计与资料索引

- 当前运营后台原型：`prototypes/c1-sell/outputs/looply-c1-sell-ops-local.html`。该本地文件用于核心正常路径的可点击演示；异常状态、接口结果及验收边界以本 PRD 的完整规则为准。
- 用户端数据字段基线：`docs/product/looply-C1-Sell-PRD-v0.9.md`
