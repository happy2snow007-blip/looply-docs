...
> 版本：v0.9  日期：2026-09-09  状态：开发评审稿
> 本文是 C1 埋点唯一开发阅读入口，整合一方平台与 GA4；v0.8 为上一版整合稿。本版修复 GA4 字段和 C2 公共基线引用。

## 一、范围与指标

一方平台采集 C1 前台 PC Web / Mobile Web 的页面到达、首页各屏有效曝光、业务/导航/转化相关点击、表单、校验、异常、预约和 Contact Us。首期回答：需求规模、预约漏斗、售卖方式、服务区域。除 Sell 首页外，其他页面不做逐屏曝光，仅记录页面到达和纳入字典的业务交互。来源、IP/地域、设备、Session、语言和 Market 属于平台通用维度，由统一采集体系提供，C1 不新增或重复定义；C1 业务事件只需与这些公共维度关联分析。后台操作、邮件、Download、Internal note、供给质量、处理效率和预约后有效供给转化不在本期。
...
公共字段基线：开发直接复用《Looply 一方埋点公共实施规则 v1.8》和《Looply 数据采集与埋点产品需求 v1.9》的字段路径、身份与 Session 生命周期，不在 C1 重定义。验收必须验证 C2→C1→C2 的 `anonymous_id`、`domain_userid`、`session_id` 连续。
...
- `c1_accepted_brands`、`c1_service_area`、`c1_sell_flow`、`contact_us` 等其他页面只记录 `page_view`；预约流程 Step 到达仍按业务漏斗记录 `seller_step_view`，不等同于通用屏幕曝光。
...
一期直接采用 GA4 默认的 `user_engagement` 与 `engagement_time_msec`。开发需保留 GA4 自动采集能力，确保页面真实到达后自动产生 `page_view`，页面切换时不重复触发自动与手动 `page_view`。一期不新增 `screen_view_start` / `screen_view_end`，也不在 C1 事件中自行计算逐屏 `visible_duration_ms`。

该方案可回答页面级和 Session 级平均互动时长、不同来源用户整体参与度；不能作为首页每一屏精确停留秒数的口径。若后续需要逐屏时长，再单独立项扩展一方平台 `screen_view` 计时组件和 GA4 映射。
...
| `seller_service_area_check` | `c1_service_area_check` | `business_domain=c1`、`flow_scope=c1_sell`、`query_context`、成功时 `coverage_status`、`in_home_available`、`default_method`、`retry_flag` | 否 |
| 用户主动售卖方式选择 | `c1_selling_method_select` | `business_domain=c1`、`flow_scope=c1_sell`、`selling_method`、`selection_origin=user`、`step_id`、`device_type` | 否 |
| 权威预约成功后的 `seller_request_submit` | `c1_sell_request_submit` | `business_domain=c1`、`flow_scope=c1_sell`、`selling_method`、`piece_count_bucket`、`category_count`、`brand_count`、`photo_count`、`coverage_status`、`preferred_date_present`、`referral_present`、`result_state=success` | 是 |
