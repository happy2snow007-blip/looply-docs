...
`ZIP Coverage 列表 → Add ZIP code / Edit ZIP configuration`
...
| 枚举值 | 含义 | 触发时机 | 前台结果 |
|---|---|---|---|
| `Active` | 当前 ZIP 支持 In-Home Service | 新增或编辑时选择 Active | 参与前台 Service Area 查询和预约方式推荐 |
| `Inactive` | 当前 ZIP 不支持 In-Home Service | 新增或编辑时选择 Inactive | 不参与前台覆盖判断 |
...
保存成功后关闭表单、刷新列表并更新 `Updated at`；保存失败时保留已填内容并提示重试。编辑仅允许修改 Status。
...
关键词搜索仅匹配 `Request ID`、`Full name`、`Email`。Submitted at 支持起止日期筛选，默认最近 7 天并包含起止日；起始日期晚于结束日期时阻止查询并提示 `Start date cannot be later than end date.`。

`Download` 导出当前关键词和日期筛选共同命中的全部记录，不只导出当前分页。无匹配记录时按钮置灰。文件每行对应一条预约，照片导出为可访问的文件链接。CSV 字段按以下顺序固定：
...
| Item information | Categories、Brands、Total pieces、Additional notes、Photo links |
...
用户提交的个人信息、地址和照片仅在 Appointments 详情与 Download 中提供给已登录的 C1 运营人员。用户原始业务字段保持只读；运营人员仅可编辑 Internal note，并可对 `Failed` 的确认邮件执行 `Resend email`。当前版本仅对 Download 实施 4.1 定义的专项记录。
...
| 文件能力 | 预约照片在详情和 Download 中提供仅授权运营人员可访问的链接 |
...
- ZIP Coverage：单 ZIP 新增、City/State 回填、搜索、编辑状态、启用/停用及异常状态。
...
6. Download 导出当前全部筛选结果及规定字段，并记录账号、时间、筛选条件和导出记录数。
...
- 用户端数据字段基线：`docs/product/looply-C1-Sell-PRD-v0.7.md`
