...
> 版本：v1.2  日期：2026-09-10  状态：待开发评审
> 本文定义 C1 一方与 GA4 的业务采集口径；配套《埋点开发清单 v1.0》和《C2 公共埋点规范引用入口 v1.1》。技术接入与平台配置由技术团队确定。
...
公共字段按《C1 Sell：C2 公共埋点规范引用入口 v1.1》定位原始规范及待确认冲突，不将两份冲突原文拼接为新基线。C1 只复用公共采集层，不修改身份或 Session 生命周期。开发同时读取本 PRD v1.2 和 Excel 点位清单 v1.0；Excel“交互字典”维护每个 point_id + variant_id 的完整操作组合，本文第四章是同源生成的定位索引。point_id、variant_id 是文档定位键，不要求新增上报公共字段。
...
已核对的预约流程节点 `3:6738` 明确包含：PC 预约表单四步进度条、Step 1–4、Continue/Back、In-Home / Visit Looply / Ship To Us 分支、协议勾选、Preferred date、Referral code、ZIP 与地址字段、成功/错误状态。这些元素的已有需求按 `seller_step_view`、`ui_interaction`、`seller_service_area_check`、`seller_request_submit` 分类；本次字典修订不作为全量 UI 或所有字段均已覆盖的证明。姓名、电话、邮箱、地址、备注及照片只记录填写状态/结果，不记录原文。
...
开发交付时需提供自建数据平台的查询入口或固定查询方法、字段说明、默认时间范围与时区、数据更新时间、权限范围和一份四类核心问题的示例查询结果。查询方法可以是现有平台查询页、固定 Explore、SQL 模板或等价的受控方式，但本期不新增 Looply Operations 页面；具体工具形态属于技术交付，不改变采集事件口径。

前台提交结果以接口返回状态统计；后台预约创建以独立业务事实 `seller_request_created` 统计并按 `request_id` 去重。本期不要求二者关联。GA4 关键事件反映前台接口成功，不替代后台预约创建事实。

## 十、开发评审前置确认

开发需要在评审会上确认：C2 公共 Schema 版本及可访问基线、C1 页面字典与 Figma 页面/状态的逐项对照（预约流程节点 `3:6738` 已核对，其他未核对页面需标记待核对）、Excel v1.0 与实际控件的逐条对应、现有全站 GA4 接入是否满足本 PRD 的业务验收、C2→C1→C2 运行验证环境，以及 PC/Mobile 控件差异是否仅通过 `element_id` 表达。本次修复字典一致性，不代表上述前置事项或实际运行验收已完成。
