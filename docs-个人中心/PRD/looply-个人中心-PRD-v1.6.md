...
> **v1.6 变更**：统一个人中心服务端开关反馈。Email Subscription、Performance、Functional、Targeting、Do Not Sell or Share 保存成功后显示 `Preference updated.`；保存失败显示 `Update failed. Please try again.` 并回滚开关。连续成功操作不叠加多个 Toast；Strictly Necessary 与 GPC 自动生效不触发用户操作 Toast。

> **v1.5 变更**：补充游客邮箱订阅提交反馈。首次订阅成功与重复订阅幂等成功统一显示成功 Toast `You're subscribed!`；网络异常、超时或服务端失败显示失败 Toast `Subscription failed. Please try again.`。两类 Toast 均接入多语言；邮箱格式错误继续使用输入框内联提示，不显示失败 Toast。

> **v1.4 变更**：Policies 页面按 V5 UI 效果实现 PC/Mobile 响应式展示，全部页面文案支持多语言；五类政策按 `market_id + country_code` 获取，国家之间允许使用不同政策正文、版本、生效时间和 URL。切换 Country/Region 时，政策文件、Email 订阅偏好、Cookie Preferences、Do Not Sell or Share 等区域数据与目标国家默认语言/币种一起切换。补充游客订阅 Email 基础格式校验，以及 Gender 未设置时的 `null` 展示与保存规则。PRD 不约束开发将政策列表和正文实现为几个页面。
...
> **数据模型同步说明**：v1.4 新增的政策、订阅偏好和隐私设置地域隔离规则优先于 ER v1.6 旧口径。此次仅更新 PRD；对应模块在开发方案冻结前须将 `market_id + country_code` 同步到政策分配、Email 订阅状态及账户/设备隐私同意的数据模型。
...
6. 首期支持 United States 与 Hong Kong 国家站，并根据当前 `market_id + country_code` 加载可售 listing、可用币种、适用政策、订阅偏好与隐私设置。
...
| 区域上下文 | 当前生效的 `market_id + country_code + language + currency` 组合，以及依赖该组合加载的政策、偏好和隐私设置 |
...
|---|---|---|---|
| Country/Region | Market 模块允许 C 端使用的国家/地区；示例 United States、Hong Kong | 已保存 country_code；无值时使用默认国家，market_id 由服务端归属关系确定 | 单选后 Save；原子更新区域上下文，并刷新目标国家政策、订阅偏好与隐私设置 |
...
10. 国家切换必须同时加载目标 `market_id + country_code` 下的五类政策、Email 订阅偏好、Cookie Preferences、Do Not Sell or Share 状态；这些数据不得沿用切换前国家的缓存或默认值。用户切回原国家时重新读取该国家已保存状态，不复制其他国家的选择。
11. 区域切换采用整体提交：只有目标国家的 `market_id + country_code + language + currency`、政策目录、订阅偏好和隐私设置均返回有效结果后，前端才一次性展示新国家状态。合规模块明确返回 `not_applicable` 属于有效结果，缺少配置或请求失败不属于有效结果。任一关键项失败则保持切换前完整状态并提示重试，不得出现政策已切换但隐私设置仍属于旧国家等混合状态。
...
保存或国家站上下文切换失败时保留原生效的区域上下文、政策、订阅偏好和隐私设置并提示重试；不允许只更新其中一部分。Market 模块返回 0 个有效国家、归属不匹配、目标关联停用、缺少国家默认语言/币种、默认语言未在多语言系统启用、默认币种不受归属 Market 支持或目标国家区域配置不完整时，不切换当前国家站并记录配置告警。
...
开关切换采用即时反馈；服务端保存成功后按“开关统一反馈规则”显示 `Preference updated.`。保存失败时回滚原状态并显示 `Update failed. Please try again.`。进入页面拉取失败时显示重试，不渲染一个可能错误的开关值。
...
| 结果 | 触发条件 | English Toast | i18n key | 页面行为 |
|---|---|---|---|---|
| 订阅成功 | 首次订阅成功；或重复订阅按幂等成功返回 | `You're subscribed!` | `account.subscriptions.toast.success` | 显示成功 Toast，约 3 秒后自动消失；停留当前页，不重复提交 |
| 订阅失败 | 网络异常、请求超时或服务端返回失败 | `Subscription failed. Please try again.` | `account.subscriptions.toast.failure` | 显示失败 Toast，约 3 秒后自动消失；保留已输入 Email，Subscribe 恢复可点击 |
| 格式错误 | 前端 Email 基础格式校验未通过 | 不显示 Toast | `account.subscriptions.email.invalid` | 在输入框附近显示内联错误，阻止请求 |

成功与失败 Toast 均使用全局 Toast 组件并按当前语言显示译文；不得把完整 Email 放入 Toast 或埋点。若全局 Toast 规范对位置、动效或无障碍停留时间有统一要求，以全局规范为准，本 PRD 固定文案、触发条件和反馈类型。
...
**功能描述**：在一个页面按固定顺序平铺 Manage Your Data、Cookie Preferences、Do Not Sell or Share My Personal Information。页面按当前 `market_id + country_code` 读取适用说明和已保存隐私状态；切换国家后重新加载目标国家配置，不沿用旧国家选择。

**区域隔离规则**：Cookie Preferences 与 Do Not Sell or Share 按“用户或设备主体 + `market_id + country_code`”读写。同一用户在不同国家可有不同选择；切回原国家恢复该国家已保存值。目标国家从未保存时，使用合规/政策模块为该国家配置的默认值，不复制来源国家选择。Manage Your Data 的联系方式、权利说明和履约时限允许按国家配置，页面文案由政策/合规模块返回并按语言本地化。
...
| Performance | cookie_analytics | 读取目标国家配置 | 是 | 性能和体验分析 |
| Functional | cookie_functional | 读取目标国家配置 | 是 | 偏好记忆与非广告个性化 |
| Targeting | cookie_marketing | 读取目标国家配置 | 是 | 广告和营销定向 |

选择作用于当前浏览器/设备；登录时设备与账户同意取更保护用户的一侧，任一侧关闭则账户侧关闭，并回写当前设备。

**模块三：Do Not Sell or Share**：是否适用于目标国家由合规模块返回。适用时，`dns_opt_out=false` 表示未退出；用户开启后写 true，停止出售或用于跨情境行为广告共享。该选择独立于 Cookie 开关；即使 Targeting 仍为 ON，也不得继续执行被用户退出的共享行为。不适用时返回 `not_applicable`，前端不显示该控制模块，且不得沿用旧国家 dns_opt_out 值。Privacy & Data 中其余可见模块保持原顺序平铺；V5 原型表示三模块均适用的 Case。

**GPC**：检测到浏览器 GPC 信号时自动视为 opt-out，写入 `gpc_detected=true`、`dns_opt_out=true`、`consent_source=GPC`。前端无独立 GPC 开关。

**开关统一反馈规则**：

| 适用操作 | 保存成功 | 保存失败 | 失败后的 UI |
|---|---|---|---|
| Email Subscription | `Preference updated.` | `Update failed. Please try again.` | 回滚为切换前状态 |
| Performance | `Preference updated.` | `Update failed. Please try again.` | 回滚为切换前状态 |
| Functional | `Preference updated.` | `Update failed. Please try again.` | 回滚为切换前状态 |
| Targeting | `Preference updated.` | `Update failed. Please try again.` | 回滚为切换前状态 |
| Do Not Sell or Share | `Preference updated.` | `Update failed. Please try again.` | 回滚为切换前状态 |

- 成功 Toast 仅在服务端确认保存后显示，约 3 秒后自动消失；i18n key 为 `account.preference.toast.success`。
- 失败 Toast 在网络异常、超时或服务端保存失败时显示，约 3 秒后自动消失；i18n key 为 `account.preference.toast.failure`。
- 同一页面连续成功切换多个开关时，只保留最新一条成功 Toast，不叠加多个相同提示；各开关仍分别完成保存。
- 失败 Toast 优先展示，不得被成功 Toast 合并或覆盖；对应开关必须回滚，其他已保存成功的开关保持新状态。
- Strictly Necessary 不可切换，不触发 Toast；GPC 自动生效不是用户主动切换，不触发该 Toast。
- Toast 文案按当前语言显示，不包含 Email、用户标识或隐私选择明细。游客邮箱 Subscribe 按钮继续使用“游客提交反馈”的专用 Toast，不适用本规则。

同意读取失败时不得把用户已有的退出选择重置为默认值；保存失败时按统一规则回滚 UI 并提示。审计历史由全局合规能力保存，个人中心 ER 只表示当前态。
...
| 隐私合规 | 按 market_id + country_code 返回 Cookie/Do Not Sell 默认配置、已保存状态、GPC、同意审计、DSAR 编排 | 合规/用户中心 |
...
| 国家切换部分成功 | 政策、订阅偏好或隐私设置混用两个国家 | 目标区域关键数据全部成功后一次性替换；任一失败保留旧区域完整状态 |
| 旧国家缓存污染 | 切换后短暂或持续展示旧政策/旧隐私选择 | 所有请求与缓存键包含 market_id + country_code；切换时清理页面旧状态并显示加载态 |
| 目标国家政策配置缺失 | 入口无效或用户无法查看适用条款 | 首发门禁校验五项政策均有有效版本和真实 URL；运行时显示不可用且不可跨国兜底 |
| 隐私数据模型未同步地域键 | 无法可靠恢复不同国家的独立选择 | 开发方案冻结前将 privacy_consent/device consent 与订阅状态的地域键同步至对应 ER/数据模型 |
...
- Country/Region 按 C 端有效国家数量条件显示（1 个隐藏，>1 个显示）、Language/Currency、切换国家强制重置默认语言与默认币种，并同步切换政策、Email Subscriptions 与 Privacy & Data。
...
- Subscriptions 游客提交成功/失败 Toast，多语言文案与格式错误内联提示分流。
- Email Subscription、Cookie Preferences、Do Not Sell or Share 开关使用统一成功/失败 Toast 和失败回滚规则。
...
| privacy_preference_change | Cookie/Do Not Sell 变更 | market_id、country_code、preference、from、to、source |
...
| privacy_consent | 是 | 当前账户各国家隐私选择，按 user_id + market_id + country_code 隔离；审计历史由全局合规能力拥有 |
| device_consent_store | 逻辑实体 | 当前设备各国家隐私选择，按 anonymous/device subject + market_id + country_code 隔离，不是 DB 表 |
...
| 页面 | PRD 字段 | 原型实际 | 结果 |
|---|---|---|---|
| Profile | Email/Nickname/Gender/Change Password/Log Out；Gender null 显示 Not set | V5 覆盖字段与已设置 Case | UI 一致；未设置 Case 由 PRD 补充 |
...
| Privacy & Data | Manage Data/Cookie Preferences/Do Not Sell；按国家读取适用性与状态 | V5 固定顺序平铺，表示三模块均适用 Case | UI 一致；地域状态由 PRD 补充 |
...
5. 切换国家成功后，语言、币种、政策、Email 订阅偏好、Cookie Preferences、Do Not Sell or Share 一次性切到目标国家；切回后恢复该国家已保存的区域数据。
6. 区域切换任一关键配置失败时，页面继续展示切换前完整状态并提示重试，不出现两个国家的混合数据。
7. Do Not Sell or Share 对目标国家不适用时不显示该控制模块，且不沿用旧国家值；显式 `not_applicable` 不视为加载失败。
...
10. 所有政策、订阅偏好和隐私状态请求及缓存均按 `market_id + country_code` 隔离，服务端不接受只依赖语言或前端缓存推断国家的结果。
11. 游客首次订阅成功或重复订阅幂等成功时，均显示成功 Toast `You're subscribed!`，停留当前页且不重复发送请求。
12. 游客订阅因网络异常、超时或服务端失败时，显示失败 Toast `Subscription failed. Please try again.`，保留 Email 并恢复 Subscribe 可点击；格式错误只显示输入框内联提示，不显示失败 Toast。
13. Email Subscription、Performance、Functional、Targeting、Do Not Sell or Share 保存成功后均显示 `Preference updated.`；保存失败均显示 `Update failed. Please try again.` 并将对应开关回滚。
14. 同一页面连续成功切换多个开关时不叠加相同 Toast；失败提示不得被成功提示覆盖。Strictly Necessary、GPC 自动生效和游客 Subscribe 按钮不使用开关统一 Toast。
