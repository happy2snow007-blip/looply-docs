...
未登录账户访问页同时适用于已退出用户、从未注册/登录的访客及会话失效用户，三种场景不得展示不同提示或不同按钮。PC 与移动端使用同一 i18n 内容：正文为 `Sign in or create an account to view orders, saved items, and settings.`，主按钮为 `Sign In or Create Account`；PC 可额外显示中性标签 `ACCOUNT ACCESS` 和标题 `Access your account`，但不得出现 `SIGNED OUT`、`You've been logged out` 或 `Sign In Again`。主按钮进入登录注册模块的统一认证入口，成功登录或注册后回个人中心首页或原始受保护 `returnUrl`。
