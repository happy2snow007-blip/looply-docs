...
> **v1.7 变更**：Change Password 明确跳转并引用登录注册模块的已登录改密流程，个人中心不处理密码字段或校验；My Orders、Profile、Addresses 统一采用携带精确 `returnUrl` 的登录路由守卫，登录成功后自动进入原目标页；Subscriptions 按登录状态分流，访客直接使用 Email + Subscribe，不跳登录。Privacy & Data 中 Cookie Preferences 与 Do Not Sell or Share 本期仅实现 V5 视觉展示，控件不可操作、不读写隐私同意数据、不调用合规能力；Email Subscription 仍为真实开关并保留成功/失败 Toast。
...
> **v1.4 历史变更**：Policies 页面按 V5 UI 效果实现 PC/Mobile 响应式展示，全部页面文案支持多语言；五类政策按 `market_id + country_code` 获取，国家之间允许使用不同政策正文、版本、生效时间和 URL。游客订阅 Email 基础格式校验，以及 Gender 未设置时的 `null` 展示与保存规则仍有效。原“切换国家时刷新 Cookie Preferences、Do Not Sell or Share”的规则已由 v1.7 取代，不再适用本期。
...
> **数据模型同步说明**：政策与 Email 订阅偏好继续按 `market_id + country_code` 隔离。本期个人中心不读写账户/设备隐私同意数据，Cookie 与 Do Not Sell 的实际数据模型、地域规则及合规执行留待后续隐私合规模块定义。
...
| 角色 | 说明 |
|---|---|
| 已登录买家 | 使用完整个人中心、账户级订单/偏好/订阅与咨询功能；Privacy & Data 的 Cookie/Do Not Sell 仅可查看 |
...
5. 用户查看 Privacy & Data 页面中的 Manage Your Data、Cookie Preferences 与 Do Not Sell/Share 信息；后两者本期不可操作。
...
10. 国家切换必须同时加载目标 `market_id + country_code` 下的五类政策与 Email 订阅偏好；这些数据不得沿用切换前国家的缓存或默认值。用户切回原国家时重新读取该国家已保存状态，不复制其他国家的选择。Privacy & Data 的 Cookie/Do Not Sell 为固定视觉占位，不参与此读取。
11. 区域切换采用整体提交：只有目标国家的 `market_id + country_code + language + currency`、政策目录和订阅偏好均返回有效结果后，前端才一次性展示新国家状态。缺少配置或请求失败不属于有效结果；任一关键项失败则保持切换前完整状态并提示重试，不得出现政策已切换但订阅偏好仍属于旧国家等混合状态。
...
保存或国家站上下文切换失败时保留原生效的区域上下文、政策和订阅偏好并提示重试；不允许只更新其中一部分。Market 模块返回 0 个有效国家、归属不匹配、目标关联停用、缺少国家默认语言/币种、默认语言未在多语言系统启用、默认币种不受归属 Market 支持或目标国家区域配置不完整时，不切换当前国家站并记录配置告警。
...
**功能描述**：在一个页面按固定顺序平铺 Manage Your Data、Cookie Preferences、Do Not Sell or Share My Personal Information。页面结构和可见文案按 V5 与多语言系统渲染；本期仅提供信息展示，不将本页面作为 Cookie 同意或 Do Not Sell/Share 的实际处理入口。

**本期边界**：Cookie Preferences 与 Do Not Sell or Share 的开关仅展示视觉状态，必须置为 disabled/不可操作，不能响应点击、键盘切换或辅助功能操作；不得伪造已保存状态。个人中心不得调用 Cookie 同意、Do Not Sell/Share、GPC 或同意审计接口，不读写 `privacy_consent`、`device_consent_store` 或浏览器 Cookie/本地存储中的隐私选择，也不展示成功/失败 Toast。实际的地域适用性、默认值、同意持久化、GPC 识别和合规执行均不在本期，后续由隐私合规模块定义并接入。
...
**模块二：Cookie Preferences**：按 V5 展示 Strictly Necessary、Performance、Functional、Targeting 及其说明。所有开关均为不可操作的视觉控件，不配置点击事件、保存逻辑或状态读写；Strictly Necessary 继续以不可关闭的静态状态展示。页面可翻译的标题、分类名称、说明与“暂不可设置”提示使用 UI message package。

**模块三：Do Not Sell or Share**：按 V5 展示标题、说明与不可操作的视觉开关。该模块本期固定展示，不根据国家适用性隐藏，不返回 `not_applicable`，也不产生 `dns_opt_out`、GPC 或广告共享控制效果。实际合规口径上线后再由隐私合规模块决定展示条件与交互。

**真实开关反馈规则（仅 Email Subscription）**：登录态 Email Subscription 保存成功后显示 `Preference updated.`；网络异常、超时或订阅管理模块保存失败时回滚原状态并显示 `Update failed. Please try again.`。成功与失败 Toast 均约 3 秒自动消失，按当前语言显示，连续成功操作不叠加相同 Toast，失败提示不得被成功提示覆盖。Cookie Preferences、Do Not Sell or Share 不适用该规则，也不显示任何交互 Toast。
...
| 隐私合规 | 后续负责 Cookie/Do Not Sell、GPC、同意审计与 DSAR；本期个人中心不调用 | 后续隐私合规模块 |
...
- Cookie Preferences 与 Do Not Sell/Share 本期为不可操作的视觉占位；不得接入 GPC、Cookie 分类控制、同意审计或相关浏览器存储。
...
| 隐私占位被误接入实际控制 | 用户误以为选择已生效或产生未评审合规行为 | Cookie/Do Not Sell 控件固定 disabled；代码评审禁止读写同意数据或接入 GPC，真实能力由后续隐私合规模块交付 |
...
| 国家切换部分成功 | 政策、订阅偏好混用两个国家 | 目标区域关键数据全部成功后一次性替换；任一失败保留旧区域完整状态 |
| 旧国家缓存污染 | 切换后短暂或持续展示旧政策/旧订阅选择 | 所有请求与缓存键包含 market_id + country_code；切换时清理页面旧状态并显示加载态 |
...
- Country/Region 按 C 端有效国家数量条件显示（1 个隐藏，>1 个显示）、Language/Currency、切换国家强制重置默认语言与默认币种，并同步切换政策与 Email Subscriptions。
...
- 登录态 Email Subscription 使用成功/失败 Toast 和失败回滚规则；Cookie Preferences、Do Not Sell or Share 仅视觉展示且不可操作。
...
- 共享设备匿名 ID 轮换治理。
- Cookie 同意、Do Not Sell/Share、GPC、同意审计与实际隐私偏好持久化。
...
| country_change | 保存 Country/Region | from/to market_id、from/to country_code、from/to language、from/to currency、default_reset=true、policy_refresh_result、subscription_refresh_result、result |
...
| privacy_consent | 否 | 本期个人中心不读写；后续由隐私合规模块定义账户隐私选择与审计 |
| device_consent_store | 否 | 本期个人中心不读写；后续由隐私合规模块定义设备级隐私选择 |
...
| Privacy & Data 个人中心页 | 本期不可用 | 可用；Cookie/Do Not Sell 仅展示且不可操作 | 后续合规后台另管 |
...
| Privacy & Data | Manage Data/Cookie Preferences/Do Not Sell；后两者为 disabled 视觉控件，不读写状态 | V5 固定顺序平铺，表示三模块展示 Case | UI 一致；不可操作与后续合规边界由 PRD 补充 |
...
5. 切换国家成功后，语言、币种、政策、Email 订阅偏好一次性切到目标国家；切回后恢复该国家已保存的区域数据。Cookie Preferences 与 Do Not Sell 不参与国家状态读取或恢复。
6. 区域切换任一关键配置失败时，页面继续展示切换前完整状态并提示重试，不出现两个国家的混合数据。
7. Privacy & Data 固定平铺 Manage Your Data、Cookie Preferences、Do Not Sell or Share；Cookie Preferences 与 Do Not Sell 所有控件均不可操作，不因国家切换发生状态读写，也不显示 `not_applicable`。
...
10. 所有政策与订阅偏好请求及缓存均按 `market_id + country_code` 隔离，服务端不接受只依赖语言或前端缓存推断国家的结果。
...
13. 登录态 Email Subscription 保存成功显示 `Preference updated.`；保存失败显示 `Update failed. Please try again.` 并回滚。Cookie Preferences 与 Do Not Sell 不得触发保存、回滚或 Toast。
14. 连续成功切换 Email Subscription 时不叠加相同 Toast；失败提示不得被成功提示覆盖。游客 Subscribe 继续使用专用成功/失败 Toast。
