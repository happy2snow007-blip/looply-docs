...
- Policies 列表按当前 Figma 信息结构实现 PC/Mobile 响应式展示；PC 政策详情为无个人中心左侧导航的单栏阅读页，必须具有 `Policies / 当前政策标题` 两级面包屑。本期由开发维护单页或多页面静态 HTML 路由，不接 CMS；各实现均须支持五类入口、多语言、区域隔离、加载/失败状态与返回路径。
