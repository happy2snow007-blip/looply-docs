...
| C 端 UI 设计稿 | 移动端：[Looply v1.0](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=5832-8748&p=f&m=dev)；PC 端：[Looply v1.0](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=6187-229&p=f&m=dev) | 页面、字段、平台差异、登录态与多市场设置以 Figma 为准 |
...
> **当前 UI 基线**：移动端采用 [Looply v1.0 移动端 UI](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=5832-8748&p=f&m=dev)，PC 端采用 [Looply v1.0 PC 端 UI](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=6187-229&p=f&m=dev)。除明确标注为历史变更的 V5/V7/V8 记录外，本文所有“UI 关联”“设计稿”“原型实际”和 UI 验收项均指向上述 Figma；本地 `.pen` 文件不再作为开发参考基线。

> **v1.19 变更**：PC 端左侧目录打开的一级页面统一不展示面包屑；仅从一级页面继续进入二级子页面时展示。Policies 详情使用 `Policies / 当前政策标题`；Addresses 的 Add New Address、Edit Address 使用 `Addresses / Add New Address`、`Addresses / Edit Address`。

> **v1.18 变更**：PRD C 端 UI 参考统一切换至 Looply v1.0 Figma 移动端及 PC 端链接；历史版本命名仅用于追溯，不再作为当前开发依据。

> **v1.17 变更**：PC 未登录账户访问页删除 `My Account` 下方重复的副标题。页面顶部只保留标题，账户访问说明仅在主卡片展示，避免与 `Access your account` 卡片说明重复。

> **v1.16 变更**：PC 未登录账户访问卡片删除 `Need help? Visit Contact Us anytime.` 辅助联系文案，不在账户访问主行动内重复展示 Contact Us；用户仍可从站点导航或个人中心目录进入 Contact Us。

> **v1.15 变更**：PC 与移动端的已退出/首次访问状态统一为“未登录账户访问页”，不再显示“已登出”“再次登录”等假设用户曾登录过的文案。统一正文为 `Sign in or create an account to view orders, saved items, and settings.`，统一主按钮为 `Sign In or Create Account`，点击均进入登录注册模块的统一认证入口。
...
| 未登录访客 | 看到未登录账户访问引导；可通过独立 Subscriptions 游客页订阅邮箱，也可直接进入 Privacy & Data 并按匿名设备保存偏好；设备级收藏/浏览由专项入口承载 |
...
├─ Addresses（需登录）→ 地址列表 → Add New Address / Edit Address
...
└─ Log Out → 二次确认 → 未登录账户访问页 → 登录注册模块统一认证入口
...
**前置条件**：用户已登录。未登录访问个人中心时进入未登录账户访问页，不展示完整目录。
...
| 页面层级 | 是否展示面包屑 | 规则 |
|---|:---:|---|
| 左侧目录一级页 | 否 | 从 PC 左侧目录直接进入的 Account Home、My Orders、Wishlist、Recently Viewed、Profile、Addresses 列表、Country/Region、Language、Currency、Subscriptions、Privacy & Data、Contact Us、Policies 列表、About 均不展示面包屑。 |
| Addresses 二级操作页 | 是 | 从 Addresses 列表进入 Add New Address 或 Edit Address 时，保留左侧目录；固定两级，一级可点击 `Addresses` 返回地址列表，二级为 `Add New Address` 或 `Edit Address`，纯文本不可点击。不得增加 Home、Account 或其他层级。 |
| Policies 二级详情页 | 是 | 从 Policies 列表进入具体政策时，不展示左侧目录；固定两级 `Policies / 当前政策标题`，一级可点击返回列表，二级纯文本不可点击。不得增加 Home、Account 或其他层级。 |

一级目录页即使通过浏览器刷新、深链或登录后的 `returnUrl` 直接访问，仍不得补充面包屑；二级页深链按其父级渲染相同面包屑。浏览器返回优先返回来源页，无来源时返回对应父级列表。
...
| 商品预览 | 使用商品模块返回的当前国家站可展示 listing；卡片数量、宽度和露出比例以当前 Figma PC UI 为准。卡片展示商品图片、品牌、商品名与当前价格；折扣、原价等动态字段由商品模块返回时展示 |
...
**状态变体**：已登录默认态、昵称为空（邮箱前缀兜底）、订单计数加载态、部分数据读取失败、未登录账户访问、退出确认。
...
**UI 关联**：[Looply v1.0 移动端 UI](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=5832-8748&p=f&m=dev)；[Looply v1.0 PC 端 UI](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=6187-229&p=f&m=dev)。
...
**UI 关联**：上述 Figma 移动端 UI 与 PC 端 UI 中的 Profile 页面及编辑状态。
...
14. PC Addresses 列表为左侧目录一级页，不显示面包屑；从其进入 Add New Address 或 Edit Address 时，地址模块按本 PRD 的二级面包屑规则渲染 `Addresses / Add New Address` 或 `Addresses / Edit Address`。一级面包屑返回 Addresses 列表；保存成功、Cancel、浏览器返回均回到来源地址列表或 Addresses 列表。
...
**UI 关联**：上述 Figma 移动端 UI 与 PC 端 UI 中的 Country/Region、Language、Currency 页面及状态。Country/Region 行与 PC 导航项采用条件渲染，不新增页面；设计稿中的入口代表“有效国家数 > 1”Case。
...
**UI 关联**：上述 Figma 移动端 UI 与 PC 端 UI 中的 Subscriptions 登录态及未登录态。

### 2.5 Privacy & Data

**功能描述**：在一个页面按固定顺序平铺 Manage Your Data、Cookie Preferences、Do Not Sell or Share My Personal Information。页面结构和可见文案按当前 Figma UI 与多语言系统渲染；Cookie Preferences 与 Do Not Sell or Share 均为可操作开关，登录用户按账户、未登录访客按当前匿名设备，在当前国家站记录偏好。
...
**模块二：Cookie Preferences**：按当前 Figma UI 展示 Strictly Necessary、Performance、Functional、Targeting 及其说明。Strictly Necessary 固定 ON 且不可关闭，不产生偏好记录；其余三项均可切换、按当前已保存值展示。United States 与 Hong Kong 在当前主体和国家站不存在历史记录时，三项均默认 ON；用户切换后以已保存值为准。页面可翻译的标题、分类名称、说明与 Toast 使用 UI message package。
...
**模块三：Do Not Sell or Share**：按当前 Figma UI 展示标题、说明与可操作开关。`dns_opt_out=false` 表示未选择退出，开启后保存为 `true`。该模块本期固定展示，不根据国家适用性隐藏；只记录偏好，不停止出售/共享数据或跨情境行为广告。实际展示条件和合规执行由后续隐私合规模块定义。
...
**UI 关联**：上述 Figma 移动端 UI 与 PC 端 UI 中的 Privacy & Data 页面。
...
**UI 关联**：上述 Figma 移动端 UI 与 PC 端 UI 中的 Contact Us 页面。
...
**功能描述**：不分 POLICIES/LEGAL 组，按当前 Figma UI 效果平铺五类固定入口；PC Web、Mobile Web 和 APP 使用一致的政策列表信息结构并按屏幕宽度适配。本期不建设政策 CMS 或配置后台，开发直接维护静态政策 HTML 页面壳、版本、地区适用关系、生效时间与真实 URL；政策标题、摘要与正文从翻译中心读取已发布译文，状态和错误提示使用 UI message package；不得在前端写死 English。PC Web 点击可用政策后进入独立政策详情页；列表页保留个人中心左侧导航，详情页不保留该导航。
...
**UI 关联**：上述 Figma 移动端 UI 与 PC 端 UI 中的 Policies 列表及政策详情页面。
...
**UI 关联**：上述 Figma 移动端 UI 与 PC 端 UI 中的 About 页面。

### 2.10 Log Out 与未登录账户访问

**操作流程**：点击 Log Out → 打开二次确认 → Cancel 留在当前页；确认后清理前端 access/refresh token，并请求用户中心终止 session → 跳站点首页或未登录账户访问页。

退出接口失败时前端仍清理本地 token，避免用户误以为仍安全退出失败；同时记录异常。账户级订单/地址不可见，但设备匿名域收藏和浏览原件保留。

未登录账户访问页同时适用于已退出用户、从未注册/登录的访客及会话失效用户，三种场景不得展示不同提示或不同按钮。PC 与移动端使用同一 i18n 内容：正文为 `Sign in or create an account to view orders, saved items, and settings.`，主按钮为 `Sign In or Create Account`；PC 可额外显示中性标签 `ACCOUNT ACCESS` 和标题 `Access your account`，但不得出现 `SIGNED OUT`、`You've been logged out` 或 `Sign In Again`。PC 页面顶部只保留 `My Account` 标题，不展示重复的副标题；PC 卡片不展示 `Need help? Visit Contact Us anytime.` 或其他辅助联系文案，以避免和全局/个人中心 Contact Us 入口重复。主按钮进入登录注册模块的统一认证入口，成功登录或注册后回个人中心首页或原始受保护 `returnUrl`。

**UI 关联**：上述 Figma 移动端 UI 与 PC 端 UI 中的退出确认及未登录账户访问页面。
...
- PC 左侧目录的一级页面均不展示面包屑；Policies 详情与 Addresses 的 Add New Address/Edit Address 仅作为二级页展示两级面包屑。Policies 列表按当前 Figma 信息结构实现 PC/Mobile 响应式展示；PC 政策详情为无个人中心左侧导航的单栏阅读页，必须具有 `Policies / 当前政策标题` 两级面包屑。本期由开发维护单页或多页面静态 HTML 路由，不接 CMS；各实现均须支持五类入口、多语言、区域隔离、加载/失败状态与返回路径。
...
- 当前 Figma 移动端 UI 与 PC 端 UI 中的全部正式页面与状态。
- Processing/Shipped/Delivered/Refunds/View All 订单入口；Refunds 仅查看取消、退货及退款进度或结果。
- Profile 字段以当前 Figma UI 为准；Gender 未设置为 null，不默认选中 Prefer not to say。
...
|---|---|---|
| 个人中心首页 | [Looply v1.0 移动端 UI](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=5832-8748&p=f&m=dev) | [Looply v1.0 PC 端 UI](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=6187-229&p=f&m=dev) |
| 退出确认/未登录账户访问 | 上述移动端 UI | 上述 PC 端 UI |
| Profile | 上述移动端 UI | 上述 PC 端 UI |
| Country/Region | 上述移动端 UI | 上述 PC 端 UI |
| Language | 上述移动端 UI | 上述 PC 端 UI |
| Currency | 上述移动端 UI | 上述 PC 端 UI |
| Addresses | 跳地址库模块，移动端页面不在本需求 | 上述 PC 端 UI |
| Subscriptions 登录态 | 上述移动端 UI | 上述 PC 端 UI |
| Subscriptions 游客态 | 上述移动端 UI | 上述 PC 端 UI |
| Privacy & Data | 上述移动端 UI | 上述 PC 端 UI |
| Contact Us | 上述移动端 UI | 上述 PC 端 UI |
| Policies | 上述移动端 UI | 上述 PC 端 UI |
| About | 上述移动端 UI | 上述 PC 端 UI |
...
| 页面 | PRD 字段 | 原型实际 | 结果 |
|---|---|---|---|
| Profile | Email/Nickname/Gender/Change Password/Log Out；Gender null 显示 Not set；Change Password 跳登录注册模块 | Figma 覆盖字段与已设置 Case | UI 一致；未设置 Case 与外部改密边界由 PRD 补充 |
...
| Privacy & Data | Manage Data/Cookie Preferences/Do Not Sell；后两者可操作、按当前国家站读取和保存偏好，切换后提示 Preference updated | Figma 固定顺序平铺，表示三模块展示 Case | UI 一致；偏好持久化与不触发实际合规动作由 PRD 补充 |
| Contact Us | Full Name/Email*/Message*/Send Message/reCAPTCHA | 一致 | 一致 |
| Policies 列表 | 五类固定入口；政策版本与 URL 按 market_id + country_code 返回 | Figma 覆盖 PC/Mobile 五项列表 | UI 一致；动态地域规则由 PRD 补充 |
| PC 政策详情 | 无个人中心左侧导航；Policies / 当前标题两级面包屑；标题、时间、富文本正文、不可用与返回状态 | Figma 覆盖政策详情示例 | UI 一致；其他四类政策复用相同详情结构 |
| About | 品牌文案；仅 APP Version | 已拆 Mobile Web/APP/PC | 一致 |
| Account Home | Figma 订单标签；Country/Region 条件入口；PC Recently Viewed/Wishlist 商品预览；Mobile 昵称为空时展示邮箱前缀 | Figma PC 按 Recently Viewed → Wishlist 顺序展示横向卡片条、轮播箭头和详情小箭头；移动端不展示 | 商品卡尺寸/露出按 Figma；列表业务归专项 PRD |
...
- C 端 UI：移动端 [Looply v1.0](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=5832-8748&p=f&m=dev)；PC 端 [Looply v1.0](https://www.figma.com/design/rLK7XCdVvYqEHQHd7WjOkk/Looply-v1.0?node-id=6187-229&p=f&m=dev)
...
1. Policies 列表在 PC Web、Mobile Web、APP 均按当前 Figma 信息结构展示，长译文可换行且无横向溢出；页面数量和路由形态不作为验收项。
...
15. PC Account Home 固定按 Recently Viewed、Wishlist 顺序展示完整宽度商品预览区；商品卡当前国家站、数量/尺寸、横向露出以当前 Figma PC UI 为准。支持左右滑动和轮播箭头，标题行右侧小箭头进入对应专项列表；移动端/APP 首页不展示该预览区。个人中心不得在本 PRD 定义专项列表规则。
...
23. PC 左侧目录打开的一级页面一律不展示面包屑。仅 Addresses → Add New Address/Edit Address、Policies → 政策详情等二级页面展示两级面包屑；不得出现 Home、Account 或超过两级的路径。Addresses 二级页一级固定为 `Addresses`，Policies 二级页一级固定为 `Policies`，均可点击返回各自父级列表。
