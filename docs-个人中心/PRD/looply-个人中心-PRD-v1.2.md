...
| 政策正文 | 标题、正文、富文本 | 外部动态业务内容 | 本模块不注册卡片 | 外部政策/CMS 模块定义 | 外部模块定义 | 外部模块定义 | 政策正文页 | 同一政策版本可含 Global/Market/国家适用章节；按 `market_id + country_code` 返回适用内容与本地化 URL |
...
|---|---|---|---|
| Country/Region | Market 模块允许 C 端使用的国家/地区；示例 United States、Hong Kong | 已保存 country_code；无值时使用默认国家，market_id 由服务端归属关系确定 | 单选后 Save；原子更新 country_code、market_id、language、currency 并刷新国家站相关数据 |
...
2. 保存时服务端校验国家归属 Market 且关联可用，读取目标国家在 Market 模块配置的 `default_language_code + default_currency_code`，再原子更新用户/设备当前 `country_code + market_id + language + currency`；默认语言必须是多语言系统启用值，默认币种必须是归属 Market 支持值。前端不得自由提交这四项组合，IP 仅可推荐国家。
3. 商品列表、Wishlist、Recently Viewed 及其 PC 数量均按当前 `market_id + country_code` 查询；收藏/浏览实体自身保存两字段，不再仅通过 listing 过滤。
4. 其他 Market/国家的收藏和浏览记录只隐藏、不删除；用户切回原国家后恢复展示。
5. 每次切换 Country/Region 都强制覆盖为目标国家默认 Language 与 Currency，即使原值在目标国家仍可用、或在同一 Market 内切换国家也不保留；不按国家记忆历史手动选择。
6. 切换成功提示 `Country/region updated. Language changed to {language}; currency changed to {currency}.`，并立即按新语言重载静态 UI、政策内容及错误提示，按新币种刷新金额展示。
7. 地址选择、格式和可配送范围由地址模块控制；个人中心不根据 market_id/country_code 改写地址数据。
8. 国家未来调整所属 Market 时，历史收藏/浏览的地域快照不静默改写；如需迁移必须走显式数据迁移方案。

登录用户以 `user_preference` 为跨端权威值；设备偏好用于首次体验和缓存。登录时账户无偏好则设备值 seed；账户已有值则账户优先并回写设备。

保存或国家站上下文切换失败时保留原生效的 `market_id + country_code + language + currency` 并提示重试；不允许四项只更新一部分。Market 模块返回空国家列表、归属不匹配、目标关联停用、缺少国家默认语言/币种、默认语言未在多语言系统启用或默认币种不受归属 Market 支持时，不切换当前国家站并记录配置告警。

**UI 关联**：`V5-移动端-选择国家地区`、`V5-PC-CountryRegion`、`V5-移动端-选择语言`、`V5-PC-Language`、`V5-移动端-选择币种`、`V5-PC-Currency`、`V5-移动端-切换市场-币种回退Case`。原型 Case 的产品语义升级为“切换国家后默认语言与默认币种同时重置”；页面结构不变，成功提示按本节文案实现。
...
个人中心只维护入口名称、摘要和目标 URL，不拥有正文。五项政策各自只维护一个当前版本，不因 Market/国家拆成多份平行文件；同一版本可包含 Global、Market、国家适用章节，由政策/CMS 模块根据当前 `market_id + country_code` 返回适用内容、锚点或 URL。目标 URL 缺失时不得渲染可点击空链接；读取失败显示重试。Cookie Policy 不在该五项列表中。
...
| Market/国家配置异常 | 无法切换国家站或币种不合法 | 切换前校验国家归属 Market、关联可用和支持币种；异常保留原 market_id + country_code 并告警 |
...
- Profile V5 字段。
- Country/Region、Language/Currency、切换国家强制重置默认语言与默认币种、Email Subscriptions、Privacy & Data。
...
| country_change | 保存 Country/Region | from/to market_id、from/to country_code、from/to language、from/to currency、default_reset=true、result |
...
| user_preference | 是 | 登录态 market_id/country_code/language/currency |
